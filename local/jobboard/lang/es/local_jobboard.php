<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Cadenas de idioma para local_jobboard (Español).
 *
 * @package   local_jobboard
 * @copyright 2024-2025 ISER - Instituto Superior de Educación Rural
 * @author    Alonso Arias <soporteplataformas@iser.edu.co>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

// =============================================================================
// IDENTIFICACIÓN DEL PLUGIN
// =============================================================================

$string['pluginname'] = 'Bolsa de Empleo';
$string['pluginname_desc'] = 'Sistema de bolsa de empleo para reclutamiento docente y gestión de vacantes';
$string['jobboard'] = 'Bolsa de Empleo';
$string['jobboard:view'] = 'Ver bolsa de empleo';
$string['jobboard:manage'] = 'Gestionar bolsa de empleo';

// =============================================================================
// CAPACIDADES
// =============================================================================

$string['jobboard:view'] = 'Ver bolsa de empleo';
$string['jobboard:viewinternal'] = 'Ver vacantes internas';
$string['jobboard:viewpublicvacancies'] = 'Ver vacantes públicas';
$string['jobboard:viewinternalvacancies'] = 'Ver vacantes internas';
$string['jobboard:manage'] = 'Gestionar vacantes';
$string['jobboard:createvacancy'] = 'Crear vacantes';
$string['jobboard:editvacancy'] = 'Editar vacantes';
$string['jobboard:deletevacancy'] = 'Eliminar vacantes';
$string['jobboard:publishvacancy'] = 'Publicar vacantes';
$string['jobboard:viewallvacancies'] = 'Ver todas las vacantes';
$string['jobboard:manageconvocatorias'] = 'Gestionar convocatorias';
$string['jobboard:apply'] = 'Postularse a vacantes';
$string['jobboard:viewownapplications'] = 'Ver postulaciones propias';
$string['jobboard:viewallapplications'] = 'Ver todas las postulaciones';
$string['jobboard:changeapplicationstatus'] = 'Cambiar estado de postulación';
$string['jobboard:review'] = 'Revisar postulaciones';
$string['jobboard:validatedocuments'] = 'Validar documentos';
$string['jobboard:reviewdocuments'] = 'Revisar documentos';
$string['jobboard:assignreviewers'] = 'Asignar revisores';
$string['jobboard:downloadanydocument'] = 'Descargar cualquier documento';
$string['jobboard:evaluate'] = 'Evaluar candidatos';
$string['jobboard:viewevaluations'] = 'Ver evaluaciones';
$string['jobboard:manageworkflow'] = 'Gestionar flujo de trabajo';
$string['jobboard:viewreports'] = 'Ver reportes';
$string['jobboard:exportreports'] = 'Exportar reportes';
$string['jobboard:exportdata'] = 'Exportar datos';
$string['jobboard:configure'] = 'Configurar plugin';
$string['jobboard:managedoctypes'] = 'Gestionar tipos de documento';
$string['jobboard:manageemailtemplates'] = 'Gestionar plantillas de correo';
$string['jobboard:manageexemptions'] = 'Gestionar excepciones';
$string['jobboard:unlimitedapplications'] = 'Postulaciones ilimitadas';

// =============================================================================
// NAVEGACIÓN Y MENÚ
// =============================================================================

$string['dashboard'] = 'Panel de control';
$string['convocatorias'] = 'Convocatorias';
$string['vacancies'] = 'Vacantes';
$string['applications'] = 'Postulaciones';
$string['myapplications'] = 'Mis postulaciones';
$string['review'] = 'Revisión';
$string['myreviews'] = 'Mis revisiones';
$string['reports'] = 'Reportes';
$string['settings'] = 'Configuración';
$string['administration'] = 'Administración';
$string['manage'] = 'Gestionar';
$string['public'] = 'Público';
$string['browse'] = 'Explorar';
$string['home'] = 'Inicio';
$string['back'] = 'Volver';
$string['backtodashboard'] = 'Volver al panel';
$string['backtolist'] = 'Volver a la lista';

// =============================================================================
// ACCIONES COMUNES
// =============================================================================

$string['view'] = 'Ver';
$string['edit'] = 'Editar';
$string['delete'] = 'Eliminar';
$string['save'] = 'Guardar';
$string['cancel'] = 'Cancelar';
$string['create'] = 'Crear';
$string['update'] = 'Actualizar';
$string['submit'] = 'Enviar';
$string['confirm'] = 'Confirmar';
$string['close'] = 'Cerrar';
$string['open'] = 'Abrir';
$string['access'] = 'Acceder';
$string['search'] = 'Buscar';
$string['filter'] = 'Filtrar';
$string['reset'] = 'Restablecer';
$string['clear'] = 'Limpiar';
$string['apply'] = 'Aplicar';
$string['download'] = 'Descargar';
$string['upload'] = 'Subir';
$string['export'] = 'Exportar';
$string['import'] = 'Importar';
$string['print'] = 'Imprimir';
$string['preview'] = 'Vista previa';
$string['refresh'] = 'Actualizar';
$string['duplicate'] = 'Duplicar';
$string['archive'] = 'Archivar';
$string['restore'] = 'Restaurar';
$string['approve'] = 'Aprobar';
$string['reject'] = 'Rechazar';
$string['select'] = 'Seleccionar';
$string['selectall'] = 'Seleccionar todo';
$string['deselectall'] = 'Deseleccionar todo';
$string['actions'] = 'Acciones';
$string['options'] = 'Opciones';
$string['more'] = 'Más';
$string['less'] = 'Menos';
$string['showmore'] = 'Mostrar más';
$string['showless'] = 'Mostrar menos';
$string['viewall'] = 'Ver todo';
$string['viewdetails'] = 'Ver detalles';
$string['viewprofile'] = 'Ver perfil';
$string['continue'] = 'Continuar';
$string['finish'] = 'Finalizar';
$string['next'] = 'Siguiente';
$string['previous'] = 'Anterior';
$string['assign'] = 'Asignar';
$string['unassign'] = 'Desasignar';
$string['enable'] = 'Habilitar';
$string['disable'] = 'Deshabilitar';
$string['activate'] = 'Activar';
$string['deactivate'] = 'Desactivar';

// =============================================================================
// ETIQUETAS COMUNES
// =============================================================================

$string['name'] = 'Nombre';
$string['title'] = 'Título';
$string['description'] = 'Descripción';
$string['code'] = 'Código';
$string['status'] = 'Estado';
$string['type'] = 'Tipo';
$string['category'] = 'Categoría';
$string['date'] = 'Fecha';
$string['time'] = 'Hora';
$string['datetime'] = 'Fecha y hora';
$string['startdate'] = 'Fecha de inicio';
$string['enddate'] = 'Fecha de fin';
$string['closedate'] = 'Fecha de cierre';
$string['opendate'] = 'Fecha de apertura';
$string['deadline'] = 'Fecha límite';
$string['created'] = 'Creado';
$string['modified'] = 'Modificado';
$string['createdby'] = 'Creado por';
$string['modifiedby'] = 'Modificado por';
$string['timecreated'] = 'Fecha de creación';
$string['timemodified'] = 'Fecha de modificación';
$string['location'] = 'Ubicación';
$string['department'] = 'Departamento';
$string['company'] = 'Sede / Centro tutorial';
$string['faculty'] = 'Facultad';
$string['program'] = 'Programa';
$string['positions'] = 'Posiciones';
$string['salary'] = 'Salario';
$string['contracttype'] = 'Tipo de contrato';
$string['modality'] = 'Modalidad';
$string['schedule'] = 'Horario';
$string['requirements'] = 'Requisitos';
$string['benefits'] = 'Beneficios';
$string['notes'] = 'Notas';
$string['comments'] = 'Comentarios';
$string['observations'] = 'Observaciones';
$string['reason'] = 'Razón';
$string['priority'] = 'Prioridad';
$string['order'] = 'Orden';
$string['total'] = 'Total';
$string['count'] = 'Cantidad';
$string['quantity'] = 'Cantidad';
$string['percentage'] = 'Porcentaje';
$string['average'] = 'Promedio';
$string['yes'] = 'Sí';
$string['no'] = 'No';
$string['all'] = 'Todos';
$string['none'] = 'Ninguno';
$string['any'] = 'Cualquiera';
$string['other'] = 'Otro';
$string['unknown'] = 'Desconocido';
$string['notavailable'] = 'No disponible';
$string['notapplicable'] = 'No aplica';
$string['days'] = 'días';
$string['day'] = 'día';
$string['hours'] = 'horas';
$string['hour'] = 'hora';
$string['minutes'] = 'minutos';
$string['minute'] = 'minuto';
$string['ago'] = 'hace';
$string['remaining'] = 'restantes';
$string['daysremaining'] = 'Días restantes';
$string['page'] = 'Página';
$string['of'] = 'de';
$string['to'] = 'a';
$string['from'] = 'desde';
$string['for'] = 'para';
$string['by'] = 'por';
$string['at'] = 'a las';
$string['in'] = 'en';
$string['on'] = 'el';

// =============================================================================
// CADENAS DE ESTADO
// =============================================================================

// Estado general
$string['active'] = 'Activo';
$string['inactive'] = 'Inactivo';
$string['enabled'] = 'Habilitado';
$string['disabled'] = 'Deshabilitado';
$string['pending'] = 'Pendiente';
$string['approved'] = 'Aprobado';
$string['rejected'] = 'Rechazado';
$string['completed'] = 'Completado';
$string['cancelled'] = 'Cancelado';
$string['expired'] = 'Vencido';

// Estado de vacante
$string['status:draft'] = 'Borrador';
$string['status:published'] = 'Publicada';
$string['status:closed'] = 'Cerrada';
$string['status:archived'] = 'Archivada';

// Estado de convocatoria
$string['convocatoria_status_draft'] = 'Borrador';
$string['convocatoria_status_open'] = 'Abierta';
$string['convocatoria_status_closed'] = 'Cerrada';
$string['convocatoria_status_archived'] = 'Archivada';

// Estado de postulación
$string['appstatus:draft'] = 'Borrador';
$string['appstatus:submitted'] = 'Enviada';
$string['appstatus:under_review'] = 'En revisión';
$string['appstatus:in_progress'] = 'En proceso';
$string['appstatus:docs_validated'] = 'Documentos validados';
$string['appstatus:docs_rejected'] = 'Documentos rechazados';
$string['appstatus:interview'] = 'Entrevista';
$string['appstatus:selected'] = 'Seleccionado/a';
$string['appstatus:rejected'] = 'No seleccionado/a';
$string['appstatus:waitlist'] = 'Lista de espera';
$string['appstatus:withdrawn'] = 'Retirada';

// Cadena para observaciones guardadas (notificaciones deshabilitadas)
$string['observationssaved'] = 'Observaciones guardadas';

// Estado de documento
$string['docstatus:pending'] = 'Pendiente';
$string['docstatus:approved'] = 'Aprobado';
$string['docstatus:rejected'] = 'Rechazado';
$string['docstatus:resubmitted'] = 'Reenviado';
$string['uploaded'] = 'Cargado';

// Cadenas dinámicas de estado (usadas con get_string('status_' . $status))
// Estados de postulación - Nuevo flujo de trabajo v4.0
$string['status_submitted'] = 'Enviada';
$string['status_pending_dean_review'] = 'Pendiente revisión Decano';
$string['status_dean_approved'] = 'Aprobado por Decano';
$string['status_dean_rejected'] = 'Rechazado por Decano';
$string['status_pending_hr_validation'] = 'Pendiente validación T. Humano';
$string['status_hr_validated'] = 'Validado por Talento Humano';
$string['status_hr_rejected'] = 'Rechazado por Talento Humano';
$string['status_withdrawn'] = 'Retirada';
// Acciones de revisión
$string['startreview'] = 'Iniciar Revisión';
// Estados de vacante
$string['status_draft'] = 'Borrador';
$string['status_published'] = 'Publicada';
$string['status_closed'] = 'Cerrada';
$string['status_archived'] = 'Archivada';
$string['status_assigned'] = 'Asignada';

// Etiquetas de estado de vacante (usadas con get_string('vacancystatus:' . $status))
$string['vacancystatus:draft'] = 'Borrador';
$string['vacancystatus:published'] = 'Publicada';
$string['vacancystatus:closed'] = 'Cerrada';
$string['vacancystatus:archived'] = 'Archivada';
$string['vacancystatus:assigned'] = 'Asignada';

// Etiquetas de estado de convocatoria (usadas con get_string('convocatoriastatus:' . $status))
$string['convocatoriastatus:draft'] = 'Borrador';
$string['convocatoriastatus:open'] = 'Abierta';
$string['convocatoriastatus:closed'] = 'Cerrada';
$string['convocatoriastatus:archived'] = 'Archivada';

// =============================================================================
// CONVOCATORIAS
// =============================================================================

$string['convocatoria'] = 'Convocatoria';
$string['newconvocatoria'] = 'Nueva convocatoria';
$string['editconvocatoria'] = 'Editar convocatoria';
$string['deleteconvocatoria'] = 'Eliminar convocatoria';
$string['viewconvocatoria'] = 'Ver convocatoria';
$string['convocatorialist'] = 'Lista de convocatorias';
$string['convocatoriadetails'] = 'Detalles de la convocatoria';
$string['convocatoriacode'] = 'Código de convocatoria';
$string['convocatorianame'] = 'Nombre de la convocatoria';
$string['convocatoriadescription'] = 'Descripción';
$string['convocatoriaterms'] = 'Términos y condiciones';
$string['convocatoriapdf'] = 'PDF Acuerdo/Resolución';
$string['convocatoriapdf_help'] = 'Suba el documento oficial de acuerdo o resolución (ej. Acuerdo 050) en formato PDF';
$string['convocatoriaterms_pdf'] = 'PDF Términos de Convocatoria';
$string['convocatoriaterms_pdf_help'] = 'Suba el documento de términos y condiciones de esta convocatoria en formato PDF';
$string['convocatoriastartdate'] = 'Fecha de inicio';
$string['convocatoriaenddate'] = 'Fecha de cierre';
$string['convocatoriastatus'] = 'Estado';
$string['publishconvocatoria'] = 'Publicar convocatoria';
$string['closeconvocatoria'] = 'Cerrar convocatoria';
$string['archiveconvocatoria'] = 'Archivar convocatoria';
$string['reopenconvocatoria'] = 'Reabrir convocatoria';
$string['noconvocatorias'] = 'No se encontraron convocatorias';
$string['selectconvocatoria'] = 'Seleccione convocatoria';
$string['allconvocatorias'] = 'Todas las convocatorias';
$string['activeconvocatorias'] = 'Convocatorias activas';
$string['convocatoriavacancies'] = 'Vacantes en esta convocatoria';
$string['convocatoriaapplications'] = 'Postulaciones en esta convocatoria';
$string['convocatoriarequired'] = 'Por favor seleccione una convocatoria';
$string['convocatoriacreated'] = 'Convocatoria creada exitosamente';
$string['convocatoriaupdated'] = 'Convocatoria actualizada exitosamente';
$string['convocatoriadeleted'] = 'Convocatoria eliminada exitosamente';
$string['convocatoriapublished'] = 'Convocatoria publicada exitosamente';
$string['convocatoriaclosed'] = 'Convocatoria cerrada exitosamente';
$string['confirmdeleteconvocatoria'] = '¿Está seguro que desea eliminar esta convocatoria? Esta acción no se puede deshacer.';

// =============================================================================
// VACANTES
// =============================================================================

$string['vacancy'] = 'Vacante';
$string['newvacancy'] = 'Nueva vacante';
$string['editvacancy'] = 'Editar vacante';
$string['deletevacancy'] = 'Eliminar vacante';
$string['viewvacancy'] = 'Ver vacante';
$string['vacancylist'] = 'Lista de vacantes';
$string['vacancydetails'] = 'Detalles de la vacante';
$string['vacancycode'] = 'Código de vacante';
$string['vacancytitle'] = 'Título de la vacante';
$string['vacancydescription'] = 'Descripción';
$string['vacancyrequirements'] = 'Requisitos';
$string['vacancybenefits'] = 'Beneficios';
$string['vacancylocation'] = 'Ubicación';
$string['vacancydepartment'] = 'Departamento';
$string['vacancypositions'] = 'Número de posiciones';
$string['vacancycontracttype'] = 'Tipo de contrato';
$string['vacancymodality'] = 'Modalidad';
$string['vacancyschedule'] = 'Horario';
$string['vacancysalary'] = 'Salario';
$string['vacancystartdate'] = 'Fecha de inicio';
$string['vacancyenddate'] = 'Fecha de fin';
$string['vacancyclosedate'] = 'Fecha límite de postulación';
$string['vacancystatus'] = 'Estado';
$string['publishvacancy'] = 'Publicar vacante';
$string['unpublishvacancy'] = 'Despublicar vacante';
$string['closevacancy'] = 'Cerrar vacante';
$string['reopenvacancy'] = 'Reabrir vacante';
$string['duplicatevacancy'] = 'Duplicar vacante';
$string['novacancies'] = 'No se encontraron vacantes';
$string['novacancies_desc'] = 'No hay vacantes disponibles en este momento. Por favor, vuelva a consultar más tarde.';
$string['selectvacancy'] = 'Seleccione vacante';
$string['allvacancies'] = 'Todas las vacantes';
$string['activevacancies'] = 'Vacantes activas';
$string['closedvacancies'] = 'Vacantes cerradas';
$string['internalvacancies'] = 'Vacantes internas';
$string['publicvacancies'] = 'Vacantes públicas';
$string['vacancycreated'] = 'Vacante creada exitosamente';
$string['vacancyupdated'] = 'Vacante actualizada exitosamente';
$string['vacancydeleted'] = 'Vacante eliminada exitosamente';
$string['vacancypublished'] = 'Vacante publicada exitosamente';
$string['vacancyclosed'] = 'Vacante cerrada exitosamente';
$string['vacancyclosed_desc'] = 'Esta vacante ya no acepta postulaciones';
$string['vacancyclosed_draft'] = 'La vacante ha cerrado. Su postulación quedó guardada como borrador pero ya no puede ser enviada.';
$string['confirmdeletevacancy'] = '¿Está seguro que desea eliminar esta vacante? Esta acción no se puede deshacer.';
$string['vacancyhasapplications'] = 'Esta vacante tiene postulaciones y no puede ser eliminada';
$string['closingsoon'] = 'Cierra pronto';
$string['urgent'] = 'Urgente';
$string['newapplicants'] = 'Nuevos postulantes';
$string['totalapplicants'] = 'Total de postulantes';
$string['pagination'] = 'Navegación de páginas';
$string['applicationcount'] = 'Postulaciones';
$string['vacancyactions'] = 'Acciones';
$string['vacancyoverview'] = 'Resumen';
$string['metadata'] = 'Metadatos';

// Tipos de contrato
$string['contract:catedra'] = 'Docente de Cátedra';
$string['contract:planta'] = 'Docente de Planta';
$string['contract:temporal'] = 'Temporal';
$string['contract:ocasional'] = 'Ocasional';
$string['contract:ocasional_tc'] = 'Ocasional Tiempo Completo';
$string['contract:ocasional_mt'] = 'Ocasional Medio Tiempo';
$string['contract:hora_catedra'] = 'Hora Cátedra';

// Modalidades
$string['modality:presencial'] = 'Presencial';
$string['modality:virtual'] = 'Virtual';
$string['modality:hibrida'] = 'Híbrida';
$string['modality:distancia'] = 'A Distancia';

// =============================================================================
// POSTULACIONES
// =============================================================================

$string['application'] = 'Postulación';
$string['newapplication'] = 'Nueva postulación';
$string['editapplication'] = 'Editar postulación';
$string['deleteapplication'] = 'Eliminar postulación';
$string['viewapplication'] = 'Ver postulación';
$string['applicationlist'] = 'Lista de postulaciones';
$string['applicationdetails'] = 'Detalles de la postulación';
$string['applicationcode'] = 'Código de postulación';
$string['applicationdate'] = 'Fecha de postulación';
$string['applicationstatus'] = 'Estado de la postulación';
$string['applicant'] = 'Postulante';
$string['applicantinfo'] = 'Información del postulante';
$string['applynow'] = 'Postularse ahora';
$string['applyforvacancy'] = 'Postularse a la vacante';
$string['submitapplication'] = 'Enviar postulación';
$string['savedraft'] = 'Guardar borrador';
$string['submitbuttonsinfo'] = 'Haga clic en "Enviar postulación" para enviar su postulación a revisión. Use "Guardar borrador" solo si necesita completarla después.';
$string['draftsaved'] = 'Borrador guardado exitosamente. Puede volver más tarde para completar su postulación.';
$string['withdrawapplication'] = 'Retirar postulación';
$string['applicationsubmitted'] = 'Postulación enviada exitosamente';
$string['appstatus:draft'] = 'Borrador';
$string['continueapplication'] = 'Continuar postulación';
$string['completeapplication'] = 'Completar postulación';
$string['completeandsubmit'] = 'Completar y enviar';
$string['draft_pending'] = 'Postulación no enviada';
$string['draft_pending_desc'] = 'Esta postulación está guardada como borrador. Complétela y haga clic en "Enviar" para enviarla a revisión.';
$string['draft_action_desc'] = 'Complete los campos y documentos requeridos, luego envíe su postulación para revisión.';
$string['applicationupdated'] = 'Postulación actualizada exitosamente';
$string['applicationwithdrawn'] = 'Postulación retirada exitosamente';
$string['applicationreactivated'] = 'Postulación reactivada para reenvío';
$string['applicationreactivated_notice'] = 'Su postulación anterior retirada ha sido reactivada. Ahora puede actualizar sus documentos y volver a enviarla.';
$string['error:cannotreactivate'] = 'Solo las postulaciones retiradas pueden ser reactivadas';
$string['applicationdeleted'] = 'Postulación eliminada exitosamente';
$string['noapplications'] = 'No se encontraron postulaciones';
$string['noapplications_desc'] = 'Aún no ha postulado a ninguna vacante. Explore las posiciones disponibles para comenzar.';
$string['yourapplications'] = 'Sus postulaciones';
$string['allapplications'] = 'Todas las postulaciones';
$string['pendingapplications'] = 'Postulaciones pendientes';
$string['reviewedapplications'] = 'Postulaciones revisadas';
$string['approvedapplications'] = 'Postulaciones aprobadas';
$string['rejectedapplications'] = 'Postulaciones rechazadas';
$string['applicationhistory'] = 'Historial de postulación';
$string['applicationtimeline'] = 'Línea de tiempo';
$string['applicationprogress'] = 'Progreso de la postulación';
$string['confirmwithdrawapplication'] = '¿Está seguro que desea retirar esta postulación?';
$string['confirmdeleteapplication'] = '¿Está seguro que desea eliminar esta postulación?';
$string['deleteapplicationreason'] = 'Motivo de eliminación';
$string['deleteapplicationreason_help'] = 'Debe indicar el motivo por el cual se elimina esta postulación. Esta información quedará registrada en el log de auditoría.';
$string['deleteapplicationwarning'] = 'Esta acción eliminará permanentemente la postulación y todos sus documentos asociados. Esta acción no se puede deshacer.';
$string['reasonrequired'] = 'Debe ingresar un motivo';
$string['deleteapplication'] = 'Eliminar postulación';
$string['newstatus'] = 'Nuevo estado';
$string['statusnotes'] = 'Notas del cambio de estado';
$string['alreadyapplied'] = 'Ya se ha postulado a esta vacante';
$string['applicationlimit'] = 'Solo puede postularse a una vacante por convocatoria';
$string['cannotapply'] = 'No puede postularse a esta vacante';
$string['applicationclosed'] = 'Las postulaciones están cerradas para esta vacante';
$string['reviewapplications'] = 'Revisar postulaciones';
$string['manageapplications'] = 'Gestionar postulaciones';
$string['changeapplicationstatus'] = 'Cambiar estado de postulación';
$string['applicationstatuschanged'] = 'Estado de postulación cambiado exitosamente';
$string['letterofintent'] = 'Carta de intención';
$string['letterofintent_help'] = 'Escriba una carta breve explicando por qué está interesado en esta posición y qué lo hace un buen candidato';
$string['coveringletter'] = 'Carta de presentación';

// =============================================================================
// DOCUMENTOS
// =============================================================================

$string['document'] = 'Documento';
$string['documents'] = 'Documentos';
$string['uploaddocument'] = 'Subir documento';
$string['reupload'] = 'Volver a subir';
$string['reuploaddocument'] = 'Volver a subir documento';
$string['viewdocument'] = 'Ver documento';
$string['downloaddocument'] = 'Descargar documento';
$string['deletedocument'] = 'Eliminar documento';
$string['attachments'] = 'Documentos adjuntos';
$string['viewpdf'] = 'Ver PDF';
$string['officialdocument'] = 'Documento oficial';
$string['documentlist'] = 'Lista de documentos';
$string['documentdetails'] = 'Detalles del documento';
$string['documenttype'] = 'Tipo de documento';
$string['documentname'] = 'Nombre del documento';
$string['documentfile'] = 'Archivo';
$string['documentstatus'] = 'Estado del documento';
$string['documentuploaded'] = 'Documento subido exitosamente';
$string['documentdeleted'] = 'Documento eliminado exitosamente';
$string['documentupdated'] = 'Documento actualizado exitosamente';
$string['nodocuments'] = 'No se encontraron documentos';
$string['requireddocuments'] = 'Documentos requeridos';
$string['optionaldocuments'] = 'Documentos opcionales';
$string['uploadeddocuments'] = 'Documentos subidos';
$string['pendingdocuments'] = 'Documentos pendientes';
$string['approveddocuments'] = 'Documentos aprobados';
$string['rejecteddocuments'] = 'Documentos rechazados';
$string['documentprogress'] = 'Progreso de documentos';
$string['documentsvalidated'] = 'Documentos validados';
$string['documentsrejected'] = 'Documentos rechazados';
$string['documentspending'] = 'Documentos pendientes';
$string['alldocumentsapproved'] = 'Todos los documentos aprobados';
$string['somedocumentsrejected'] = 'Algunos documentos fueron rechazados';
$string['documentrequired'] = 'Este documento es requerido';
$string['documentoptional'] = 'Este documento es opcional';
$string['maxfilesize'] = 'Tamaño máximo de archivo';
$string['allowedfiletypes'] = 'Tipos de archivo permitidos';
$string['invalidfiletype'] = 'Tipo de archivo no válido';
$string['filetoobig'] = 'El archivo es demasiado grande';

// Tipos de documento
$string['doctype_cedula'] = 'Cédula de Ciudadanía';
$string['doctype_rut'] = 'RUT';
$string['doctype_titulo_pregrado'] = 'Título de Pregrado';
$string['doctype_titulo_posgrado'] = 'Título de Posgrado';
$string['doctype_acta_grado'] = 'Acta de Grado';
$string['doctype_certificado_laboral'] = 'Certificado Laboral';
$string['doctype_hoja_vida'] = 'Hoja de Vida';
$string['doctype_foto'] = 'Fotografía';
$string['doctype_libreta_militar'] = 'Libreta Militar';
$string['doctype_antecedentes_penales'] = 'Antecedentes Penales';
$string['doctype_antecedentes_disciplinarios'] = 'Antecedentes Disciplinarios';
$string['doctype_antecedentes_fiscales'] = 'Antecedentes Fiscales';
$string['doctype_certificado_medico'] = 'Certificado Médico';
$string['doctype_eps'] = 'Certificado de EPS';
$string['doctype_pension'] = 'Certificado de Pensión';
$string['doctype_arl'] = 'Certificado de ARL';
$string['doctype_cuenta_bancaria'] = 'Certificación Bancaria';
$string['doctype_contrato_firmado'] = 'Contrato Firmado';
$string['doctype_resolucion_convalidacion'] = 'Resolución de Convalidación';
$string['doctype_carta_intencion'] = 'Carta de Intención';

// Categorías de documento
$string['doccat_identification'] = 'Identificación';
$string['doccat_academic'] = 'Académico';
$string['doccat_employment'] = 'Laboral';
$string['doccat_legal'] = 'Legal';
$string['doccat_financial'] = 'Financiero';
$string['doccat_health'] = 'Salud';
$string['doccat_other'] = 'Otro';

// =============================================================================
// VALIDACIÓN DE DOCUMENTOS
// =============================================================================

$string['validatedocument'] = 'Validar documento';
$string['validatedocuments'] = 'Validar documentos';
$string['bulkvalidate'] = 'Validación masiva';
$string['bulkvalidatehelp'] = 'Seleccione documentos para validarlos en masa. Use las casillas de verificación para seleccionar múltiples documentos.';
$string['bulkvalidation'] = 'Validación masiva';
$string['selected'] = 'seleccionados';
$string['documentvalidation'] = 'Validación de documentos';
$string['validationchecklist'] = 'Lista de verificación';
$string['validationresult'] = 'Resultado de validación';
$string['validationdate'] = 'Fecha de validación';
$string['validatedby'] = 'Validado por';
$string['approvedocument'] = 'Aprobar documento';
$string['rejectdocument'] = 'Rechazar documento';
$string['documentapproved'] = 'Documento aprobado exitosamente';
$string['documentrejected'] = 'Documento rechazado';
$string['rejectionreason'] = 'Razón de rechazo';
$string['rejectionreasons'] = 'Razones de rechazo';
$string['selectrejectionreason'] = 'Seleccione razón de rechazo';
$string['additionalcomments'] = 'Comentarios adicionales';
$string['validationcomments'] = 'Comentarios de validación';

// Razones de rechazo
$string['rejection:illegible'] = 'El documento es ilegible';
$string['rejection:expired'] = 'El documento está vencido';
$string['rejection:incomplete'] = 'El documento está incompleto';
$string['rejection:wrongtype'] = 'Tipo de documento incorrecto';
$string['rejection:mismatch'] = 'La información no coincide';
$string['rejection:other'] = 'Otra razón';

// Ítems de lista de verificación
$string['check_legible'] = 'El documento es legible';
$string['check_complete'] = 'El documento está completo';
$string['check_notexpired'] = 'El documento no está vencido';
$string['check_correcttype'] = 'Tipo de documento correcto';
$string['check_matchesinfo'] = 'La información coincide con los datos del postulante';
$string['check_originaldocument'] = 'Parece ser documento original';
$string['check_propersignatures'] = 'Contiene las firmas apropiadas';
$string['check_notampered'] = 'Sin señales de alteración';

// =============================================================================
// REVISORES
// =============================================================================

$string['reviewer'] = 'Revisor';
$string['reviewers'] = 'Revisores';
$string['assignreviewer'] = 'Asignar revisor';
$string['assignreviewers'] = 'Asignar revisores';
$string['unassignreviewer'] = 'Desasignar revisor';
$string['reviewerassigned'] = 'Revisor asignado exitosamente';
$string['reviewerunassigned'] = 'Revisor desasignado exitosamente';
$string['noreviewer'] = 'Sin revisor asignado';
$string['selectreviewer'] = 'Seleccione revisor';
$string['availablereviewers'] = 'Revisores disponibles';
$string['assignedreviewers'] = 'Revisores asignados';
$string['reviewerworkload'] = 'Carga de trabajo del revisor';
$string['reviewerprogress'] = 'Progreso del revisor';
$string['reviewerstatistics'] = 'Estadísticas del revisor';
$string['facultyreviewers'] = 'Revisores por facultad';
$string['assignedapplications'] = 'Postulaciones asignadas';
$string['completedreviews'] = 'Revisiones completadas';
$string['pendingreviews'] = 'Revisiones pendientes';
$string['averagereviewtime'] = 'Tiempo promedio de revisión';
$string['autoassign'] = 'Auto-asignar';
$string['autoassignreviewer'] = 'Auto-asignar revisor';
$string['autoassignbased'] = 'Auto-asignar basado en';
$string['workload'] = 'Carga de trabajo';
$string['expertise'] = 'Experiencia';

// =============================================================================
// COMITÉS
// =============================================================================

$string['committee'] = 'Comité';
$string['committees'] = 'Comités';
$string['selectioncommittee'] = 'Comité de selección';
$string['createcommittee'] = 'Crear comité';
$string['editcommittee'] = 'Editar comité';
$string['deletecommittee'] = 'Eliminar comité';
$string['managecommittee'] = 'Gestionar comité';
$string['committeemembers'] = 'Miembros del comité';
$string['addmember'] = 'Agregar miembro';
$string['removemember'] = 'Remover miembro';
$string['memberadded'] = 'Miembro agregado exitosamente';
$string['memberremoved'] = 'Miembro removido exitosamente';
$string['nocommittees'] = 'No se encontraron comités';
$string['allcommittees'] = 'Todos los comités';
$string['selectcommittee'] = 'Seleccione comité';
$string['facultycommittee'] = 'Comité de facultad';
$string['committeerole'] = 'Rol en el comité';
$string['committeechair'] = 'Presidente del comité';
$string['committeemember'] = 'Miembro del comité';
$string['committeesecretary'] = 'Secretario del comité';

// =============================================================================
// ENTREVISTAS
// =============================================================================

$string['interview'] = 'Entrevista';
$string['interviews'] = 'Entrevistas';
$string['scheduleinterview'] = 'Programar entrevista';
$string['rescheduleinterview'] = 'Reprogramar entrevista';
$string['cancelinterview'] = 'Cancelar entrevista';
$string['interviewscheduled'] = 'Entrevista programada exitosamente';
$string['interviewrescheduled'] = 'Entrevista reprogramada exitosamente';
$string['interviewcancelled'] = 'Entrevista cancelada';
$string['interviewdate'] = 'Fecha de entrevista';
$string['interviewtime'] = 'Hora de entrevista';
$string['interviewlocation'] = 'Lugar de entrevista';
$string['interviewtype'] = 'Tipo de entrevista';
$string['interviewers'] = 'Entrevistadores';
$string['interviewnotes'] = 'Notas de entrevista';
$string['interviewresult'] = 'Resultado de entrevista';
$string['upcominginterviews'] = 'Próximas entrevistas';
$string['pastinterviews'] = 'Entrevistas pasadas';
$string['nointerviews'] = 'Sin entrevistas programadas';
$string['interviewonsite'] = 'Entrevista presencial';
$string['interviewvirtual'] = 'Entrevista virtual';
$string['interviewphone'] = 'Entrevista telefónica';

// =============================================================================
// EVALUACIONES
// =============================================================================

$string['evaluation'] = 'Evaluación';
$string['evaluations'] = 'Evaluaciones';
$string['evaluate'] = 'Evaluar';
$string['evaluatecandidate'] = 'Evaluar candidato';
$string['submiteval'] = 'Enviar evaluación';
$string['evaluationsubmitted'] = 'Evaluación enviada exitosamente';
$string['evaluationcriteria'] = 'Criterios de evaluación';
$string['evaluationscore'] = 'Puntaje';
$string['evaluationcomments'] = 'Comentarios';
$string['overallscore'] = 'Puntaje general';
$string['recommendation'] = 'Recomendación';
$string['stronglyrecommend'] = 'Altamente recomendado';
$string['recommend'] = 'Recomendado';
$string['neutral'] = 'Neutral';
$string['notrecommend'] = 'No recomendado';
$string['stronglynotrecommend'] = 'Definitivamente no recomendado';

// =============================================================================
// EXCEPCIONES
// =============================================================================

$string['exemption'] = 'Excepción';
$string['exemptions'] = 'Excepciones';
$string['createexemption'] = 'Crear excepción';
$string['editexemption'] = 'Editar excepción';
$string['deleteexemption'] = 'Eliminar excepción';
$string['manageexemptions'] = 'Gestionar excepciones';
$string['exemptiontype'] = 'Tipo de excepción';
$string['exemptionapplied'] = 'Excepción aplicada';
$string['exemptionreason'] = 'Razón de excepción';
$string['noexemptions'] = 'No se encontraron excepciones';
$string['exemptionsaved'] = 'Excepción guardada exitosamente';
$string['exemptiondeleted'] = 'Excepción eliminada exitosamente';
$string['importexemptions'] = 'Importar excepciones';
$string['exportexemptions'] = 'Exportar excepciones';

// Tipos de excepción
$string['exemptiontype_historico_iser'] = 'Histórico ISER';
$string['exemptiontype_documentos_recientes'] = 'Documentos recientes';
$string['exemptiontype_traslado_interno'] = 'Traslado interno';
$string['exemptiontype_recontratacion'] = 'Recontratación';
$string['exemptiontype_age'] = 'Excepción por edad';
$string['exemptiontype_gender'] = 'Excepción por género';
$string['exemptiontype_profession'] = 'Excepción por profesión';

$string['exemptiontype_historico_iser_desc'] = 'Empleado con documentos históricos en archivo del ISER';
$string['exemptiontype_documentos_recientes_desc'] = 'Documentos presentados en los últimos 6 meses';
$string['exemptiontype_traslado_interno_desc'] = 'Traslado interno entre centros tutoriales';
$string['exemptiontype_recontratacion_desc'] = 'Ex empleado siendo recontratado';

// =============================================================================
// PLANTILLAS DE CORREO
// =============================================================================

$string['emailtemplate'] = 'Plantilla de correo';
$string['emailtemplates'] = 'Plantillas de correo';
$string['createemailtemplate'] = 'Crear plantilla de correo';
$string['editemailtemplate'] = 'Editar plantilla de correo';
$string['deleteemailtemplate'] = 'Eliminar plantilla de correo';
$string['manageemailtemplates'] = 'Gestionar plantillas de correo';
$string['templatename'] = 'Nombre de plantilla';
$string['templatesubject'] = 'Asunto';
$string['templatebody'] = 'Cuerpo';
$string['templatevariables'] = 'Variables disponibles';
$string['previewtemplate'] = 'Vista previa de plantilla';
$string['testtemplate'] = 'Probar plantilla';
$string['templatesaved'] = 'Plantilla guardada exitosamente';
$string['templatedeleted'] = 'Plantilla eliminada exitosamente';
$string['notemplates'] = 'No se encontraron plantillas de correo';
$string['duplicatetemplate'] = 'Duplicar plantilla';
$string['resettodefault'] = 'Restablecer valores predeterminados';

// Marcadores de plantillas de correo
$string['ph_firstname'] = 'Nombre';
$string['ph_lastname'] = 'Apellido';
$string['ph_fullname'] = 'Nombre completo';
$string['ph_email'] = 'Correo electrónico';
$string['ph_username'] = 'Nombre de usuario';
$string['ph_vacancytitle'] = 'Título de la vacante';
$string['ph_vacancycode'] = 'Código de vacante';
$string['ph_convocatorianame'] = 'Nombre de convocatoria';
$string['ph_applicationcode'] = 'Código de postulación';
$string['ph_applicationstatus'] = 'Estado de postulación';
$string['ph_documentname'] = 'Nombre del documento';
$string['ph_rejectionreason'] = 'Razón de rechazo';
$string['ph_interviewdate'] = 'Fecha de entrevista';
$string['ph_interviewtime'] = 'Hora de entrevista';
$string['ph_interviewlocation'] = 'Lugar de entrevista';
$string['ph_siteurl'] = 'URL del sitio';
$string['ph_sitename'] = 'Nombre del sitio';
$string['ph_supportemail'] = 'Correo de soporte';

// =============================================================================
// REGISTRO Y PERFIL
// =============================================================================

$string['signup'] = 'Registrarse';
$string['register'] = 'Registrar';
$string['createaccount'] = 'Crear cuenta';
$string['alreadyhaveaccount'] = '¿Ya tiene una cuenta?';
$string['donthaveaccount'] = '¿No tiene una cuenta?';
$string['login'] = 'Iniciar sesión';
$string['logout'] = 'Cerrar sesión';
$string['profile'] = 'Perfil';
$string['myprofile'] = 'Mi perfil';
$string['editprofile'] = 'Editar perfil';
$string['updateprofile'] = 'Actualizar perfil';
$string['profileupdated'] = 'Perfil actualizado exitosamente';
$string['personalinformation'] = 'Información personal';
$string['contactinformation'] = 'Información de contacto';
$string['academicinfo'] = 'Información académica';
$string['workexperience'] = 'Experiencia laboral';
$string['additionalinfo'] = 'Información adicional';

// Campos personales
$string['firstname'] = 'Nombre';
$string['lastname'] = 'Apellido';
$string['middlename'] = 'Segundo nombre';
$string['fullname'] = 'Nombre completo';
$string['idnumber'] = 'Número de identificación';
$string['idtype'] = 'Tipo de identificación';
$string['dateofbirth'] = 'Fecha de nacimiento';
$string['gender'] = 'Género';
$string['gender_male'] = 'Masculino';
$string['gender_female'] = 'Femenino';
$string['gender_other'] = 'Otro';
$string['gender_prefernotsay'] = 'Prefiero no decir';
$string['nationality'] = 'Nacionalidad';
$string['maritalstatus'] = 'Estado civil';
$string['marital_single'] = 'Soltero/a';
$string['marital_married'] = 'Casado/a';
$string['marital_divorced'] = 'Divorciado/a';
$string['marital_widowed'] = 'Viudo/a';
$string['marital_other'] = 'Otro';

// Campos de contacto
$string['email'] = 'Correo electrónico';
$string['phone'] = 'Teléfono';
$string['mobilephone'] = 'Teléfono móvil';
$string['address'] = 'Dirección';
$string['city'] = 'Ciudad';
$string['state'] = 'Departamento/Estado';
$string['country'] = 'País';
$string['postalcode'] = 'Código postal';

// Campos académicos
$string['highestdegree'] = 'Máximo nivel de estudios';
$string['degreefield'] = 'Área de estudio';
$string['institution'] = 'Institución';
$string['graduationyear'] = 'Año de graduación';
$string['degree_technical'] = 'Técnico';
$string['degree_technology'] = 'Tecnológico';
$string['degree_undergraduate'] = 'Pregrado';
$string['degree_specialization'] = 'Especialización';
$string['degree_masters'] = 'Maestría';
$string['degree_doctorate'] = 'Doctorado';

// Experiencia laboral
$string['yearsexperience'] = 'Años de experiencia';
$string['currentposition'] = 'Cargo actual';
$string['currentemployer'] = 'Empleador actual';
$string['teachingexperience'] = 'Experiencia docente';

// Registro específico
$string['signupsuccess'] = 'Registro exitoso';
$string['signupsuccess_message'] = 'Su cuenta ha sido creada. Por favor revise su correo electrónico ({$a->email}) para instrucciones adicionales.';
$string['acceptterms'] = 'Acepto los términos y condiciones';
$string['acceptprivacy'] = 'Acepto la política de privacidad';
$string['termsandconditions'] = 'Términos y condiciones';
$string['privacypolicy'] = 'Política de privacidad';
$string['dataprotection'] = 'Protección de datos';
$string['consent'] = 'Consentimiento';
$string['consenttext'] = 'Consiento el tratamiento de mis datos personales de acuerdo con la política de privacidad';

// =============================================================================
// REPORTES
// =============================================================================

$string['report'] = 'Reporte';
$string['generatereport'] = 'Generar reporte';
$string['downloadreport'] = 'Descargar reporte';
$string['exportreport'] = 'Exportar reporte';
$string['reporttype'] = 'Tipo de reporte';
$string['reportformat'] = 'Formato de reporte';
$string['reportperiod'] = 'Período del reporte';
$string['reportfilters'] = 'Filtros del reporte';
$string['noreportdata'] = 'No hay datos disponibles para este reporte';

// Tipos de reporte
$string['report_overview'] = 'Resumen general';
$string['report_applications'] = 'Reporte de postulaciones';
$string['report_documents'] = 'Reporte de documentos';
$string['report_reviewers'] = 'Reporte de revisores';
$string['report_timeline'] = 'Reporte de línea de tiempo';
$string['report_statistics'] = 'Estadísticas';
$string['report_export'] = 'Exportación de datos';

// Estadísticas
$string['statistics'] = 'Estadísticas';
$string['totalvacancies'] = 'Total de vacantes';
$string['totalapplications'] = 'Total de postulaciones';
$string['totaldocuments'] = 'Total de documentos';
$string['totalreviewers'] = 'Total de revisores';
$string['applicationspervacancy'] = 'Postulaciones por vacante';
$string['documentsperapp'] = 'Documentos por postulación';
$string['approvalrate'] = 'Tasa de aprobación';
$string['rejectionrate'] = 'Tasa de rechazo';
$string['averageprocessingtime'] = 'Tiempo promedio de procesamiento';

// =============================================================================
// EXPORTACIÓN DE DATOS
// =============================================================================

$string['dataexport'] = 'Exportación de datos';
$string['exportdata'] = 'Exportar datos';
$string['exportformat'] = 'Formato de exportación';
$string['exportcsv'] = 'Exportar como CSV';
$string['exportexcel'] = 'Exportar como Excel';
$string['exportpdf'] = 'Exportar como PDF';
$string['exportjson'] = 'Exportar como JSON';
$string['exportzip'] = 'Exportar documentos como ZIP';
$string['selectfieldstoexport'] = 'Seleccione campos a exportar';
$string['exportstarted'] = 'Exportación iniciada';
$string['exportcompleted'] = 'Exportación completada';
$string['exportfailed'] = 'Exportación fallida';
$string['downloadexport'] = 'Descargar exportación';

// =============================================================================
// PANEL DE CONTROL
// =============================================================================

$string['welcomeback'] = 'Bienvenido de nuevo, {$a}';
$string['dashboardoverview'] = 'Resumen del panel';
$string['quickactions'] = 'Acciones rápidas';
$string['recentactivity'] = 'Actividad reciente';
$string['alerts'] = 'Alertas';
$string['notifications'] = 'Notificaciones';
$string['pendingactions'] = 'Acciones pendientes';
$string['todaystasks'] = 'Tareas de hoy';
$string['upcomingdeadlines'] = 'Próximos vencimientos';
$string['recentapplications'] = 'Postulaciones recientes';
$string['recentdocuments'] = 'Documentos recientes';
$string['recentreviews'] = 'Revisiones recientes';
$string['performancemetrics'] = 'Métricas de desempeño';
$string['noalerts'] = 'Sin alertas';
$string['nonotifications'] = 'Sin notificaciones';
$string['norecentactivity'] = 'Sin actividad reciente';

// Widgets del panel
$string['widget_vacancies'] = 'Vacantes';
$string['widget_applications'] = 'Postulaciones';
$string['widget_documents'] = 'Documentos';
$string['widget_reviews'] = 'Revisiones';
$string['widget_pending'] = 'Pendientes';
$string['widget_approved'] = 'Aprobados';
$string['widget_rejected'] = 'Rechazados';

// =============================================================================
// NOTIFICACIONES Y ALERTAS
// =============================================================================

$string['notification'] = 'Notificación';
$string['markasread'] = 'Marcar como leído';
$string['markallread'] = 'Marcar todo como leído';
$string['clearnotifications'] = 'Limpiar notificaciones';
$string['newnotification'] = 'Nueva notificación';
$string['unreadnotifications'] = 'Notificaciones sin leer';

// Mensajes de notificación
$string['notify_application_submitted'] = 'Nueva postulación enviada para {$a->vacancy}';
$string['notify_application_status_changed'] = 'El estado de su postulación ha sido actualizado a {$a->status}';
$string['notify_document_approved'] = 'Su documento {$a->document} ha sido aprobado';
$string['notify_document_rejected'] = 'Su documento {$a->document} ha sido rechazado';
$string['notify_review_assigned'] = 'Se le ha asignado revisar la postulación {$a->application}';
$string['notify_interview_scheduled'] = 'Entrevista programada para el {$a->date} a las {$a->time}';
$string['notify_vacancy_closing'] = 'La vacante {$a->vacancy} cierra pronto';
$string['notify_deadline_approaching'] = 'Fecha límite próxima para {$a->item}';

// =============================================================================
// AUDITORÍA Y REGISTRO
// =============================================================================

$string['audit'] = 'Auditoría';
$string['auditlog'] = 'Registro de auditoría';
$string['audittrail'] = 'Rastro de auditoría';
$string['action'] = 'Acción';
$string['actor'] = 'Usuario';
$string['target'] = 'Objetivo';
$string['timestamp'] = 'Fecha y hora';
$string['ipaddress'] = 'Dirección IP';
$string['details'] = 'Detalles';
$string['viewauditlog'] = 'Ver registro de auditoría';

// Acciones de auditoría
$string['audit_create'] = 'Crear';
$string['audit_update'] = 'Actualizar';
$string['audit_delete'] = 'Eliminar';
$string['audit_view'] = 'Ver';
$string['audit_login'] = 'Iniciar sesión';
$string['audit_logout'] = 'Cerrar sesión';
$string['audit_export'] = 'Exportar';
$string['audit_statuschange'] = 'Cambio de estado';

// =============================================================================
// CONFIGURACIÓN
// =============================================================================

$string['generalsettings'] = 'Configuración general';
$string['pluginsettings'] = 'Configuración del plugin';
$string['configureplugin'] = 'Configurar plugin';
$string['defaultsettings'] = 'Configuración predeterminada';
$string['advancedsettings'] = 'Configuración avanzada';

// Etiquetas de configuración
$string['setting_enablepublic'] = 'Habilitar vista pública';
$string['setting_enablepublic_desc'] = 'Permitir que usuarios no autenticados vean vacantes públicas';
$string['setting_requirelogin'] = 'Requerir inicio de sesión para postularse';
$string['setting_requirelogin_desc'] = 'Los usuarios deben iniciar sesión para enviar postulaciones';
$string['setting_maxfilesize'] = 'Tamaño máximo de archivo';
$string['setting_maxfilesize_desc'] = 'Tamaño máximo de archivo para subida de documentos (en MB)';
$string['setting_allowedfiletypes'] = 'Tipos de archivo permitidos';
$string['setting_allowedfiletypes_desc'] = 'Lista separada por comas de extensiones de archivo permitidas';
$string['setting_defaultcontracttype'] = 'Tipo de contrato predeterminado';
$string['setting_defaultmodality'] = 'Modalidad predeterminada';
$string['setting_notifyreviewer'] = 'Notificar revisores';
$string['setting_notifyreviewer_desc'] = 'Enviar notificación por correo cuando se asigna un revisor';
$string['setting_notifyapplicant'] = 'Notificar postulantes';
$string['setting_notifyapplicant_desc'] = 'Enviar notificación por correo en cambios de estado';
$string['setting_autoassignreviewers'] = 'Auto-asignar revisores';
$string['setting_autoassignreviewers_desc'] = 'Asignar automáticamente revisores basado en carga de trabajo';
$string['setting_applicationlimit'] = 'Límite de postulaciones por convocatoria';
$string['setting_applicationlimit_desc'] = 'Número máximo de postulaciones por usuario por convocatoria (0 = ilimitado)';

// =============================================================================
// GESTIÓN DE TIPOS DE DOCUMENTO
// =============================================================================

$string['doctypes'] = 'Tipos de documento';
$string['managedoctypes'] = 'Gestionar tipos de documento';
$string['createdoctype'] = 'Crear tipo de documento';
$string['editdoctype'] = 'Editar tipo de documento';
$string['deletedoctype'] = 'Eliminar tipo de documento';
$string['doctypename'] = 'Nombre del tipo de documento';
$string['doctypecode'] = 'Código';
$string['doctypecategory'] = 'Categoría';
$string['doctyperequired'] = 'Requerido';
$string['doctypeenabled'] = 'Habilitado';
$string['doctypesortorder'] = 'Orden';
$string['doctypechecklist'] = 'Lista de verificación';
$string['doctypeexpirationdays'] = 'Días de vencimiento';
$string['doctypegender'] = 'Específico por género';
$string['doctypeiserexempted'] = 'Exento ISER';
$string['doctypesaved'] = 'Tipo de documento guardado exitosamente';
$string['doctypedeleted'] = 'Tipo de documento eliminado exitosamente';
$string['nodoctypes'] = 'No se encontraron tipos de documento';
$string['confirmdeletedoctype'] = '¿Está seguro que desea eliminar este tipo de documento?';

// =============================================================================
// MENSAJES DE ERROR
// =============================================================================

$string['error'] = 'Error';
$string['error_general'] = 'Ocurrió un error. Por favor intente de nuevo.';
$string['error_notfound'] = 'El elemento solicitado no fue encontrado';
$string['error_permission'] = 'No tiene permiso para realizar esta acción';
$string['error_invalid'] = 'Solicitud no válida';
$string['error_required'] = 'Este campo es requerido';
$string['error_invalidformat'] = 'Formato no válido';
$string['error_invaliddate'] = 'Fecha no válida';
$string['error_invalidemail'] = 'Correo electrónico no válido';
$string['error_invalidphone'] = 'Número de teléfono no válido';
$string['error_invalidfile'] = 'Archivo no válido';
$string['error_fileupload'] = 'Error al subir archivo';
$string['error_filetoobig'] = 'El archivo excede el tamaño máximo permitido';
$string['error_invalidfiletype'] = 'Tipo de archivo no permitido';
$string['error_missingfield'] = 'Campo requerido faltante: {$a}';
$string['error_duplicate'] = 'Ya existe un registro con esta información';
$string['error_cannotdelete'] = 'Este elemento no puede ser eliminado';
$string['error_cannotupdate'] = 'Este elemento no puede ser actualizado';
$string['error_databaseerror'] = 'Ocurrió un error de base de datos';
$string['error_sessionexpired'] = 'Su sesión ha expirado. Por favor inicie sesión nuevamente.';
$string['error_accessdenied'] = 'Acceso denegado';
$string['error_notloggedin'] = 'Debe iniciar sesión para acceder a esta página';
$string['error_convocatoriaclosed'] = 'Esta convocatoria está cerrada';
$string['error_vacancyclosed'] = 'Esta vacante está cerrada';
$string['error_applicationlimit'] = 'Ha alcanzado el límite de postulaciones para esta convocatoria';
$string['error_alreadyapplied'] = 'Ya se ha postulado a esta vacante';
$string['error_documentrequired'] = 'Falta documento requerido: {$a}';
$string['error_invaliddocument'] = 'Documento no válido enviado';
$string['error_reviewernotfound'] = 'Revisor no encontrado';
$string['error_noreviewers'] = 'No hay revisores disponibles';
$string['error_exportfailed'] = 'Exportación fallida. Por favor intente de nuevo.';
$string['error_importfailed'] = 'Importación fallida. Por favor verifique el formato del archivo.';

// =============================================================================
// MENSAJES DE ÉXITO
// =============================================================================

$string['success'] = 'Éxito';
$string['success_saved'] = 'Cambios guardados exitosamente';
$string['success_created'] = 'Elemento creado exitosamente';
$string['success_updated'] = 'Elemento actualizado exitosamente';
$string['success_deleted'] = 'Elemento eliminado exitosamente';
$string['success_uploaded'] = 'Archivo subido exitosamente';
$string['success_submitted'] = 'Formulario enviado exitosamente';
$string['success_sent'] = 'Mensaje enviado exitosamente';
$string['success_exported'] = 'Datos exportados exitosamente';
$string['success_imported'] = 'Datos importados exitosamente';

// =============================================================================
// MENSAJES DE CONFIRMACIÓN
// =============================================================================

$string['confirm'] = 'Confirmar';
$string['confirm_delete'] = '¿Está seguro que desea eliminar este elemento?';
$string['confirm_cancel'] = '¿Está seguro que desea cancelar?';
$string['confirm_submit'] = '¿Está seguro que desea enviar?';
$string['confirm_withdraw'] = '¿Está seguro que desea retirar?';
$string['confirm_approve'] = '¿Está seguro que desea aprobar?';
$string['confirm_reject'] = '¿Está seguro que desea rechazar?';
$string['confirm_publish'] = '¿Está seguro que desea publicar?';
$string['confirm_close'] = '¿Está seguro que desea cerrar?';
$string['confirm_archive'] = '¿Está seguro que desea archivar?';
$string['confirm_action'] = '¿Está seguro que desea realizar esta acción?';
$string['actioncannnotbeundone'] = 'Esta acción no se puede deshacer.';

// Cadenas de página de confirmación de correo.
$string['confirm_success_title'] = 'Correo Electrónico Confirmado';
$string['confirm_success_message'] = 'Su dirección de correo electrónico ha sido verificada exitosamente. Ahora puede iniciar sesión en su cuenta.';
$string['confirm_failed_title'] = 'Confirmación Fallida';
$string['confirm_failed_message'] = 'No pudimos verificar su dirección de correo electrónico. El enlace de confirmación puede haber expirado o es inválido. Por favor intente registrarse nuevamente o contacte a soporte.';
$string['confirm_pending_vacancy'] = 'Tiene una postulación pendiente. Después de iniciar sesión, será redirigido para completar su postulación.';
$string['confirm_next_steps'] = 'Próximos Pasos';
$string['confirm_step_login'] = 'Inicie sesión con su usuario y contraseña';
$string['confirm_step_application'] = 'Complete su postulación pendiente';
$string['confirm_step_browse'] = 'Explore las vacantes disponibles';
$string['confirm_step_apply'] = 'Envíe sus postulaciones';
$string['confirm_username_reminder'] = 'Su nombre de usuario es:';
$string['confirm_possible_reasons'] = 'Posibles Razones';
$string['confirm_reason_expired'] = 'El enlace de confirmación ha expirado';
$string['confirm_reason_invalid'] = 'El enlace estaba incompleto o fue modificado';
$string['confirm_reason_already'] = 'Su cuenta ya fue confirmada anteriormente';
$string['confirm_reason_notexist'] = 'La cuenta no existe';
$string['confirm_what_to_do'] = 'Qué Puede Hacer';
$string['confirm_todo_trylogin'] = 'Intente iniciar sesión - su cuenta puede que ya esté activa';
$string['confirm_todo_register'] = 'Regístrese nuevamente si no completó el proceso';
$string['confirm_todo_contact'] = 'Contacte a soporte si el problema persiste';
$string['confirm_need_help'] = '¿Necesita ayuda?';

// =============================================================================
// CADENAS DE AYUDA
// =============================================================================

$string['help'] = 'Ayuda';
$string['help_convocatoria'] = 'Una convocatoria es un llamado a postulaciones que agrupa múltiples vacantes';
$string['help_vacancy'] = 'Una vacante representa una posición de trabajo a la cual los postulantes pueden aplicar';
$string['help_application'] = 'Una postulación es un envío de un candidato para una vacante específica';
$string['help_documents'] = 'Los documentos son los archivos que los postulantes deben enviar como parte de su postulación';
$string['help_reviewer'] = 'Los revisores son responsables de validar los documentos de los postulantes';
$string['help_committee'] = 'El comité de selección evalúa candidatos y toma decisiones de contratación';
$string['help_exemption'] = 'Las excepciones permiten a ciertos postulantes omitir requisitos específicos de documentos';

// =============================================================================
// ESTADOS VACÍOS
// =============================================================================

$string['noresults'] = 'No se encontraron resultados';
$string['nodata'] = 'No hay datos disponibles';
$string['noitems'] = 'No hay elementos para mostrar';
$string['emptylist'] = 'La lista está vacía';
$string['getstarted'] = 'Comenzar';
$string['createfirst'] = 'Cree su primer {$a}';

// =============================================================================
// VALIDACIÓN DE FORMULARIOS
// =============================================================================

$string['required'] = 'Requerido';
$string['optional'] = 'Opcional';
$string['fieldrequired'] = 'Este campo es requerido';
$string['invalidvalue'] = 'Valor no válido';
$string['minlength'] = 'La longitud mínima es {$a} caracteres';
$string['maxlength'] = 'La longitud máxima es {$a} caracteres';
$string['minvalue'] = 'El valor mínimo es {$a}';
$string['maxvalue'] = 'El valor máximo es {$a}';
$string['dateformat'] = 'Formato de fecha: AAAA-MM-DD';
$string['selectoption'] = 'Por favor seleccione una opción';
$string['entervalue'] = 'Por favor ingrese un valor';
$string['uploadorselect'] = 'Suba o seleccione un archivo';

// =============================================================================
// PRIVACY API
// =============================================================================

$string['privacy:metadata:local_jobboard_application'] = 'Información sobre las postulaciones de usuarios';
$string['privacy:metadata:local_jobboard_application:userid'] = 'El ID del usuario que envió la postulación';
$string['privacy:metadata:local_jobboard_application:vacancyid'] = 'El ID de la vacante a la que se postuló';
$string['privacy:metadata:local_jobboard_application:status'] = 'El estado de la postulación';
$string['privacy:metadata:local_jobboard_application:timecreated'] = 'La hora en que se creó la postulación';

$string['privacy:metadata:local_jobboard_document'] = 'Información sobre los documentos de usuarios';
$string['privacy:metadata:local_jobboard_document:userid'] = 'El ID del usuario que subió el documento';
$string['privacy:metadata:local_jobboard_document:filename'] = 'El nombre del archivo subido';
$string['privacy:metadata:local_jobboard_document:timecreated'] = 'La hora en que se subió el documento';

$string['privacy:metadata:local_jobboard_applicant_profile'] = 'Información sobre los perfiles de postulantes';
$string['privacy:metadata:local_jobboard_applicant_profile:userid'] = 'El ID del usuario';
$string['privacy:metadata:local_jobboard_applicant_profile:personaldata'] = 'Información personal proporcionada por el postulante';

$string['privacy:metadata:local_jobboard_audit'] = 'Registro de auditoría de acciones de usuarios';
$string['privacy:metadata:local_jobboard_audit:userid'] = 'El ID del usuario que realizó la acción';
$string['privacy:metadata:local_jobboard_audit:action'] = 'La acción realizada';
$string['privacy:metadata:local_jobboard_audit:timecreated'] = 'La hora en que se realizó la acción';

// Privacy metadata para todas las tablas del plugin (cumplimiento AGENTS.md)
$string['privacy:metadata:application'] = 'Registros de postulaciones enviadas por usuarios';
$string['privacy:metadata:application:userid'] = 'ID del usuario postulante';
$string['privacy:metadata:application:vacancyid'] = 'ID de la vacante a la que postuló';
$string['privacy:metadata:application:status'] = 'Estado de la postulación';
$string['privacy:metadata:application:digitalsignature'] = 'Firma digital proporcionada por el postulante';
$string['privacy:metadata:application:coverletter'] = 'Texto de carta de presentación';
$string['privacy:metadata:application:applicationdata'] = 'Datos adicionales del formulario de postulación (JSON)';
$string['privacy:metadata:application:consentgiven'] = 'Si se otorgó consentimiento';
$string['privacy:metadata:application:consenttimestamp'] = 'Hora en que se otorgó el consentimiento';
$string['privacy:metadata:application:consentip'] = 'Dirección IP al momento del consentimiento';
$string['privacy:metadata:application:consentuseragent'] = 'Agente de navegador al momento del consentimiento';
$string['privacy:metadata:application:timecreated'] = 'Hora en que se envió la postulación';

$string['privacy:metadata:document'] = 'Documentos subidos por postulantes';
$string['privacy:metadata:document:applicationid'] = 'ID de la postulación a la que pertenece el documento';
$string['privacy:metadata:document:documenttype'] = 'Tipo de documento';
$string['privacy:metadata:document:filename'] = 'Nombre original del archivo';
$string['privacy:metadata:document:uploadedby'] = 'Usuario que subió el documento';
$string['privacy:metadata:document:issuedate'] = 'Fecha de expedición del documento';
$string['privacy:metadata:document:timecreated'] = 'Marca de tiempo de carga';

$string['privacy:metadata:exemption'] = 'Registros de exención del ISER para personal histórico';
$string['privacy:metadata:exemption:userid'] = 'ID del usuario con exención';
$string['privacy:metadata:exemption:exemptiontype'] = 'Tipo de exención';
$string['privacy:metadata:exemption:exempteddocs'] = 'Documentos exentos';
$string['privacy:metadata:exemption:validfrom'] = 'Fecha de inicio de la exención';
$string['privacy:metadata:exemption:validuntil'] = 'Fecha de fin de la exención';
$string['privacy:metadata:exemption:notes'] = 'Notas administrativas sobre la exención';
$string['privacy:metadata:exemption:timecreated'] = 'Hora en que se creó la exención';

$string['privacy:metadata:workflowlog'] = 'Historial de cambios de estado de postulación';
$string['privacy:metadata:workflowlog:applicationid'] = 'ID de la postulación';
$string['privacy:metadata:workflowlog:previousstatus'] = 'Estado anterior de la postulación';
$string['privacy:metadata:workflowlog:newstatus'] = 'Nuevo estado de la postulación';
$string['privacy:metadata:workflowlog:changedby'] = 'Usuario que cambió el estado';
$string['privacy:metadata:workflowlog:comments'] = 'Comentarios sobre el cambio de estado';
$string['privacy:metadata:workflowlog:timecreated'] = 'Hora del cambio de estado';

$string['privacy:metadata:audit'] = 'Entradas del registro de auditoría';
$string['privacy:metadata:audit:userid'] = 'Usuario que realizó la acción';
$string['privacy:metadata:audit:action'] = 'Acción realizada';
$string['privacy:metadata:audit:entitytype'] = 'Tipo de entidad afectada';
$string['privacy:metadata:audit:entityid'] = 'ID de la entidad afectada';
$string['privacy:metadata:audit:ipaddress'] = 'Dirección IP del usuario';
$string['privacy:metadata:audit:useragent'] = 'Agente de navegador del usuario';
$string['privacy:metadata:audit:extradata'] = 'Datos adicionales de la acción';
$string['privacy:metadata:audit:timecreated'] = 'Hora de la acción';

$string['privacy:metadata:notification'] = 'Registros de notificaciones por correo';
$string['privacy:metadata:notification:userid'] = 'ID del usuario destinatario';
$string['privacy:metadata:notification:templatecode'] = 'Plantilla de correo utilizada';
$string['privacy:metadata:notification:data'] = 'Datos de marcadores de posición de la notificación';
$string['privacy:metadata:notification:status'] = 'Estado de la notificación';
$string['privacy:metadata:notification:timecreated'] = 'Hora en que se encoló la notificación';

$string['privacy:metadata:interviewer'] = 'Asignaciones de miembros del panel de entrevistas';
$string['privacy:metadata:interviewer:userid'] = 'Usuario asignado como entrevistador';
$string['privacy:metadata:interviewer:interviewid'] = 'ID de la entrevista';
$string['privacy:metadata:interviewer:timecreated'] = 'Hora de la asignación';

$string['privacy:metadata:committeemember'] = 'Membresías del comité de selección';
$string['privacy:metadata:committeemember:userid'] = 'ID del usuario miembro del comité';
$string['privacy:metadata:committeemember:committeeid'] = 'ID del comité';
$string['privacy:metadata:committeemember:role'] = 'Rol en el comité';
$string['privacy:metadata:committeemember:addedby'] = 'Usuario que agregó al miembro';
$string['privacy:metadata:committeemember:timecreated'] = 'Hora en que se agregó el miembro';

$string['privacy:metadata:evaluation'] = 'Evaluaciones de postulaciones por miembros del comité';
$string['privacy:metadata:evaluation:userid'] = 'Usuario que envió la evaluación';
$string['privacy:metadata:evaluation:applicationid'] = 'Postulación siendo evaluada';
$string['privacy:metadata:evaluation:score'] = 'Puntaje de la evaluación';
$string['privacy:metadata:evaluation:vote'] = 'Voto del evaluador';
$string['privacy:metadata:evaluation:comments'] = 'Comentarios de la evaluación';
$string['privacy:metadata:evaluation:timecreated'] = 'Hora en que se envió la evaluación';

$string['privacy:metadata:consent'] = 'Registros de consentimiento del usuario para procesamiento de datos';
$string['privacy:metadata:consent:userid'] = 'Usuario que dio el consentimiento';
$string['privacy:metadata:consent:consenttype'] = 'Tipo de consentimiento (tratamiento de datos, términos, privacidad)';
$string['privacy:metadata:consent:consentgiven'] = 'Si se dio el consentimiento';
$string['privacy:metadata:consent:consentversion'] = 'Versión de la política consentida';
$string['privacy:metadata:consent:ipaddress'] = 'Dirección IP al momento del consentimiento';
$string['privacy:metadata:consent:useragent'] = 'Agente de navegador al momento del consentimiento';
$string['privacy:metadata:consent:timecreated'] = 'Hora en que se registró el consentimiento';

$string['privacy:metadata:applicantprofile'] = 'Datos extendidos del perfil de postulantes de empleo';
$string['privacy:metadata:applicantprofile:userid'] = 'ID del usuario postulante';
$string['privacy:metadata:applicantprofile:doctype'] = 'Tipo de documento (CC, CE, etc.)';
$string['privacy:metadata:applicantprofile:birthdate'] = 'Fecha de nacimiento';
$string['privacy:metadata:applicantprofile:gender'] = 'Género';
$string['privacy:metadata:applicantprofile:education_level'] = 'Nivel educativo más alto';
$string['privacy:metadata:applicantprofile:degree_title'] = 'Título o grado obtenido';
$string['privacy:metadata:applicantprofile:expertise_area'] = 'Área de experticia';
$string['privacy:metadata:applicantprofile:experience_years'] = 'Años de experiencia';
$string['privacy:metadata:applicantprofile:timecreated'] = 'Hora de creación del perfil';

$string['privacy:metadata:files'] = 'Archivos subidos por usuarios (documentos de postulación)';

// Cadenas adicionales para exportación de privacidad
$string['consent'] = 'Consentimiento';
$string['applicantprofile'] = 'Perfil del Postulante';

// =============================================================================
// FUNCIONALIDADES CONSOLIDADAS DEL DASHBOARD (AGENTS.md 22.2)
// =============================================================================

// Cadenas de tiempo relativo
$string['timeago_justnow'] = 'Ahora mismo';
$string['timeago_minutes'] = 'Hace {$a} minutos';
$string['timeago_hours'] = 'Hace {$a} horas';
$string['timeago_days'] = 'Hace {$a} días';

// Banner de próxima convocatoria
$string['critical'] = 'CRÍTICO';
$string['closingsoon'] = 'Cierra Pronto';
$string['closingon'] = 'Cierra el';
$string['daysremaining'] = 'días restantes';
$string['viewdetails'] = 'Ver Detalles';

// Notificaciones pendientes
$string['pendingnotifications'] = 'Notificaciones Pendientes';
$string['notification_application_received'] = 'Postulación recibida';
$string['notification_application_received_subject'] = 'Confirmación de Postulación - {$a->vacancy}';
$string['notification_application_received_body'] = 'Estimado/a {$a->fullname},

Hemos recibido su postulación para la vacante "{$a->vacancy}" (Código: {$a->code}).

Su postulación ha sido registrada exitosamente y se encuentra en proceso de revisión. Recibirá notificaciones sobre el estado de su postulación a través de este correo electrónico.

Fecha de postulación: {$a->date}

Puede consultar el estado de su postulación en cualquier momento accediendo a la plataforma.

Atentamente,
{$a->sitename}';
$string['notification_docs_validated'] = 'Documentos validados';
$string['notification_docs_rejected'] = 'Documentos requieren atención';
$string['notification_interview_scheduled'] = 'Entrevista programada';
$string['notification_status_changed'] = 'Estado de postulación cambiado';
$string['notification_application_selected'] = '¡Felicitaciones! Ha sido seleccionado';
$string['notification_application_rejected'] = 'Postulación no seleccionada';

// Actividad reciente
$string['recentactivity'] = 'Actividad Reciente';
$string['activity_application_submitted'] = 'Nueva postulación enviada';
$string['activity_document_uploaded'] = 'Documento subido';
$string['activity_vacancy_published'] = 'Vacante publicada';
$string['activity_convocatoria_opened'] = 'Convocatoria abierta';
$string['activity_status_viewed'] = 'Estado de postulación consultado';

// =============================================================================
// FILTRO DE CONVOCATORIA EN REPORTES (AGENTS.md 16.1)
// =============================================================================

$string['selectconvocatoria_required'] = 'Selección de Convocatoria Requerida';
$string['selectconvocatoria_required_desc'] = 'Por favor seleccione una convocatoria para ver los reportes. Todos los reportes deben estar filtrados por convocatoria según la política institucional.';
$string['currentconvocatoria'] = 'Convocatoria Actual';
$string['changeconvocatoria'] = 'Cambiar Convocatoria';
$string['noconvocatorias_forreports'] = 'No hay convocatorias disponibles. Debe crear al menos una convocatoria antes de ver reportes.';
$string['continue'] = 'Continuar';

// =============================================================================
// CLI E IMPORTACIÓN
// =============================================================================

$string['cli_usage'] = 'Uso: php cli.php [opciones]';
$string['cli_help'] = 'Mostrar este mensaje de ayuda';
$string['cli_version'] = 'Mostrar información de versión';
$string['cli_import'] = 'Importar datos desde archivo';
$string['cli_export'] = 'Exportar datos a archivo';
$string['cli_processing'] = 'Procesando...';
$string['cli_complete'] = 'Completado';
$string['cli_error'] = 'Error: {$a}';
$string['cli_success'] = 'Éxito: {$a}';
$string['cli_recordsprocessed'] = '{$a} registros procesados';
$string['cli_recordsimported'] = '{$a} registros importados';
$string['cli_recordsexported'] = '{$a} registros exportados';
$string['cli_recordsfailed'] = '{$a} registros fallidos';

$string['import'] = 'Importar';
$string['importfile'] = 'Archivo de importación';
$string['importformat'] = 'Formato de importación';
$string['importoptions'] = 'Opciones de importación';
$string['importpreview'] = 'Vista previa de importación';
$string['importconfirm'] = 'Confirmar importación';
$string['importstarted'] = 'Importación iniciada';
$string['importcompleted'] = 'Importación completada';
$string['importfailed'] = 'Importación fallida';
$string['importresults'] = 'Resultados de importación';
$string['rowsimported'] = '{$a} filas importadas';
$string['rowsskipped'] = '{$a} filas omitidas';
$string['rowsfailed'] = '{$a} filas fallidas';

// =============================================================================
// EVENTOS
// =============================================================================

$string['event_application_created'] = 'Postulación creada';
$string['event_application_status_changed'] = 'Estado de postulación cambiado';
$string['event_document_uploaded'] = 'Documento subido';
$string['event_vacancy_created'] = 'Vacante creada';
$string['event_vacancy_updated'] = 'Vacante actualizada';
$string['event_vacancy_published'] = 'Vacante publicada';
$string['event_vacancy_closed'] = 'Vacante cerrada';
$string['event_vacancy_deleted'] = 'Vacante eliminada';

// =============================================================================
// MISCELÁNEOS
// =============================================================================

$string['loading'] = 'Cargando...';
$string['pleasewait'] = 'Por favor espere...';
$string['processing'] = 'Procesando...';
$string['saving'] = 'Guardando...';
$string['uploading'] = 'Subiendo...';
$string['searching'] = 'Buscando...';
$string['nochanges'] = 'No se detectaron cambios';
$string['unsavedchanges'] = 'Tiene cambios sin guardar';
$string['confirmleavepage'] = '¿Está seguro que desea abandonar esta página? Los cambios sin guardar se perderán.';
$string['sessiontimeout'] = 'Su sesión ha expirado';
$string['connectionerror'] = 'Error de conexión. Por favor verifique su conexión a internet.';
$string['tryagain'] = 'Intentar de nuevo';
$string['contactsupport'] = 'Contactar soporte';
$string['reporterror'] = 'Reportar un error';
$string['needhelp'] = '¿Necesita ayuda?';
$string['supporthelptext'] = 'Si encuentra algún problema o necesita asistencia, por favor contacte a nuestro equipo de soporte.';
$string['supportemail'] = 'soporteplataformas@iser.edu.co';
$string['supportemailsubject'] = 'Solicitud de Soporte - Bolsa de Empleo';
$string['supportemailbody'] = 'Estimado equipo de soporte,%0D%0A%0D%0ANecesito asistencia con el sistema de Bolsa de Empleo.%0D%0A%0D%0ADescripción del problema:%0D%0A[Por favor describa su problema aquí]%0D%0A%0D%0AURL de la página: {$a->url}%0D%0AUsuario: {$a->username}%0D%0AFecha: {$a->date}%0D%0A%0D%0AGracias.';

// Strings del formulario de soporte.
$string['support_page_title'] = 'Reportar Error Técnico';
$string['support_warning_title'] = 'Solo Soporte Técnico';
$string['support_warning_message'] = 'Este formulario es ÚNICAMENTE para reportar errores técnicos con el sistema de Bolsa de Empleo (bugs, errores, problemas con carga de documentos, etc.). NO es para preguntas sobre procesos de selección, estado de postulaciones ni consultas generales. Para esas consultas, por favor contacte directamente al departamento de Recursos Humanos.';
$string['support_info_title'] = 'Qué Reportar';
$string['support_info_description'] = 'Use este formulario para reportar problemas técnicos que haya encontrado al usar el sistema de Bolsa de Empleo. Por favor proporcione la mayor cantidad de detalles posible para ayudarnos a identificar y corregir el problema.';
$string['support_form_title'] = 'Formulario de Reporte';
$string['support_examples_title'] = 'Ejemplos de problemas a reportar:';
$string['support_example_1'] = 'Problemas al cargar documentos (mensajes de error, archivos que no se guardan)';
$string['support_example_2'] = 'Errores de página o comportamiento inesperado';
$string['support_example_3'] = 'Formularios que no se envían correctamente';
$string['support_example_4'] = 'Problemas de visualización o diseño roto';
$string['support_error_details'] = 'Detalles del Error';
$string['support_error_type'] = 'Tipo de Error';
$string['support_select_error_type'] = 'Seleccione el tipo de error...';
$string['support_type_document_upload'] = 'Problema al cargar documentos';
$string['support_type_form_submission'] = 'Error al enviar formulario';
$string['support_type_page_error'] = 'Error de página o caída';
$string['support_type_login_issue'] = 'Problema de inicio de sesión o acceso';
$string['support_type_display_issue'] = 'Problema de visualización o diseño';
$string['support_type_notification_issue'] = 'Problema con notificaciones o correos';
$string['support_type_other'] = 'Otro problema técnico';
$string['support_error_description'] = 'Descripción del Error';
$string['support_error_description_help'] = 'Describa el error que encontró en detalle. Incluya lo que estaba tratando de hacer y lo que ocurrió en su lugar.';
$string['support_description_placeholder'] = 'Describa el error en detalle. ¿Qué estaba tratando de hacer? ¿Qué mensaje de error vio?';
$string['support_description_too_short'] = 'Por favor proporcione una descripción más detallada (al menos 20 caracteres)';
$string['support_steps_too_short'] = 'Por favor proporcione pasos más detallados para reproducir el problema (al menos 20 caracteres)';
$string['support_instructions_title'] = 'Instrucciones importantes';
$string['support_instructions_detail'] = 'Para que podamos ayudarle de manera efectiva, es fundamental que sea lo más detallado posible en su reporte. Una descripción clara y completa nos permite resolver su problema más rápidamente.';
$string['support_instructions_tip1'] = '<strong>Sea específico:</strong> Describa exactamente qué estaba haciendo cuando ocurrió el problema.';
$string['support_instructions_tip2'] = '<strong>Incluya capturas de pantalla:</strong> Las imágenes nos ayudan a entender mejor el problema. Use el botón de imagen en el editor para adjuntarlas.';
$string['support_instructions_tip3'] = '<strong>Detalle los pasos:</strong> Enumere paso a paso cómo reproducir el error para que podamos verificarlo.';
$string['support_steps_to_reproduce'] = 'Pasos para Reproducir (Obligatorio)';
$string['support_steps_to_reproduce_help'] = 'Describa paso a paso las acciones que realizó antes de encontrar el error. Esto es crucial para que podamos reproducir y solucionar el problema.';
$string['support_steps_placeholder'] = '1. Ir a la página...\n2. Hacer clic en...\n3. Llenar...\n4. Aparece el error...';
$string['support_expected_behavior'] = 'Comportamiento Esperado';
$string['support_expected_placeholder'] = '¿Qué esperaba que ocurriera?';
$string['support_page_url'] = 'URL de la Página';
$string['support_contact_info'] = 'Su Información de Contacto';
$string['support_reporter_name'] = 'Su Nombre';
$string['support_reporter_email'] = 'Su Correo Electrónico';
$string['support_browser'] = 'Navegador';
$string['support_timestamp'] = 'Fecha/Hora';
$string['support_user_info'] = 'Información del Usuario';
$string['support_submit'] = 'Enviar Reporte';
$string['support_success_message'] = 'Su reporte de error ha sido enviado exitosamente. Nuestro equipo técnico lo revisará y puede contactarlo si necesita información adicional.';
$string['support_email_failed'] = 'Hubo un problema al enviar su reporte. Por favor intente nuevamente más tarde o contacte a soporte directamente al correo mostrado.';
$string['support_email_subject'] = 'Bolsa de Empleo - Solicitud de Soporte Técnico';
$string['support_email_header'] = 'NUEVA SOLICITUD DE SOPORTE TÉCNICO - BOLSA DE EMPLEO';
$string['technicaldetails'] = 'Detalles técnicos';
$string['debuginfo'] = 'Información de depuración';
$string['version'] = 'Versión';
$string['copyright'] = 'Derechos de autor';
$string['allrightsreserved'] = 'Todos los derechos reservados';
$string['poweredby'] = 'Desarrollado por';
$string['lastupdate'] = 'Última actualización';
$string['timezone'] = 'Zona horaria';
$string['language'] = 'Idioma';
$string['selectlanguage'] = 'Seleccionar idioma';

// Configuración de soporte.
$string['supportsettings'] = 'Configuración de Soporte';
$string['supportsettings_desc'] = 'Configure el sistema de soporte para reportes de problemas técnicos.';
$string['support_emails'] = 'Correos de Soporte';
$string['support_emails_desc'] = 'Ingrese las direcciones de correo (una por línea o separadas por coma) que recibirán las solicitudes de soporte técnico de los usuarios. Estas direcciones reciben notificaciones cuando los usuarios reportan errores o problemas a través del formulario de soporte.';

// Accesibilidad
$string['skiptomaincontent'] = 'Saltar al contenido principal';
$string['opensinnewwindow'] = 'Abre en nueva ventana';
$string['expandcollapse'] = 'Expandir/Contraer';
$string['sortascending'] = 'Ordenar ascendente';
$string['sortdescending'] = 'Ordenar descendente';
$string['sortby'] = 'Ordenar por';
$string['filterby'] = 'Filtrar por';
$string['showing'] = 'Mostrando';
$string['entries'] = 'entradas';
$string['firstpage'] = 'Primera página';
$string['lastpage'] = 'Última página';
$string['previouspage'] = 'Página anterior';
$string['nextpage'] = 'Página siguiente';

// =============================================================================
// CADENAS ADICIONALES - GESTIÓN DE CONVOCATORIAS
// =============================================================================

$string['addconvocatoria'] = 'Agregar convocatoria';
$string['editconvocatoria'] = 'Editar convocatoria';
$string['deleteconvocatoria'] = 'Eliminar convocatoria';
$string['convocatoriaactive'] = 'Activa';
$string['convocatoriaarchived'] = 'Archivada';
$string['convocatoriaclosedmsg'] = 'Esta convocatoria está cerrada';
$string['convocatoriadates'] = 'Fechas de la convocatoria';
$string['convocatoriadocexemptions'] = 'Exenciones de documentos para esta convocatoria';
$string['convocatoriahelp'] = 'Ayuda sobre convocatorias';
$string['convocatoriaopened'] = 'Convocatoria abierta exitosamente';
$string['convocatoriareopened'] = 'Convocatoria reabierta exitosamente';
$string['convocatorias_dashboard_desc'] = 'Gestione todas las convocatorias desde este panel';
$string['openconvocatoria'] = 'Abrir convocatoria';
$string['manageconvocatorias'] = 'Gestionar convocatorias';
$string['manageconvocatorias_desc'] = 'Crear, editar y gestionar todas las convocatorias';
$string['browseconvocatorias'] = 'Explorar convocatorias';
$string['browseconvocatorias_desc'] = 'Ver todas las convocatorias disponibles';
$string['backtoconvocatoria'] = 'Volver a la convocatoria';
$string['backtoconvocatorias'] = 'Volver a convocatorias';
$string['totalconvocatorias'] = 'Total de convocatorias';
$string['confirmarchiveconvocatoria'] = '¿Está seguro de que desea archivar esta convocatoria?';
$string['confirmcloseconvocatoria'] = '¿Está seguro de que desea cerrar esta convocatoria?';
$string['confirmopenconvocatoria'] = '¿Está seguro de que desea abrir esta convocatoria?';
$string['confirmreopenconvocatoria'] = '¿Está seguro de que desea reabrir esta convocatoria?';
$string['confirmdeletevconvocatoria'] = '¿Está seguro de que desea eliminar esta convocatoria? Esta acción no se puede deshacer.';
$string['gotocreateconvocatoria'] = 'Crear nueva convocatoria';
$string['createvacancyinconvocatoriadesc'] = 'Crear una nueva vacante dentro de esta convocatoria';

// =============================================================================
// CADENAS ADICIONALES - GESTIÓN DE VACANTES
// =============================================================================

$string['addvacancy'] = 'Agregar vacante';
$string['managevacancies'] = 'Gestionar vacantes';
$string['vacancies_dashboard_desc'] = 'Ver y gestionar todas las vacantes';
$string['backtovacancies'] = 'Volver a vacantes';
$string['availablevacancies'] = 'Vacantes disponibles';
$string['openvacancies'] = 'Vacantes abiertas';
$string['publishedvacancies'] = 'Vacantes publicadas';
$string['vacanciesfound'] = 'Vacantes encontradas';
$string['searchvacancies'] = 'Buscar vacantes';
$string['explorevacancias'] = 'Explorar vacantes';
$string['explore'] = 'Explorar';
$string['browse_vacancies_desc'] = 'Explorar todas las vacantes disponibles';
$string['applytovacancy'] = 'Postularse a vacante';
$string['vacancyinfo'] = 'Información de la vacante';
$string['vacancyopen'] = 'Vacante abierta';
$string['vacancyreopened'] = 'Vacante reabierta';
$string['vacancyunpublished'] = 'Vacante despublicada';
$string['vacancy_inherits_dates'] = 'Esta vacante hereda las fechas de la convocatoria';
$string['vacancy_status_draft'] = 'Borrador';
$string['vacancy_status_published'] = 'Publicada';
$string['unknownvacancy'] = 'Vacante desconocida';
$string['totalpositions'] = 'Total de posiciones';
$string['closingdate'] = 'Fecha de cierre';
$string['closesindays'] = 'Cierra en {$a} días';
$string['closingsoondays'] = 'Cierra pronto (dentro de {$a} días)';
$string['publish'] = 'Publicar';
$string['unpublish'] = 'Despublicar';
$string['reopen'] = 'Reabrir';
$string['confirmpublish'] = '¿Está seguro de que desea publicar esta vacante?';
$string['confirmunpublish'] = '¿Está seguro de que desea despublicar esta vacante?';
$string['confirmreopen'] = '¿Está seguro de que desea reabrir esta vacante?';
$string['confirmclose'] = '¿Está seguro de que desea cerrar esta vacante?';
$string['sharethisvacancy'] = 'Compartir esta vacante';

// =============================================================================
// CADENAS ADICIONALES - GESTIÓN DE POSTULACIONES
// =============================================================================

$string['applicationerror'] = 'Error al procesar la postulación';
$string['applicationlimits'] = 'Límites de postulación';
$string['applicationlimits_perconvocatoria_desc'] = 'Máximo de postulaciones por usuario por convocatoria';
$string['backtoapplications'] = 'Volver a postulaciones';
$string['noapplicationsfound'] = 'No se encontraron postulaciones';
$string['allapplicants'] = 'Todos los postulantes';
$string['myapplicationcount'] = 'Mis postulaciones';
$string['myapplications_desc'] = 'Ver y gestionar sus postulaciones';
$string['dateapplied'] = 'Fecha de postulación';
$string['datesubmitted'] = 'Fecha de envío';
$string['viewmyapplications'] = 'Ver mis postulaciones';
$string['confirmwithdraw'] = '¿Está seguro de que desea retirar esta postulación?';
$string['loginrequiredtoapply'] = 'Debe iniciar sesión para postularse';
$string['completeprofile_required'] = 'Por favor complete su perfil antes de postularse';
$string['maxapplicationsperuser'] = 'Máximo de postulaciones por usuario';

// =============================================================================
// CADENAS ADICIONALES - TIPOS Y GESTIÓN DE DOCUMENTOS
// =============================================================================

$string['adddoctype'] = 'Agregar tipo de documento';
$string['doctypelist'] = 'Lista de tipos de documentos';
$string['doctypes_desc'] = 'Configure los tipos de documentos requeridos para las postulaciones';
$string['doctypeshelp'] = 'Ayuda sobre tipos de documentos';
$string['doctypecreated'] = 'Tipo de documento creado exitosamente';
$string['doctypeupdated'] = 'Tipo de documento actualizado exitosamente';
$string['confirmdeletedoctype_msg'] = '¿Está seguro de que desea eliminar este tipo de documento?';
$string['aboutdoctypes'] = 'Acerca de tipos de documentos';
$string['totaldoctypes'] = 'Total de tipos de documentos';
$string['enableddoctypes'] = 'Tipos de documentos habilitados';
$string['requireddoctypes'] = 'Tipos de documentos requeridos';
$string['conditionaldoctypes'] = 'Tipos de documentos condicionales';
$string['conditionalnote'] = 'Este documento es condicionalmente requerido';
$string['conditional_document_note'] = 'Este documento puede ser requerido según ciertas condiciones';
$string['docrequirements'] = 'Requisitos de documentos';
$string['documentsettings'] = 'Configuración de documentos';
$string['documentshelp'] = 'Ayuda sobre documentos';
$string['documentnotfound'] = 'Documento no encontrado';
$string['documentexpired'] = 'Este documento ha vencido';
$string['documentissuedate'] = 'Fecha de expedición';
$string['documentnumber'] = 'Número de documento';
$string['documentref'] = 'Referencia del documento';
$string['documentref_desc'] = 'Número de referencia para este documento';
$string['documentreuploaded'] = 'Documento recargado exitosamente';
$string['documentsapproved'] = 'Documentos aprobados';
$string['documentsreviewed'] = 'Documentos revisados';
$string['documentvalidated'] = 'Documento validado exitosamente';
$string['newdocument'] = 'Nuevo documento';
$string['numdocs'] = 'Número de documentos';
$string['pendingdocs'] = 'Documentos pendientes';
$string['pending_docs_alert'] = 'Tiene {$a} documentos pendientes por cargar';
$string['missing_docs_alert'] = 'Tiene {$a} documentos requeridos por cargar';
$string['pending_review_alert'] = 'Tiene {$a} documentos en espera de revisión';
$string['pendingreviews'] = 'Revisiones pendientes';
$string['actionrequired'] = 'Acción requerida';
$string['missing_documents_list'] = 'Documentos faltantes';
$string['missing_documents'] = 'Documentos sin cargar';
$string['rejected_documents_reupload'] = 'Documentos rechazados';
$string['rejected_documents_list'] = 'Requieren nueva carga';
$string['alreadyvalidated'] = 'Ya validado';
$string['autovalidated'] = 'Auto-validado';
$string['validated'] = 'Validado';
$string['validationapproved'] = 'Validación aprobada';
$string['validationrequirements'] = 'Requisitos de validación';
$string['reuploadhelp'] = 'Cargar una nueva versión de este documento';
$string['uploadnewfile'] = 'Cargar nuevo archivo';
$string['uploadfailed'] = 'Error al cargar';

// =============================================================================
// CADENAS ADICIONALES - CONDICIONES Y CATEGORÍAS DE DOCUMENTOS
// =============================================================================

$string['gendercondition'] = 'Condición de género';
$string['menonly'] = 'Solo hombres';
$string['womenonly'] = 'Solo mujeres';
$string['doc_condition_men_only'] = 'Requerido solo para hombres';
$string['doc_condition_women_only'] = 'Requerido solo para mujeres';
$string['doc_condition_iser_exempt'] = 'Exento para empleados del ISER';
$string['doc_condition_profession_exempt'] = 'Exento según profesión';
$string['professionexempt'] = 'Exento por profesión';
$string['iserexempted'] = 'Exento ISER';
$string['iserexempted_help'] = 'Documentos exentos para empleados del ISER';
$string['multipledocs_'] = 'Múltiples documentos';
$string['multipledocs_notice'] = 'Puede cargar múltiples documentos de este tipo';

// =============================================================================
// CADENAS ADICIONALES - VIGENCIA Y VENCIMIENTO
// =============================================================================

$string['validfrom'] = 'Válido desde';
$string['validuntil'] = 'Válido hasta';
$string['validityperiod'] = 'Período de vigencia';
$string['defaultvalidfrom'] = 'Válido desde por defecto';
$string['defaultvaliduntil'] = 'Válido hasta por defecto';
$string['noexpiry'] = 'Sin vencimiento';
$string['antecedentesmaxdays'] = 'Vigencia de antecedentes (días)';
$string['epsmaxdays'] = 'Vigencia de certificado EPS (días)';
$string['pensionmaxdays'] = 'Vigencia de certificado de pensión (días)';
$string['defaultmaxagedays'] = 'Vigencia predeterminada de documentos (días)';

// =============================================================================
// CADENAS ADICIONALES - EXENCIONES
// =============================================================================

$string['addexemption'] = 'Agregar exención';
$string['exempteddocs'] = 'Documentos exentos';
$string['exempteddocs_desc'] = 'Documentos exentos de requisitos';
$string['exempteddoctypes'] = 'Tipos de documentos exentos';
$string['exemptioncreated'] = 'Exención creada exitosamente';
$string['exemptiondetails'] = 'Detalles de la exención';
$string['exemptionerror'] = 'Error al crear exención';
$string['exemptionnotice'] = 'Aviso de exención';
$string['exemptionreduceddocs'] = 'Requisitos de documentos reducidos por exención';
$string['exemptionrevoked'] = 'Exención revocada exitosamente';
$string['exemptionrevokeerror'] = 'Error al revocar exención';
$string['exemptionupdated'] = 'Exención actualizada exitosamente';
$string['exemptionusagehistory'] = 'Historial de uso de exenciones';
$string['noexemptionusage'] = 'Sin registros de uso de exención';
$string['confirmrevokeexemption'] = '¿Está seguro de que desea revocar esta exención?';
$string['revokeexemption'] = 'Revocar exención';
$string['revoke'] = 'Revocar';
$string['revoked'] = 'Revocado';
$string['revokedby'] = 'Revocado por';
$string['revokereason'] = 'Motivo de revocación';
$string['manageexemptions_desc'] = 'Configurar y gestionar exenciones de documentos';
$string['defaultexemptiontype'] = 'Tipo de exención predeterminado';
$string['age_exempt_notice'] = 'Aviso de exención por edad';
$string['ageexemptionthreshold'] = 'Umbral de exención por edad';

// =============================================================================
// CADENAS ADICIONALES - REVISIÓN Y VALIDACIÓN
// =============================================================================

$string['reviewall'] = 'Revisar todos';
$string['reviewapplication'] = 'Revisar postulación';
$string['reviewdocuments'] = 'Revisar documentos';
$string['reviewsubmitted'] = 'Revisión enviada exitosamente';
$string['reviewsubmitted_with_notification'] = 'Revisión enviada y notificación enviada';
$string['review_dashboard_desc'] = 'Revisar postulaciones y documentos';
$string['backtoreviewlist'] = 'Volver a lista de revisiones';
$string['pendingreview'] = 'Revisión pendiente';
$string['pendingassignments'] = 'Asignaciones pendientes';
$string['bulkvalidation_desc'] = 'Validar múltiples documentos a la vez';
$string['bulkvalidationcomplete'] = 'Validación masiva completada';
$string['bulkrejected'] = 'Rechazado masivamente';
$string['bulkactionerrors'] = 'Algunas acciones masivas fallaron';
$string['avgvalidationtime'] = 'Tiempo promedio de validación';
$string['checklistitems'] = 'Elementos de lista de verificación';

// Dean-specific stats labels.
$string['profilesapproved'] = 'Perfiles aprobados';
$string['profilesrejected'] = 'Perfiles rechazados';

// Export functionality.
$string['exportapplications'] = 'Exportar postulaciones';
$string['exportapplications_desc'] = 'Descargar archivo Excel con la información de postulantes';
$string['applicationid'] = 'ID postulación';
$string['vacancycode'] = 'Código vacante';
$string['applicationdate'] = 'Fecha de postulación';
$string['lastupdate'] = 'Última actualización';
$string['lastreviewer'] = 'Último revisor';
$string['observations'] = 'Observaciones';
$string['statuslabel'] = 'Estado (etiqueta)';
$string['noapplicationsfound'] = 'No se encontraron postulaciones para exportar';
$string['nofacultyassignment'] = 'No tiene facultades asignadas para la exportación';
$string['deandecision'] = 'Decisión decano';
$string['deanobservations'] = 'Observaciones del decano';
$string['reviewedby'] = 'Revisado por';
$string['reviewdate'] = 'Fecha de revisión';
$string['currentstatus'] = 'Estado actual';
$string['pendingdeanreview'] = 'Pendiente revisión';
$string['filterreviews'] = 'Filtrar revisiones';
$string['closed'] = 'Cerrada';

// =============================================================================
// CADENAS ADICIONALES - REVISORES
// =============================================================================

$string['revieweradded'] = 'Revisor agregado exitosamente';
$string['revieweradderror'] = 'Error al agregar revisor';
$string['reviewerremoved'] = 'Revisor eliminado exitosamente';
$string['reviewerremoveerror'] = 'Error al eliminar revisor';
$string['assignreviewers_desc'] = 'Asignar revisores a postulaciones';
$string['assignreviewerhelp'] = 'Asigne revisores a las postulaciones para validación de documentos. Use auto-asignación para distribución balanceada o seleccione postulaciones manualmente.';
$string['autoassigncomplete'] = 'Asignación automática completada';
$string['faculty_reviewers'] = 'Revisores de facultad';
$string['faculty_reviewers_desc'] = 'Revisores (decanos) asignados por facultad para convocatorias tipo DOCENTE';
$string['faculty_reviewer_role_dean'] = 'Decano';
$string['faculty_reviewer_role_lead_reviewer'] = 'Revisor líder';
$string['faculty_reviewer_role_reviewer'] = 'Revisor';
$string['faculty_reviewer_status_active'] = 'Activo';
$string['faculty_reviewer_status_inactive'] = 'Inactivo';
$string['assign_faculty_reviewer'] = 'Asignar revisor de facultad';
$string['assign_faculty_reviewer_desc'] = 'Asignar un revisor (decano) a una facultad para convocatorias tipo DOCENTE';
$string['error:alreadyassigned'] = 'Este usuario ya está asignado como revisor para esta facultad';
$string['faculty_reviewer_assigned'] = 'Revisor asignado exitosamente';
$string['faculty_reviewer_removed'] = 'Revisor eliminado exitosamente';
$string['no_faculty_reviewers'] = 'No hay revisores de facultad asignados';
$string['managefacultyreviewers'] = 'Gestionar revisores por facultad';
$string['managefacultyreviewers_desc'] = 'Asignar decanos y revisores a facultades para el flujo de revisión. Cada decano revisa postulaciones a vacantes dentro de su facultad asignada.';
$string['selectfaculty'] = 'Seleccionar facultad';
$string['currentfacultyreviewers'] = 'Revisores actuales';
$string['nofacultyreviewers'] = 'No hay revisores asignados a esta facultad';
$string['addfacultyreviewer'] = 'Agregar revisor de facultad';
$string['facultyreviewersassigned'] = '{$a} revisor(es) asignado(s) a la facultad';
$string['facultyreviewerunassigned'] = 'Revisor desactivado exitosamente';
$string['facultyreviewerdeleted'] = 'Revisor eliminado exitosamente';
$string['facultyreviewerrolechanged'] = 'Rol del revisor actualizado exitosamente';
$string['facultyreviewerreactivated'] = 'Revisor reactivado exitosamente';
$string['deactivate'] = 'Desactivar';
$string['reactivate'] = 'Reactivar';
$string['assigntofaculty'] = 'Asignar a facultad';
$string['selectusers'] = 'Seleccionar usuarios';
$string['searchuser'] = 'Buscar usuario';
$string['selectmultiple'] = 'Mantenga presionado Ctrl/Cmd para seleccionar múltiples usuarios';
$string['nousersavailableforassignment'] = 'No hay usuarios disponibles para asignar. Los usuarios deben tener la capacidad "Aprobar perfiles".';
$string['assignrole'] = 'Rol';
$string['dateadded'] = 'Fecha de asignación';
$string['confirmdelete'] = '¿Está seguro de que desea eliminar permanentemente esta asignación?';
$string['facultyassignments'] = 'Asignaciones de facultad';
$string['faculties'] = 'facultades';
$string['programs'] = 'Programas';
$string['confirmunassignfaculty'] = '¿Está seguro de que desea eliminar este revisor de la facultad?';
$string['selectdean'] = 'Seleccionar decano...';
$string['nofacultiesavailable'] = 'No hay facultades disponibles';
$string['nofacultiesavailable_desc'] = 'Las facultades se crean automáticamente al importar vacantes con códigos de facultad (ej: FCAS, FII). Importe vacantes primero para crear facultades.';
$string['nofacultiesinconvocatoria_desc'] = 'No se encontraron vacantes con códigos de facultad (FCAS, FII) en esta convocatoria.';
$string['selectconvocatoria'] = 'Seleccionar convocatoria';
$string['noconvocatoriaswithvacancies'] = 'No hay convocatorias con vacantes disponibles. Por favor cree y publique una convocatoria con vacantes primero.';

// =============================================================================
// CADENAS ADICIONALES - COMITÉS
// =============================================================================

$string['committeecreated'] = 'Comité creado exitosamente';
$string['committeecreateerror'] = 'Error al crear comité';
$string['committees_desc'] = 'Gestionar comités de evaluación';
$string['managecommittees'] = 'Gestionar comités';
$string['facultycommitteedefaultname'] = 'Comité de Facultad';
$string['memberadderror'] = 'Error al agregar miembro';
$string['memberremoveerror'] = 'Error al eliminar miembro';

// =============================================================================
// CADENAS ADICIONALES - ENTREVISTAS
// =============================================================================

$string['completeinterview'] = 'Completar entrevista';
$string['interviewcompleted'] = 'Entrevista completada';
$string['interviewfeedback'] = 'Retroalimentación de entrevista';
$string['interviewinstructions'] = 'Instrucciones de entrevista';
$string['interviewscheduleerror'] = 'Error al programar entrevista';
$string['interviewstatus_'] = 'Estado de entrevista';
$string['interviewtype_'] = 'Tipo de entrevista';
$string['interviewtype_inperson'] = 'Presencial';
$string['interviewtype_phone'] = 'Telefónica';
$string['interviewtype_video'] = 'Videollamada';
$string['selectinterviewers'] = 'Seleccionar entrevistadores';
$string['rescheduledby'] = 'Reprogramado por';
$string['reschedulednote'] = 'Nota de reprogramación';
$string['markedasnoshow'] = 'Marcado como no presentado';
$string['markednoshow'] = 'No presentado';

// =============================================================================
// CADENAS ADICIONALES - CALIFICACIONES Y RECOMENDACIONES
// =============================================================================

$string['overallrating'] = 'Calificación general';
$string['rating_excellent'] = 'Excelente';
$string['rating_verygood'] = 'Muy bueno';
$string['rating_good'] = 'Bueno';
$string['rating_fair'] = 'Regular';
$string['rating_poor'] = 'Deficiente';
$string['recommend_'] = 'Recomendación';
$string['recommend_hire'] = 'Recomendar contratación';
$string['recommend_reject'] = 'No recomendar';
$string['recommend_furtherreview'] = 'Requiere revisión adicional';

// =============================================================================
// CADENAS ADICIONALES - PLANTILLAS DE CORREO
// =============================================================================

$string['email_templates'] = 'Plantillas de correo';
$string['emailtemplates_desc'] = 'Gestionar plantillas de notificaciones por correo';
$string['edit_template'] = 'Editar plantilla';
$string['back_to_templates'] = 'Volver a plantillas';
$string['no_templates'] = 'No se encontraron plantillas';
$string['total_templates'] = 'Total de plantillas';
$string['templates_enabled'] = 'Plantillas habilitadas';
$string['templates_disabled'] = 'Plantillas deshabilitadas';
$string['templates_installed'] = 'Plantillas instaladas';
$string['template_name'] = 'Nombre de plantilla';
$string['template_code'] = 'Código de plantilla';
$string['template_subject'] = 'Asunto';
$string['template_body'] = 'Cuerpo';
$string['template_content'] = 'Contenido';
$string['template_description'] = 'Descripción';
$string['template_enabled'] = 'Habilitada';
$string['template_enabled_desc'] = 'Habilitar o deshabilitar esta plantilla';
$string['template_priority'] = 'Prioridad';
$string['template_category'] = 'Categoría';
$string['template_categories'] = 'Categorías';
$string['template_preview'] = 'Vista previa';
$string['template_preview_hint'] = 'Vista previa de cómo se verá el correo';
$string['template_settings'] = 'Configuración de plantilla';
$string['template_info'] = 'Información de plantilla';
$string['template_not_found'] = 'Plantilla no encontrada';
$string['template_saved_success'] = 'Plantilla guardada exitosamente';
$string['template_deleted_success'] = 'Plantilla eliminada exitosamente';
$string['template_enabled_success'] = 'Plantilla habilitada exitosamente';
$string['template_disabled_success'] = 'Plantilla deshabilitada exitosamente';
$string['template_delete_failed'] = 'Error al eliminar plantilla';
$string['template_reset_success'] = 'Plantilla restablecida a valores predeterminados';
$string['template_help_title'] = 'Ayuda de plantillas de correo';
$string['template_help_placeholders'] = 'Use marcadores de posición para insertar contenido dinámico';
$string['template_help_html'] = 'Se admite formato HTML';
$string['template_help_tenant'] = 'Las plantillas se pueden personalizar por sede';
$string['email_updated'] = 'Plantilla de correo actualizada';
$string['email_action_reupload'] = 'Solicitar recarga de documento';
$string['toggle_status'] = 'Cambiar estado';
$string['reset_to_default'] = 'Restablecer valores predeterminados';

// =============================================================================
// CADENAS ADICIONALES - MARCADORES DE POSICIÓN
// =============================================================================

$string['placeholders'] = 'Marcadores de posición';
$string['placeholders_help'] = 'Marcadores disponibles para esta plantilla';
$string['available_placeholders'] = 'Marcadores disponibles';
$string['copy_placeholder'] = 'Copiar marcador';
$string['html_support'] = 'HTML compatible';

// =============================================================================
// CADENAS ADICIONALES - REGISTRO Y PERFIL
// =============================================================================

$string['basicinfo'] = 'Información básica';
$string['personalinfo'] = 'Información personal';
$string['education'] = 'Educación';
$string['educationlevel'] = 'Nivel de educación';
$string['coverletter'] = 'Carta de presentación';
$string['declaration'] = 'Declaración';
$string['declarationtext'] = 'Declaro que toda la información proporcionada es precisa y completa';
$string['declarationaccept'] = 'Acepto la declaración';
$string['declarationrequired'] = 'Debe aceptar la declaración para continuar';
$string['verification'] = 'Verificación';
$string['profilereview'] = 'Revisión de perfil';
$string['profilereview_info'] = 'Por favor revise su información de perfil';
$string['updateprofile_intro'] = 'Actualice su información de perfil';
$string['updateprofile_submit'] = 'Actualizar perfil';
$string['updateprofile_success'] = 'Perfil actualizado exitosamente';
$string['updateprofile_title'] = 'Actualizar perfil';
$string['update_username'] = 'Actualizar nombre de usuario';
$string['update_username_desc'] = 'Permitir a los usuarios actualizar su nombre de usuario';

// =============================================================================
// CADENAS ADICIONALES - CONTRASEÑA
// =============================================================================

$string['password'] = 'Contraseña';
$string['currentpassword'] = 'Contraseña actual';
$string['newpassword'] = 'Nueva contraseña';
$string['confirmpassword'] = 'Confirmar contraseña';
$string['currentpassword_invalid'] = 'La contraseña actual es incorrecta';
$string['currentpassword_required'] = 'Se requiere la contraseña actual';
$string['passwordsdiffer'] = 'Las contraseñas no coinciden';
$string['password_change_optional'] = 'Deje en blanco para mantener la contraseña actual';
$string['password_updated'] = 'Contraseña actualizada exitosamente';
$string['company_updated'] = 'Sede actualizada exitosamente a: {$a}';
$string['department_updated'] = 'Departamento actualizado exitosamente';
$string['company_change_notice'] = 'Ha cambiado su sede. Esta acción puede afectar las vacantes disponibles para usted.';

// =============================================================================
// CADENAS ADICIONALES - CONSENTIMIENTO Y GDPR
// =============================================================================

$string['consentaccepttext'] = 'Acepto la política de tratamiento de datos';
$string['consentheader'] = 'Consentimiento de datos';
$string['consentrequired'] = 'Debe aceptar la política de tratamiento de datos';
$string['datatreatmentpolicytitle'] = 'Política de tratamiento de datos';
$string['defaultdatatreatmentpolicy'] = 'Política de tratamiento de datos predeterminada';
$string['dataretentiondays'] = 'Período de retención de datos (días)';
$string['dataexport:consent'] = 'Registros de consentimiento';
$string['dataexport:exportdate'] = 'Fecha de exportación';
$string['dataexport:personal'] = 'Datos personales';
$string['dataexport:title'] = 'Exportación de datos';
$string['dataexport:userinfo'] = 'Información del usuario';
$string['datatorexport'] = 'Datos a exportar';

// =============================================================================
// CADENAS ADICIONALES - REPORTES
// =============================================================================

$string['reports_desc'] = 'Ver reportes de postulaciones y vacantes';
$string['viewreports'] = 'Ver reportes';
$string['reportapplications'] = 'Reporte de postulaciones';
$string['reportdocuments'] = 'Reporte de documentos';
$string['reportoverview'] = 'Reporte general';
$string['reportreviewers'] = 'Reporte de revisores';
$string['reporttimeline'] = 'Reporte cronológico';
$string['generatedon'] = 'Generado el';
$string['selectionrate'] = 'Tasa de selección';

// =============================================================================
// CADENAS ADICIONALES - IMPORTACIÓN/EXPORTACIÓN
// =============================================================================

$string['importvacancies'] = 'Importar vacantes';
$string['importvacancies_desc'] = 'Importar vacantes desde archivo CSV';
$string['importvacancies_help'] = 'Cargue un archivo CSV con datos de vacantes';
$string['importdata'] = 'Importar datos';
$string['importdata_desc'] = 'Importar datos de fuentes externas';
$string['importupload'] = 'Cargar archivo';
$string['importinstructions'] = 'Instrucciones de importación';
$string['importinstructionstext'] = 'Siga estas instrucciones para importar datos correctamente';
$string['importcomplete'] = 'Importación completada';
$string['importerror'] = 'Error de importación';
$string['importerrors'] = 'Errores de importación';
$string['importwarning'] = 'Advertencia de importación';
$string['importingfrom'] = 'Importando desde';
$string['importednote'] = 'Nota de importación';
$string['importedapplications'] = 'Postulaciones importadas';
$string['importedconvocatorias'] = 'Convocatorias importadas';
$string['importeddoctypes'] = 'Tipos de documentos importados';
$string['importeddocuments'] = 'Documentos importados';
$string['importedemails'] = 'Plantillas de correo importadas';
$string['importedexemptions'] = 'Exenciones importadas';
$string['importedfiles'] = 'Archivos importados';
$string['importedsettings'] = 'Configuraciones importadas';
$string['importedskipped'] = 'Omitidos';
$string['importedsuccess'] = 'Importados exitosamente';
$string['importedvacancies'] = 'Vacantes importadas';
$string['importerror_alreadyexempt'] = 'El usuario ya tiene exención';
$string['importerror_createfailed'] = 'Error al crear registro';
$string['importerror_usernotfound'] = 'Usuario no encontrado';
$string['importerror_vacancyexists'] = 'La vacante ya existe';
$string['exportdata_desc'] = 'Exportar datos de postulaciones y vacantes';
$string['exportdownload'] = 'Descargar exportación';
$string['exporterror'] = 'Error de exportación';
$string['exportwarning_files'] = 'Advertencia: Los archivos no se incluirán en la exportación';
$string['fullexport'] = 'Exportación completa';
$string['fullexport_info'] = 'Exportar todos los datos incluyendo archivos';

// =============================================================================
// CADENAS ADICIONALES - IMPORTACIÓN CSV
// =============================================================================

$string['csvfile'] = 'Archivo CSV';
$string['csvformat'] = 'Formato CSV';
$string['csvformat_desc'] = 'Formato esperado para importación CSV';
$string['csvdelimiter'] = 'Delimitador CSV';
$string['csvexample'] = 'Ejemplo CSV';
$string['csvexample_desc'] = 'Ejemplo del formato CSV esperado';
$string['csvexample_tip'] = 'Descargue el archivo CSV de ejemplo como plantilla';
$string['csvimporterror'] = 'Error de importación CSV';
$string['csvinvalidtype'] = 'Tipo de archivo inválido. Por favor cargue un archivo CSV.';
$string['csvlineerror'] = 'Error en la línea {$a}';
$string['csvusernotfound'] = 'Usuario no encontrado en línea CSV {$a}';
$string['csvcolumn_code'] = 'Columna de código';
$string['csvcolumn_contracttype'] = 'Columna de tipo de contrato';
$string['csvcolumn_courses'] = 'Columna de cursos';
$string['csvcolumn_faculty'] = 'Columna de facultad';
$string['csvcolumn_location'] = 'Columna de ubicación';
$string['csvcolumn_modality'] = 'Columna de modalidad';
$string['csvcolumn_profile'] = 'Columna de perfil';
$string['csvcolumn_program'] = 'Columna de programa';
$string['requiredcolumns'] = 'Columnas requeridas';
$string['optionalcolumns'] = 'Columnas opcionales';
$string['samplecsv'] = 'CSV de ejemplo';
$string['downloadcsvtemplate'] = 'Descargar plantilla CSV';
$string['encoding'] = 'Codificación';
$string['dryrunmode'] = 'Modo de prueba';
$string['dryrunresults'] = 'Resultados de prueba';
$string['overwriteexisting'] = 'Sobrescribir existentes';
$string['updateexisting'] = 'Actualizar registros existentes';
$string['updateexisting_help'] = 'Si está habilitado, las vacantes existentes con el mismo código serán actualizadas con los datos importados. Si está deshabilitado, las vacantes existentes serán omitidas.';
$string['vacancies_created'] = 'Vacantes creadas';
$string['vacancies_skipped'] = 'Vacantes omitidas';
$string['vacancies_updated'] = 'Vacantes actualizadas';

// =============================================================================
// CADENAS ADICIONALES - MIGRACIÓN
// =============================================================================

$string['migrateplugin'] = 'Migrar plugin';
$string['migrateplugin_desc'] = 'Migrar datos de otro sistema';
$string['migrationfile'] = 'Archivo de migración';
$string['migrationinfo_desc'] = 'Información de migración';
$string['migrationinfo_title'] = 'Migración';
$string['invalidmigrationfile'] = 'Archivo de migración inválido';
$string['advancedtools'] = 'Herramientas Avanzadas';
$string['migration'] = 'Migración';
$string['migration_desc'] = 'Importar y exportar datos de sistemas externos';

// =============================================================================
// CADENAS ADICIONALES - CONFIGURACIÓN
// =============================================================================

$string['configuration'] = 'Configuración';
$string['configure'] = 'Configurar';
$string['pluginsettings_desc'] = 'Configurar ajustes del plugin';
$string['navigationsettings'] = 'Configuración de navegación';
$string['navigationsettings_desc'] = 'Configurar opciones de navegación';
$string['mainmenutitle'] = 'Título del menú principal';
$string['mainmenutitle_desc'] = 'Título mostrado en el menú principal';
$string['showinmainmenu'] = 'Mostrar en menú principal';
$string['showinmainmenu_desc'] = 'Mostrar enlace en el menú de navegación principal';
$string['showpublicnavlink'] = 'Mostrar enlace de navegación pública';
$string['showpublicnavlink_desc'] = 'Mostrar enlace a la página pública de vacantes en la navegación';
$string['publicpagesettings'] = 'Configuración de página pública';
$string['publicpagesettings_desc'] = 'Configurar página pública de vacantes';
$string['publicpagetitle'] = 'Título de página pública';
$string['publicpagetitle_desc'] = 'Título para la página pública de vacantes';
$string['publicpagedesc'] = 'Descripción de página pública';
$string['publicpagedescription'] = 'Descripción de página pública';
$string['publicpagedescription_desc'] = 'Descripción mostrada en la página pública de vacantes';
$string['enablepublicpage'] = 'Habilitar página pública';
$string['enablepublicpage_desc'] = 'Permitir acceso público a listados de vacantes';
$string['enableselfregistration'] = 'Habilitar auto-registro';
$string['enableselfregistration_desc'] = 'Permitir que los usuarios se registren por sí mismos';
$string['enableapi'] = 'Habilitar API';
$string['enableencryption'] = 'Habilitar encriptación';
$string['securitysettings'] = 'Configuración de seguridad';
$string['allowedformats'] = 'Formatos de archivo permitidos';
$string['allowedformats_desc'] = 'Formatos de archivo permitidos para carga de documentos';
$string['allowmultipleapplications_convocatoria'] = 'Permitir múltiples postulaciones por convocatoria';
$string['allowmultipleapplications_convocatoria_desc'] = 'Permitir a los usuarios postularse a múltiples vacantes en la misma convocatoria';
$string['recordsperpage'] = 'Registros por página';

// =============================================================================
// CADENAS ADICIONALES - RECAPTCHA ENTERPRISE
// =============================================================================

$string['recaptchasettings'] = 'Configuración de reCAPTCHA';
$string['recaptchasettings_desc'] = 'Configurar Google reCAPTCHA Enterprise o estándar para formularios';
$string['recaptcha_enabled'] = 'Habilitar reCAPTCHA';
$string['recaptcha_enabled_desc'] = 'Habilitar verificación reCAPTCHA en formularios';
$string['recaptcha_type'] = 'Tipo de reCAPTCHA';
$string['recaptcha_type_desc'] = 'Seleccione Enterprise (recomendado) o estándar';
$string['recaptcha_type_enterprise'] = 'reCAPTCHA Enterprise';
$string['recaptcha_type_standard'] = 'reCAPTCHA Estándar';
$string['recaptcha_sitekey'] = 'Clave del sitio';
$string['recaptcha_sitekey_desc'] = 'Clave del sitio de Google reCAPTCHA';
$string['recaptcha_project_id'] = 'ID del proyecto';
$string['recaptcha_project_id_desc'] = 'ID del proyecto de Google Cloud (solo para Enterprise)';
$string['recaptcha_api_key'] = 'Clave de API';
$string['recaptcha_api_key_desc'] = 'Clave de API de Google Cloud (solo para Enterprise)';
$string['recaptcha_secretkey'] = 'Clave secreta';
$string['recaptcha_secretkey_desc'] = 'Clave secreta de Google reCAPTCHA (solo para versión estándar)';
$string['recaptcha_threshold'] = 'Umbral de puntuación';
$string['recaptcha_threshold_desc'] = 'Puntuación mínima para aprobar la verificación (0.0 a 1.0). Valores más altos son más estrictos.';
$string['recaptcha_failed'] = 'La verificación reCAPTCHA falló';
$string['recaptcha_required'] = 'Por favor complete la verificación reCAPTCHA';
$string['recaptcha_config_error'] = 'Error de configuración de reCAPTCHA. Contacte al administrador.';
$string['recaptcha_network_error'] = 'Error de red al verificar reCAPTCHA. Intente de nuevo.';
$string['recaptcha_api_error'] = 'Error de API de reCAPTCHA. Intente de nuevo.';
$string['recaptcha_invalid_response'] = 'Respuesta inválida del servidor reCAPTCHA';
$string['recaptcha_invalid_token'] = 'Token de reCAPTCHA inválido o expirado';
$string['recaptcha_action_mismatch'] = 'Acción de reCAPTCHA no coincide';
$string['recaptcha_score_too_low'] = 'La puntuación de reCAPTCHA es muy baja. Intente de nuevo.';

// =============================================================================
// CADENAS ADICIONALES - INTEGRACIÓN IOMAD
// =============================================================================

$string['iomadsettings'] = 'Configuración de IOMAD';
$string['iomadoptions'] = 'Opciones de IOMAD';
$string['iomad_department'] = 'Departamento';
$string['multi_tenant'] = 'Multi-sede';
$string['allcompanies'] = 'Todas las sedes';
$string['alldepartments'] = 'Todos los departamentos';
$string['selectcompany'] = 'Seleccionar sede';
$string['selectdepartment'] = 'Seleccionar departamento';
$string['selectconvocatoriafirst'] = 'Por favor seleccione una convocatoria primero';
$string['createcompanies'] = 'Crear sedes';
$string['createcompanies_help'] = 'Si está habilitado, las sedes referenciadas en el archivo de importación que no existan en IOMAD serán creadas automáticamente. Si está deshabilitado, las vacantes que referencien sedes inexistentes serán omitidas.';

// =============================================================================
// CADENAS ADICIONALES - ROLES Y CAPACIDADES
// =============================================================================

$string['manageroles'] = 'Gestionar roles';
$string['manageroles_desc'] = 'Configurar asignaciones de roles';
$string['rolechanged'] = 'Rol cambiado exitosamente';
$string['rolechangeerror'] = 'Error al cambiar rol';
$string['cap_assignreviewers'] = 'Asignar revisores';
$string['cap_createvacancy'] = 'Crear vacante';
$string['cap_download'] = 'Descargar';
$string['cap_evaluate'] = 'Evaluar';
$string['cap_manage'] = 'Gestionar';
$string['cap_review'] = 'Revisar';
$string['cap_validate'] = 'Validar';
$string['cap_reviewprofiles'] = 'Revisar perfiles';
$string['cap_approveprofile'] = 'Aprobar/rechazar perfiles';
$string['cap_validatehr'] = 'Validación final de documentos';
$string['cap_viewreports'] = 'Ver reportes';

// =============================================================================
// CADENAS ADICIONALES - CARACTERÍSTICAS Y DESCRIPCIONES
// =============================================================================

$string['features'] = 'Características';
$string['feature_assign_reviewers'] = 'Asignar revisores a postulaciones';
$string['feature_create_convocatorias'] = 'Crear y gestionar convocatorias';
$string['feature_create_vacancies'] = 'Crear nuevas publicaciones de vacantes';
$string['feature_import_export'] = 'Importar y exportar datos';
$string['feature_manage_vacancies'] = 'Gestionar vacantes existentes';
$string['feature_publish_vacancies'] = 'Publicar vacantes para postulantes';
$string['feature_review_documents'] = 'Revisar documentos enviados';
$string['feature_track_applications'] = 'Seguimiento de estado de postulaciones';
$string['feature_validate_applications'] = 'Validar documentos de postulaciones';

// =============================================================================
// CADENAS ADICIONALES - TAREAS Y EVENTOS
// =============================================================================

$string['task:checkclosingvacancies'] = 'Verificar vacantes por cerrar';
$string['task:cleanupolddata'] = 'Limpiar datos antiguos';
$string['task:sendnotifications'] = 'Enviar notificaciones pendientes';
$string['event:applicationcreated'] = 'Postulación creada';
$string['event:applicationdeleted'] = 'Postulación eliminada';
$string['event:documentuploaded'] = 'Documento cargado';
$string['event:statuschanged'] = 'Estado cambiado';
$string['event:vacancyclosed'] = 'Vacante cerrada';
$string['event:vacancycreated'] = 'Vacante creada';
$string['event:vacancydeleted'] = 'Vacante eliminada';
$string['event:vacancypublished'] = 'Vacante publicada';
$string['event:vacancyupdated'] = 'Vacante actualizada';

// =============================================================================
// CADENAS ADICIONALES - ERRORES Y VALIDACIÓN
// =============================================================================

$string['error:alreadyapplied'] = 'Ya se ha postulado a esta vacante';
$string['error:applicationlimitreached'] = 'Límite de postulaciones alcanzado';
$string['error:cannotdelete_hasapplications'] = 'No se puede eliminar: la vacante tiene postulaciones';
$string['error:cannotdeleteconvocatoria'] = 'No se puede eliminar la convocatoria';
$string['error:cannotreopenconvocatoria'] = 'No se puede reabrir la convocatoria';
$string['error:codealreadyexists'] = 'El código ya existe';
$string['error:codeexists'] = 'Este código ya está en uso';
$string['error:consentrequired'] = 'Debe aceptar el consentimiento de datos';
$string['error:convocatoriacodeexists'] = 'El código de convocatoria ya existe';
$string['error:convocatoriadatesinvalid'] = 'Fechas de convocatoria inválidas';
$string['error:convocatoriahasnovacancies'] = 'La convocatoria no tiene vacantes';
$string['error:convocatoriarequired'] = 'Se requiere una convocatoria';
$string['error:doctypeinuse'] = 'El tipo de documento está en uso y no puede ser eliminado';
$string['error:invalidage'] = 'Edad inválida';
$string['error:invalidcode'] = 'Formato de código inválido';
$string['error:invaliddates'] = 'Fechas inválidas';
$string['error:invalidpublicationtype'] = 'Tipo de publicación inválido';
$string['error:invalidstatus'] = 'Estado inválido';
$string['error:invalidurl'] = 'Formato de URL inválido';
$string['error:invalidapplication'] = 'Postulación inválida';
$string['error:noaccesstoapplication'] = 'No tiene acceso a esta postulación. La vacante no pertenece a su facultad asignada.';
$string['error:invaliduser'] = 'Usuario inválido';
$string['error:nodocuments'] = 'No se encontraron documentos para esta postulación';
$string['error:nodocumentstodownload'] = 'No hay documentos disponibles para descargar';
$string['error:noapplicationsselected'] = 'No hay postulaciones seleccionadas para descargar';
$string['error:cannotcreatezip'] = 'No se puede crear el archivo ZIP';
$string['error:occasionalrequiresexperience'] = 'El contrato ocasional requiere experiencia';
$string['error:pastdate'] = 'La fecha no puede ser en el pasado';
$string['error:requiredfield'] = 'Este campo es requerido';
$string['error:schedulingconflict'] = 'Conflicto de programación detectado';
$string['error:singleapplicationonly'] = 'Solo se permite una postulación';
$string['error:vacancyclosed'] = 'Esta vacante está cerrada';
$string['error:vacancynotfound'] = 'Vacante no encontrada';
$string['error:publicpagedisabled'] = 'La página pública de convocatorias no está disponible en este momento';
$string['error:publicpagedisabled_title'] = 'Página no disponible';
$string['error:publicpagedisabled_desc'] = 'El acceso público a las convocatorias y vacantes ha sido temporalmente deshabilitado por el administrador del sistema.';
$string['error:publicpagedisabled_action'] = 'Si usted es un usuario registrado, por favor inicie sesión para acceder a las convocatorias disponibles.';
$string['error:convocatorianotfound'] = 'Convocatoria no encontrada';
$string['error:convocatorianotpublic'] = 'Esta convocatoria es interna y solo está disponible para miembros de la organización';
$string['invalidemail'] = 'Dirección de correo inválida';
$string['emailexists'] = 'El correo ya existe';
$string['emailagain'] = 'Correo (de nuevo)';
$string['emailnotmatch'] = 'Los correos no coinciden';
$string['completerequiredfields'] = 'Por favor complete todos los campos requeridos';
$string['signaturetoooshort'] = 'La firma es muy corta';
$string['maximumchars'] = 'Máximo {$a} caracteres';

// =============================================================================
// CADENAS ADICIONALES - MENSAJES DE ESTADO
// =============================================================================

$string['statuschanged'] = 'Estado cambiado exitosamente';
$string['statuschangeerror'] = 'Error al cambiar estado';
$string['changessaved'] = 'Cambios guardados exitosamente';
$string['savechanges'] = 'Guardar cambios';
$string['saveresults'] = 'Guardar resultados';
$string['inprogress'] = 'En progreso';
$string['cancelledby'] = 'Cancelado por';
$string['defaultstatus'] = 'Estado predeterminado';
$string['allstatuses'] = 'Todos los estados';
$string['allfaculties'] = 'Todas las facultades';
$string['allprograms'] = 'Todos los programas';
$string['allcontracttypes'] = 'Todos los tipos de contrato';
$string['selectcontracttype'] = 'Seleccionar tipo de contrato';
$string['selectmodality'] = 'Seleccionar modalidad';
$string['idnumber_cedula'] = 'Cédula / ID';
$string['idnumber_placeholder'] = 'Ingrese número de cédula...';
$string['applicationsselected'] = 'postulaciones seleccionadas';
$string['downloadselected'] = 'Descargar seleccionados';
$string['clearselection'] = 'Limpiar selección';
$string['selectatleastone'] = 'Por favor seleccione al menos una opción';
$string['selectbackgrounddocs'] = 'Seleccionar documentos de antecedentes';
$string['selectidentitydocs'] = 'Seleccionar documentos de identidad';
$string['selected'] = 'Seleccionado';
$string['selectacountry'] = 'Seleccionar un país';
$string['confirm_reset'] = 'Confirmar restablecimiento';

// =============================================================================
// CADENAS ADICIONALES - PANEL Y BIENVENIDA
// =============================================================================

$string['dashboard_admin_welcome'] = 'Bienvenido, Administrador';
$string['dashboard_applicant_welcome'] = 'Bienvenido a su panel de postulante';
$string['dashboard_manager_welcome'] = 'Bienvenido al panel de gestión';
$string['dashboard_reviewer_welcome'] = 'Bienvenido al panel de revisor';
$string['dashboard_dean_welcome'] = 'Bienvenido al panel de decano';

// =============================================================================
// CADENAS ADICIONALES - NOTIFICACIONES
// =============================================================================

$string['notification_'] = 'Notificación';
$string['deadlinewarning'] = 'Fecha límite próxima';

// =============================================================================
// CADENAS ADICIONALES - FECHAS Y TIEMPO
// =============================================================================

$string['dates'] = 'Fechas';
$string['dateandtime'] = 'Fecha y hora';
$string['datefrom'] = 'Fecha desde';
$string['dateto'] = 'Fecha hasta';
$string['strftimedate'] = '%d de %B de %Y';
$string['strftimedateshort'] = '%d/%m/%Y';
$string['strftimedatetime'] = '%d de %B de %Y, %H:%M';
$string['duration'] = 'Duración';

// =============================================================================
// CADENAS ADICIONALES - TABLAS Y LISTAS
// =============================================================================

$string['row'] = 'Fila';
$string['items'] = 'Elementos';
$string['show'] = 'Mostrar';
$string['perpage'] = 'Por página';
$string['showingxofy'] = 'Mostrando {$a->from} a {$a->to} de {$a->total}';
$string['showingxtoy'] = 'Mostrando {$a->from} a {$a->to} de {$a->total}';
$string['andmore'] = 'y {$a} más';
$string['moveup'] = 'Mover arriba';
$string['movedown'] = 'Mover abajo';
$string['sortorder'] = 'Orden';
$string['dragtoorder'] = 'Arrastra para reordenar';
$string['reorder_success'] = 'Orden guardado exitosamente';

// =============================================================================
// CADENAS ADICIONALES - CONVERSIÓN Y ARCHIVOS
// =============================================================================

$string['conversionfailed'] = 'Conversión fallida';
$string['conversioninprogress'] = 'Conversión en progreso';
$string['conversionpending'] = 'Conversión pendiente';
$string['conversionready'] = 'Conversión lista';
$string['files'] = 'Archivos';
$string['digitalsignature'] = 'Firma digital';
$string['externalurl'] = 'URL externa';
$string['locationorurl'] = 'Ubicación o URL';
$string['courses'] = 'Cursos';

// =============================================================================
// CADENAS ADICIONALES - TIPOS DE ENTRADA
// =============================================================================

$string['inputtype'] = 'Tipo de entrada';
$string['inputtype_file'] = 'Carga de archivo';
$string['inputtype_number'] = 'Número';
$string['inputtype_text'] = 'Texto';
$string['inputtype_url'] = 'URL';
$string['inputtype_help'] = 'Seleccione cómo los postulantes proporcionarán este documento: archivo (cargar PDF/imagen), texto (campo de texto libre), URL (enlace externo) o número (valor numérico).';

// =============================================================================
// CADENAS DE AYUDA - FORMULARIO DE TIPOS DE DOCUMENTO
// =============================================================================

$string['externalurl_help'] = 'URL opcional donde los postulantes pueden obtener o tramitar este documento (ej: sitio web gubernamental para certificados).';
$string['defaultmaxagedays_help'] = 'Número máximo de días de antigüedad que puede tener el documento. Déjelo vacío o en 0 si no hay límite de vigencia.';
$string['gendercondition_help'] = 'Seleccione si este documento solo aplica a un género específico. Por ejemplo, la libreta militar puede requerirse solo para hombres.';
$string['ageexemptionthreshold_help'] = 'Edad a partir de la cual los postulantes quedan exentos de presentar este documento. Déjelo vacío si no hay exención por edad.';
$string['professionexempt_help'] = 'Seleccione los niveles educativos que están exentos de presentar este documento. Por ejemplo, profesionales o posgraduados pueden estar exentos de ciertos requisitos.';
$string['conditionalnote_help'] = 'Nota explicativa que se mostrará cuando este documento sea condicionalmente requerido, indicando en qué casos aplica o no aplica.';
$string['checklistitems_help'] = 'Lista de verificación para validar el documento. Ingrese un elemento por línea. Los revisores marcarán cada elemento al validar.';

// =============================================================================
// CADENAS ADICIONALES - ELEMENTOS DE FORMULARIO
// =============================================================================

$string['step'] = 'Paso';
$string['conditions'] = 'Condiciones';
$string['default'] = 'Predeterminado';
$string['desirable'] = 'Deseable';
$string['internal'] = 'Interno';
$string['publicationtype'] = 'Tipo de publicación';
$string['briefdescription'] = 'Descripción breve';
$string['contactemail'] = 'Correo de contacto';
$string['institutionname'] = 'Nombre de institución';
$string['subject'] = 'Asunto';
$string['example'] = 'Ejemplo';
$string['noobservations'] = 'Sin observaciones';
$string['noreason'] = 'Sin motivo proporcionado';
$string['notes_desc'] = 'Notas adicionales';
$string['hasnote'] = 'Tiene nota';

// =============================================================================
// CADENAS ADICIONALES - CONSEJOS Y DIRECTRICES
// =============================================================================

$string['guideline1'] = 'Complete todos los campos requeridos';
$string['guideline2'] = 'Cargue documentos en formato PDF';
$string['guideline3'] = 'Verifique toda la información antes de enviar';
$string['guideline4'] = 'Guarde su progreso frecuentemente';
$string['tip_checkdocs'] = 'Verifique que todos los documentos sean legibles';
$string['tip_deadline'] = 'Envíe antes de la fecha límite';
$string['tip_saveoften'] = 'Guarde su progreso frecuentemente';

// =============================================================================
// CADENAS ADICIONALES - VISTA PREVIA
// =============================================================================

$string['previewconfirm'] = 'Confirmar vista previa';
$string['previewmode'] = 'Modo de vista previa';
$string['previewmodenotice'] = 'Está en modo de vista previa. Los cambios no se guardarán.';
$string['previewonly'] = 'Solo vista previa';
$string['previewtotal'] = 'Total de vista previa';
$string['previewunavailable'] = 'Vista previa no disponible';

// =============================================================================
// CADENAS ADICIONALES - ENCRIPTACIÓN
// =============================================================================

$string['encryption:backupinstructions'] = 'Instrucciones de respaldo de clave de encriptación';
$string['encryption:nokeytobackup'] = 'No hay clave de encriptación para respaldar';

// =============================================================================
// CADENAS ADICIONALES - INSTALACIÓN
// =============================================================================

$string['install_defaults'] = 'Instalar configuración predeterminada';

// =============================================================================
// CADENAS ADICIONALES - PREFIJOS DE ESTADO DE DOCUMENTOS (Dinámicos)
// =============================================================================

$string['docstatus_'] = 'Estado del documento';
$string['appstatus:'] = 'Estado de postulación';

// =============================================================================
// CATEGORÍAS DE DOCUMENTOS
// =============================================================================

$string['doccategory_identity'] = 'Documentos de identidad';
$string['doccategory_academic'] = 'Documentos académicos';
$string['doccategory_professional'] = 'Documentos profesionales';
$string['doccategory_background'] = 'Documentos de antecedentes';
$string['doccategory_financial'] = 'Documentos financieros';
$string['doccategory_health'] = 'Documentos de salud';

// =============================================================================
// LISTA DE VERIFICACIÓN DE VALIDACIÓN DE DOCUMENTOS
// =============================================================================

$string['checklist_legible'] = 'El documento es legible';
$string['checklist_complete'] = 'El documento está completo';
$string['checklist_namematch'] = 'El nombre coincide con la postulación';
$string['checklist_cedula_number'] = 'El número de cédula es visible';
$string['checklist_cedula_photo'] = 'La foto es visible';
$string['checklist_background_date'] = 'La fecha de expedición es reciente';
$string['checklist_background_status'] = 'Estado de antecedentes limpio';
$string['checklist_title_institution'] = 'La institución está reconocida';
$string['checklist_title_date'] = 'La fecha de grado está verificada';
$string['checklist_title_program'] = 'El nombre del programa coincide';
$string['checklist_acta_number'] = 'El número de acta está presente';
$string['checklist_acta_date'] = 'La fecha del acta está verificada';
$string['checklist_tarjeta_number'] = 'El número de tarjeta profesional está presente';
$string['checklist_tarjeta_profession'] = 'La profesión coincide con el cargo';
$string['checklist_rut_nit'] = 'El NIT está verificado';
$string['checklist_rut_updated'] = 'El documento tributario está actualizado';
$string['checklist_eps_active'] = 'La EPS está activa';
$string['checklist_eps_entity'] = 'La entidad de EPS está verificada';
$string['checklist_pension_fund'] = 'El fondo de pensiones está identificado';
$string['checklist_pension_active'] = 'Las cotizaciones de pensión están activas';
$string['checklist_medical_date'] = 'La fecha del examen médico es reciente';
$string['checklist_medical_aptitude'] = 'El estado de aptitud es favorable';
$string['checklist_military_class'] = 'La clase militar está verificada';
$string['checklist_military_number'] = 'El número de libreta militar está presente';

// =============================================================================
// REPORTES - MENSAJES DE ESTADO VACÍO
// =============================================================================

$string['noapplicationsreport'] = 'No se encontraron postulaciones para los criterios seleccionados';
$string['novacanciesreport'] = 'No se encontraron vacantes para los criterios seleccionados';
$string['nodocumentsreport'] = 'No se encontraron documentos para los criterios seleccionados';
$string['noconvocatoriasreport'] = 'No se encontraron convocatorias para los criterios seleccionados';

// =============================================================================
// NAVEGACIÓN - ADICIONAL
// =============================================================================

$string['browsevacancies'] = 'Explorar vacantes';
$string['browsevacancies_desc'] = 'Explore todas las vacantes disponibles en nuestras convocatorias activas';
$string['createconvocatoria'] = 'Crear convocatoria';
$string['backtomanage'] = 'Volver a gestión';
$string['reviewdashboard'] = 'Panel de revisión';
$string['featuredvacancies'] = 'Vacantes destacadas';
$string['noconvocatoriasavailable'] = 'No hay convocatorias disponibles';
$string['novacanciesavailable'] = 'No hay vacantes disponibles';
$string['noapplicationsavailable'] = 'No hay postulaciones disponibles';

// =============================================================================
// ROLES DE USUARIO
// =============================================================================

$string['role_reviewer'] = 'Revisor';
$string['role_coordinator'] = 'Coordinador';
$string['role_administrator'] = 'Administrador';
$string['role_manager'] = 'Gestor';
$string['role_applicant'] = 'Postulante';
$string['role_evaluator'] = 'Evaluador';
$string['role_secretary'] = 'Secretario';
$string['role_director'] = 'Director';
$string['role_dean'] = 'Decano';

// =============================================================================
// RAZONES DE RECHAZO
// =============================================================================

$string['rejectreason_expired'] = 'El documento ha expirado';
$string['rejectreason_illegible'] = 'El documento es ilegible';
$string['rejectreason_mismatch'] = 'La información no coincide';
$string['rejectreason_incomplete'] = 'El documento está incompleto';
$string['rejectreason_invalid'] = 'El documento es inválido';
$string['rejectreason_wrong_format'] = 'El documento está en formato incorrecto';
$string['rejectreason_other'] = 'Otra razón';

// =============================================================================
// PASOS DEL FLUJO DE TRABAJO
// =============================================================================

$string['step_consent'] = 'Consentimiento';
$string['step_documents'] = 'Documentos';
$string['step_profile'] = 'Perfil';
$string['step_coverletter'] = 'Carta de presentación';
$string['step_submit'] = 'Enviar';
$string['step_review'] = 'Revisión';
$string['step_evaluation'] = 'Evaluación';
$string['step_interview'] = 'Entrevista';
$string['step_selection'] = 'Selección';

// =============================================================================
// TIPOS DE DOCUMENTOS - ESPECÍFICOS
// =============================================================================

$string['doctype_tarjeta_profesional'] = 'Tarjeta profesional';
$string['doctype_sigep'] = 'Registro SIGEP';
$string['doctype_antecedentes_policia'] = 'Antecedentes de policía';
$string['doctype_antecedentes_fiscalia'] = 'Antecedentes de fiscalía';
$string['doctype_antecedentes_contraloria'] = 'Antecedentes de contraloría';
$string['doctype_antecedentes_procuraduria'] = 'Antecedentes de procuraduría';
$string['doctype_libreta_militar'] = 'Libreta militar';
$string['doctype_certificado_eps'] = 'Certificado de EPS';
$string['doctype_certificado_pension'] = 'Certificado de pensión';
$string['doctype_rut'] = 'RUT';
$string['doctype_examen_medico'] = 'Examen médico ocupacional';
$string['doctype_titulo'] = 'Título profesional';
$string['doctype_acta_grado'] = 'Acta de grado';

// =============================================================================
// TIPOS DE PUBLICACIÓN
// =============================================================================

$string['publicationtype:public'] = 'Público';
$string['publicationtype:internal'] = 'Interno';
$string['publicationtype:both'] = 'Público e interno';

// =============================================================================
// MISCELÁNEOS - ADICIONAL
// =============================================================================

$string['user'] = 'Usuario';
$string['useridentifier'] = 'Identificador de usuario';
$string['workflowsettings'] = 'Configuración del flujo de trabajo';
$string['column'] = 'Columna';
$string['doctype_isrequired_help'] = 'Si está marcado, este tipo de documento será obligatorio para las postulaciones';

// =============================================================================
// CADENAS DEL FORMULARIO DE REGISTRO
// =============================================================================

$string['signup_title'] = 'Crear su cuenta';
$string['signup_intro'] = 'Complete el formulario a continuación para crear su cuenta y comenzar a postularse.';
$string['signup_account_header'] = 'Información de la cuenta';
$string['signup_personalinfo'] = 'Información personal';
$string['signup_contactinfo'] = 'Información de contacto';
$string['signup_academic_header'] = 'Información académica';
$string['signup_professional_profile'] = 'Perfil profesional';
$string['signup_termsheader'] = 'Términos y condiciones';
$string['signup_companyinfo'] = 'Información de la sede';
$string['signup_company_help'] = 'Seleccione la sede a la que pertenece';
$string['signup_required_fields'] = 'Los campos marcados con * son obligatorios';
$string['signup_username_is_idnumber'] = 'Su nombre de usuario será su número de identificación';
$string['signup_doctype'] = 'Tipo de documento de identidad';
$string['signup_doctype_cc'] = 'Cédula de ciudadanía (CC)';
$string['signup_doctype_ce'] = 'Cédula de extranjería (CE)';
$string['signup_doctype_passport'] = 'Pasaporte';
$string['signup_doctype_pep'] = 'Permiso especial de permanencia (PEP)';
$string['signup_doctype_ppt'] = 'Permiso de protección temporal (PPT)';
$string['signup_idnumber'] = 'Número de identificación';
$string['signup_idnumber_exists'] = 'Este número de identificación ya está registrado';
$string['signup_idnumber_exists_as_user'] = 'Este número de identificación ya está registrado como usuario';
$string['signup_idnumber_tooshort'] = 'El número de identificación es muy corto';
$string['signup_birthdate'] = 'Fecha de nacimiento';
$string['signup_birthdate_minage'] = 'Debe tener al menos 18 años';
$string['signup_gender'] = 'Género';
$string['signup_gender_male'] = 'Masculino';
$string['signup_gender_female'] = 'Femenino';
$string['signup_gender_other'] = 'Otro';
$string['signup_gender_prefer_not'] = 'Prefiero no decir';
$string['signup_phone_mobile'] = 'Teléfono móvil';
$string['signup_phone_home'] = 'Teléfono fijo';
$string['signup_department_region'] = 'Departamento/Región';
$string['signup_education_level'] = 'Nivel de educación';
$string['signup_edu_highschool'] = 'Bachillerato';
$string['signup_edu_technical'] = 'Técnico';
$string['signup_edu_tecnico'] = 'Título técnico';
$string['signup_edu_technological'] = 'Tecnológico';
$string['signup_edu_tecnologo'] = 'Título tecnológico';
$string['signup_edu_undergraduate'] = 'Pregrado';
$string['signup_edu_profesional'] = 'Título profesional';
$string['signup_edu_specialization'] = 'Especialización';
$string['signup_edu_especialista'] = 'Título de especialista';
$string['signup_edu_masters'] = 'Maestría';
$string['signup_edu_magister'] = 'Título de magíster';
$string['signup_edu_doctorate'] = 'Doctorado';
$string['signup_edu_doctor'] = 'Título de doctor';
$string['signup_edu_postdoctorate'] = 'Posdoctorado';
$string['signup_degree_title'] = 'Título obtenido';
$string['signup_expertise_area'] = 'Área de especialización';
$string['signup_experience_years'] = 'Años de experiencia';
$string['signup_exp_none'] = 'Sin experiencia';
$string['signup_exp_less_1'] = 'Menos de 1 año';
$string['signup_exp_1_3'] = '1-3 años';
$string['signup_exp_3_5'] = '3-5 años';
$string['signup_exp_5_10'] = '5-10 años';
$string['signup_exp_more_10'] = 'Más de 10 años';
$string['signup_terms_accept'] = 'Acepto los términos y condiciones';
$string['signup_terms_required'] = 'Debe aceptar los términos y condiciones';
$string['signup_datatreatment_accept'] = 'Acepto la política de tratamiento de datos';
$string['signup_datatreatment_required'] = 'Debe aceptar la política de tratamiento de datos';
$string['signup_dataaccuracy_accept'] = 'Certifico que la información proporcionada es veraz';
$string['signup_dataaccuracy_required'] = 'Debe certificar la veracidad de la información';
$string['signup_privacy_text'] = 'Sus datos personales serán tratados según nuestra política de privacidad';
$string['signup_createaccount'] = 'Crear cuenta';
$string['signup_already_account'] = '¿Ya tiene una cuenta?';
$string['signup_applying_for'] = 'Postulándose a';
$string['signup_success_title'] = 'Registro exitoso';
$string['signup_success_message'] = 'Su cuenta ha sido creada exitosamente. Por favor revise su correo electrónico para confirmar.';
$string['signup_error_creating'] = 'Error al crear la cuenta';
$string['signup_check_spam'] = 'Revise su carpeta de spam si no recibe el correo';
$string['signup_email_instructions_title'] = 'Verificación de correo electrónico';
$string['signup_email_instruction_1'] = 'Revise su bandeja de entrada';
$string['signup_email_instruction_2'] = 'Haga clic en el enlace de verificación';
$string['signup_email_instruction_3'] = 'Complete su perfil';
$string['signup_help'] = '¿Necesita ayuda? Contacte a nuestro equipo de soporte.';
$string['signup_benefits_title'] = '¿Por qué registrarse?';
$string['signup_benefit_1'] = 'Postúlese a múltiples vacantes con un solo perfil';
$string['signup_benefit_2'] = 'Siga el estado de sus postulaciones en tiempo real';
$string['signup_benefit_3'] = 'Reciba notificaciones sobre nuevas oportunidades';
$string['signup_benefit_4'] = 'Gestione sus documentos en un solo lugar';
$string['signup_security_title'] = 'Sus datos están seguros';
$string['signup_security_text'] = 'Su información personal está protegida y solo será utilizada para el proceso de selección.';
$string['alreadyregistered'] = '¿Ya está registrado?';
$string['logintosubmit'] = 'Inicie sesión para enviar su postulación.';
$string['signup_idnumber_username'] = 'Número de identificación como usuario';
$string['signup_idnumber_username_help'] = 'Su número de identificación se utilizará como su nombre de usuario para iniciar sesión. Esto garantiza un nombre de usuario único y fácil de recordar.';
$string['companyid'] = 'Sede';
$string['companyid_help'] = 'Seleccione la sede o institución a la que desea postularse. Esto determina qué vacantes estarán disponibles para usted.';
$string['departmentid'] = 'Departamento';
$string['departmentid_help'] = 'Seleccione el departamento o campus dentro de la sede seleccionada. Esto ayuda a enrutar su postulación a los revisores apropiados.';

// =============================================================================
// CADENAS DEL PANEL DE CONTROL
// =============================================================================

$string['admindashboard'] = 'Panel de administrador';
$string['managerdashboard'] = 'Panel de gestor';
$string['companydashboard'] = 'Panel de sede';
$string['applicantdashboarddesc'] = 'Ver sus postulaciones y seguir su progreso';
$string['adminstatistics'] = 'Estadísticas de administrador';
$string['applicantstatistics'] = 'Estadísticas de postulante';
$string['convocatoriastatistics'] = 'Estadísticas de convocatorias';
$string['vacancystatistics'] = 'Estadísticas de vacantes';
$string['reviewerperformance'] = 'Desempeño de revisores';
$string['applicationstats'] = 'Estadísticas de postulaciones';
$string['documentstats'] = 'Estadísticas de documentos';
$string['systemhealth'] = 'Estado del sistema';
$string['systemconfiguration'] = 'Configuración del sistema';
$string['quicklinks'] = 'Enlaces rápidos';
$string['quicktips'] = 'Consejos rápidos';
$string['recentsnapshots'] = 'Capturas recientes';
$string['recentvacancies'] = 'Vacantes recientes';
$string['activecommittees'] = 'Comités activos';
$string['activeassignments'] = 'Asignaciones activas';
$string['activeexemptions'] = 'Exenciones activas';
$string['activereviewers'] = 'Revisores activos';
$string['totalcommittees'] = 'Total de comités';
$string['totalcommmembers'] = 'Total de miembros de comité';
$string['totalexemptions'] = 'Total de exenciones';
$string['totalusers'] = 'Total de usuarios';
$string['totalassigned'] = 'Total asignados';
$string['totalassignedusers'] = 'Total de usuarios asignados';
$string['avgtime'] = 'Tiempo promedio';
$string['avgworkload'] = 'Carga de trabajo promedio';
$string['currentworkload'] = 'Carga de trabajo actual';
$string['trend'] = 'Tendencia';
$string['trending_up'] = 'Tendencia al alza';
$string['trending_down'] = 'Tendencia a la baja';

// =============================================================================
// MARCADORES DE POSICIÓN DE PLANTILLAS DE CORREO
// =============================================================================

$string['availableplaceholders'] = 'Marcadores de posición disponibles';
$string['placeholder'] = 'Marcador de posición';
$string['ph_user_fullname'] = 'Nombre completo del usuario';
$string['ph_user_firstname'] = 'Nombre del usuario';
$string['ph_user_lastname'] = 'Apellido del usuario';
$string['ph_user_email'] = 'Correo del usuario';
$string['ph_applicant_name'] = 'Nombre del postulante';
$string['ph_vacancy_code'] = 'Código de vacante';
$string['ph_vacancy_title'] = 'Título de vacante';
$string['ph_vacancy_description'] = 'Descripción de vacante';
$string['ph_vacancy_url'] = 'URL de vacante';
$string['ph_application_id'] = 'ID de postulación';
$string['ph_application_url'] = 'URL de postulación';
$string['ph_submit_date'] = 'Fecha de envío';
$string['ph_current_date'] = 'Fecha actual';
$string['ph_deadline'] = 'Fecha límite';
$string['ph_days_remaining'] = 'Días restantes';
$string['ph_hours_until'] = 'Horas hasta';
$string['ph_close_date'] = 'Fecha de cierre';
$string['ph_open_date'] = 'Fecha de apertura';
$string['ph_company_name'] = 'Nombre de sede';
$string['ph_faculty_name'] = 'Nombre de facultad';
$string['ph_contact_info'] = 'Información de contacto';
$string['ph_site_name'] = 'Nombre del sitio';
$string['ph_site_url'] = 'URL del sitio';
$string['ph_interview_date'] = 'Fecha de entrevista';
$string['ph_interview_time'] = 'Hora de entrevista';
$string['ph_interview_location'] = 'Lugar de entrevista';
$string['ph_interview_type'] = 'Tipo de entrevista';
$string['ph_interview_duration'] = 'Duración de entrevista';
$string['ph_interview_notes'] = 'Notas de entrevista';
$string['ph_interview_feedback'] = 'Retroalimentación de entrevista';
$string['ph_interviewer_name'] = 'Nombre del entrevistador';
$string['ph_reviewer_name'] = 'Nombre del revisor';
$string['ph_feedback'] = 'Retroalimentación';
$string['ph_observations'] = 'Observaciones';
$string['ph_next_steps'] = 'Próximos pasos';
$string['ph_action_required'] = 'Acción requerida';
$string['ph_rejection_reason'] = 'Razón de rechazo';
$string['ph_rejected_docs'] = 'Documentos rechazados';
$string['ph_rejected_count'] = 'Cantidad de rechazados';
$string['ph_approved_count'] = 'Cantidad de aprobados';
$string['ph_documents_count'] = 'Cantidad de documentos';
$string['ph_review_summary'] = 'Resumen de revisión';
$string['ph_selection_notes'] = 'Notas de selección';
$string['ph_notification_note'] = 'Nota de notificación';
$string['ph_resubmit_deadline'] = 'Fecha límite de reenvío';
$string['ph_waitlist_position'] = 'Posición en lista de espera';

// =============================================================================
// CADENAS DE UI Y NAVEGACIÓN
// =============================================================================

$string['navigation'] = 'Navegación';
$string['breadcrumb'] = 'Ruta de navegación';
$string['pagination'] = 'Paginación';
$string['filterform'] = 'Formulario de filtros';
$string['filters'] = 'Filtros';
$string['applyfilters'] = 'Aplicar filtros';
$string['clearfilters'] = 'Limpiar filtros';
$string['resetfilters'] = 'Restablecer filtros';
$string['searchplaceholder'] = 'Buscar...';
$string['searchjobs'] = 'Buscar empleos';
$string['searchuser'] = 'Buscar usuario';
$string['searchusers'] = 'Buscar usuarios';
$string['searchusersplaceholder'] = 'Buscar por nombre o correo...';
$string['searchapplicant'] = 'Buscar postulante';
$string['searchbyusername'] = 'Buscar por nombre de usuario';
$string['searchagain'] = 'Buscar de nuevo';
$string['searchresultsfor'] = 'Resultados de búsqueda para';
$string['showingresults'] = 'Mostrando resultados';
$string['noresultsforsearch'] = 'No se encontraron resultados para su búsqueda';
$string['trydifferentsearch'] = 'Intente con un término de búsqueda diferente';
$string['trydifferentfilters'] = 'Intente con filtros diferentes';
$string['sortby:newest'] = 'Más recientes primero';
$string['sortby:closedate'] = 'Fecha de cierre';
$string['sortby:title'] = 'Título';
$string['sortby:positions'] = 'Posiciones';
$string['ascending'] = 'Ascendente';
$string['descending'] = 'Descendente';
$string['allcategories'] = 'Todas las categorías';
$string['alllocations'] = 'Todas las ubicaciones';
$string['filter_code_placeholder'] = 'Filtrar por código...';
$string['filter_search_placeholder'] = 'Buscar por nombre, correo...';
$string['selectaction'] = 'Seleccionar acción';
$string['selectuser'] = 'Seleccionar usuario';
$string['selectusers'] = 'Seleccionar usuarios';
$string['selecttype'] = 'Seleccionar tipo';
$string['selectreason'] = 'Seleccionar razón';
$string['selectfaculty'] = 'Seleccionar facultad';
$string['selectroletoassign'] = 'Seleccionar rol a asignar';
$string['selectmultiplehelp'] = 'Mantenga presionado Ctrl/Cmd para seleccionar varios';

// =============================================================================
// CADENAS DEL FLUJO DE POSTULACIÓN
// =============================================================================

$string['applicationid'] = 'ID de postulación';
$string['applicationguidelines'] = 'Guía de postulación';
$string['applicationsubmitteddesc'] = 'Su postulación ha sido enviada exitosamente';
$string['applicationsqueue'] = 'Cola de postulaciones';
$string['applicationsbystatus'] = 'Postulaciones por estado';
$string['applicationsbyvacancy'] = 'Postulaciones por vacante';
$string['applicationstopreview'] = 'Postulaciones para revisar';
$string['applied'] = 'Postulado';
$string['applyfor'] = 'Postularse a';
$string['applyto'] = 'Postularse a';
$string['applynow_desc'] = 'Envíe su postulación ahora';
$string['applynowdesc'] = 'Envíe su postulación ahora';
$string['applyhelp_text'] = 'Siga los pasos para completar su postulación';
$string['readytoapply'] = 'Listo para postularse';
$string['wanttoapply'] = 'Quiero postularme';
$string['createaccounttoapply'] = 'Cree una cuenta para postularse';
$string['logintoapply'] = 'Inicie sesión para postularse';
$string['loginandapply'] = 'Iniciar sesión y postularse';
$string['submitted'] = 'Enviada';
$string['reviewing'] = 'En revisión';
$string['assigned'] = 'Asignado';
$string['currentstatus'] = 'Estado actual';
$string['statushistory'] = 'Historial de estados';
$string['updatestatus'] = 'Actualizar estado';
$string['changestatus'] = 'Cambiar estado';
$string['submitreview'] = 'Enviar revisión';
$string['nextsteps'] = 'Próximos pasos';
$string['nextapplication'] = 'Siguiente postulación';
$string['previousapplication'] = 'Postulación anterior';
$string['viewmyapplication'] = 'Ver mi postulación';
$string['withdraw'] = 'Retirar';
$string['resubmit'] = 'Reenviar';

// =============================================================================
// CADENAS DE VALIDACIÓN DE DOCUMENTOS
// =============================================================================

$string['documentchecklist'] = 'Lista de verificación de documentos';
$string['documentinfo'] = 'Información del documento';
$string['documentpreview'] = 'Vista previa del documento';
$string['previewdocument'] = 'Ver vista previa del documento';
$string['documentactions'] = 'Acciones del documento';
$string['documentstoreview'] = 'Documentos por revisar';
$string['documentsremaining'] = 'Documentos restantes';
$string['documenttypes'] = 'Tipos de documentos';
$string['requireddocument'] = 'Documento requerido';
$string['uploadfile'] = 'Cargar archivo';
$string['uploadform'] = 'Formulario de carga';
$string['uploaded'] = 'Cargado';
$string['uploaddocsreminder'] = 'Recuerde cargar todos los documentos requeridos';
$string['docsawaitingreview'] = 'Sus documentos han sido cargados y están en espera de revisión';
$string['downloadtoview'] = 'Descargar para ver';
$string['validate'] = 'Validar';
$string['validateall'] = 'Validar todos';
$string['validateindividual'] = 'Validar individual';
$string['validateddate'] = 'Fecha de validación';
$string['validation'] = 'Validación';
$string['validationdecision'] = 'Decisión de validación';
$string['validationsummary'] = 'Resumen de validación';
$string['nochecklist'] = 'No hay lista de verificación disponible';
$string['nodocumentspending'] = 'No hay documentos pendientes';
$string['nodocumentstoreview'] = 'No hay documentos por revisar';
$string['nodocumentcontent'] = 'Sin contenido de documento';
$string['nodocumentcontent_desc'] = 'Este documento no tiene contenido para mostrar. El postulante aún no ha cargado ningún contenido.';
$string['nodocumentfile'] = 'Archivo de documento no disponible';
$string['nodocumentfile_desc'] = 'El archivo de este documento no está disponible. Por favor contacte al postulante para que vuelva a cargar el documento.';
$string['alldocsreviewed'] = 'Todos los documentos revisados';
$string['allvalidated'] = 'Todos validados';
$string['approveselected'] = 'Aprobar seleccionados';
$string['rejectselected'] = 'Rechazar seleccionados';
$string['rejectreason'] = 'Razón de rechazo';
$string['rejectreason_placeholder'] = 'Ingrese la razón del rechazo...';
$string['rejectreason_wrongtype'] = 'Tipo de documento incorrecto';
$string['rejectreason_help'] = 'Indique el motivo por el cual se rechaza el documento';
$string['unknowndoctype'] = 'Tipo de documento desconocido';

// Document observations (for reviewer feedback per document)
$string['documentobservation'] = 'Observaciones del documento';
$string['documentobservation_placeholder'] = 'Ingrese observaciones sobre este documento...';
$string['documentobservation_help'] = 'Las observaciones se compilarán en un email para el postulante en caso de correcciones';
$string['required_for_rejection'] = 'Obligatorio para rechazar';
$string['observation_required_for_rejection'] = 'La observación es obligatoria para rechazar el documento';
$string['observation_required_title'] = 'Observación requerida';
$string['compiledocservations'] = 'Compilar observaciones';
$string['sendobservationsemail'] = 'Enviar observaciones por email';
$string['observationsemailsent'] = 'Email de observaciones enviado correctamente';
$string['noobservationstosend'] = 'No hay observaciones para enviar';
$string['saving'] = 'Guardando...';
$string['sending'] = 'Enviando...';
$string['saveandsend'] = 'Guardar y Enviar';
$string['sent'] = '¡Enviado!';
$string['emailerror'] = 'Error de email';
$string['downloadzip'] = 'Descargar ZIP';
$string['downloadallzip'] = 'Descargar todos los documentos como ZIP';

// Cadenas de revisión secuencial
$string['sequentialreview'] = 'Revisión Secuencial';
$string['reviewingdocument'] = 'Revisando documento';
$string['of'] = 'de';
$string['current'] = 'Actual';
$string['locked'] = 'Bloqueado';
$string['admin_review_mode'] = 'Modo administrador: Puede revisar cualquier documento sin restricciones de orden ni fechas';
$string['dean_review_mode'] = 'Modo Decano: Puede ver todos los documentos pero solo aprobar o rechazar la postulación completa, no documentos individuales';

// Cadenas de aplicación y consentimiento
$string['consentgiven'] = 'Consentimiento otorgado';
$string['selectstatus'] = 'Seleccionar estado';
$string['optionalnotes'] = 'Notas opcionales';
$string['optionalnotes_placeholder'] = 'Ingrese notas opcionales...';

// =============================================================================
// CADENAS DE REVISIÓN Y COMITÉS
// =============================================================================

$string['reviewcomments'] = 'Comentarios de revisión';
$string['reviewdecision'] = 'Decisión de revisión';
$string['reviewdocuments_desc'] = 'Revisar y validar documentos de postulantes';
$string['reviewed'] = 'Revisado';
$string['reviewedby'] = 'Revisado por';
$string['reviewertasks'] = 'Tareas del revisor';
$string['deantasks'] = 'Tareas del decano';
$string['deansection_title'] = 'Panel de Revisión del Decano';
$string['deansection_desc'] = 'Revisar y aprobar postulaciones de sus facultades asignadas';
$string['assignedfaculties'] = 'Facultades asignadas';
$string['myfaculties'] = 'Mis facultades';
$string['nofacultiesassigned'] = 'Sin facultades asignadas';
$string['nofacultiesassigned_desc'] = 'Aún no se le han asignado facultades. Por favor, contacte al administrador para solicitar la asignación de facultades.';
$string['viewfacultyassignments'] = 'Ver asignaciones de facultades';
$string['pendingdeanreviews'] = 'Revisiones de decano pendientes';
$string['completeddeanreviews'] = 'Revisiones de decano completadas';
$string['reviewobservations'] = 'Observaciones de revisión';
$string['reviewobservations_placeholder'] = 'Ingrese sus observaciones...';
$string['reviewprogress'] = 'Progreso de revisión';
$string['reviewstatistics'] = 'Estadísticas de revisión';
$string['myreviews_desc'] = 'Ver sus revisiones asignadas';
$string['addreviewer'] = 'Agregar revisor';
$string['addreviewerstoprogram'] = 'Agregar revisores al programa';
$string['currentreviewers'] = 'Revisores actuales';
$string['assignedusers'] = 'Usuarios asignados';
$string['assignnewusers'] = 'Asignar nuevos usuarios';
$string['assignselected'] = 'Asignar seleccionados';
$string['assignto'] = 'Asignar a';
$string['autoassignall'] = 'Auto-asignar todos';
$string['autoassignhelp'] = 'Distribuir postulaciones automáticamente entre revisores';
$string['manualassign'] = 'Asignación manual';
$string['maxperreviewer'] = 'Máximo por revisor';
$string['noreviewers'] = 'Sin revisores';
$string['noreviewersavailable'] = 'No hay revisores disponibles';
$string['leadreviewers'] = 'Revisores líderes';
$string['committeename'] = 'Nombre del comité';
$string['committeeautoroleassign'] = 'Auto-asignar roles del comité';
$string['members'] = 'Miembros';
$string['membercount'] = 'Cantidad de miembros';
$string['managemembers'] = 'Gestionar miembros';
$string['nomembers'] = 'Sin miembros';
$string['nocommitteeforthisvacancy'] = 'No hay comité para esta vacante';
$string['existingvacancycommittee'] = 'Comité existente de vacante';
$string['legacyvacancycommittee'] = 'Comité heredado de vacante';
$string['chairhelp'] = 'El presidente lidera el comité';
$string['evaluatorshelp'] = 'Los evaluadores evalúan a los candidatos';
$string['confirmremovemember'] = '¿Eliminar este miembro del comité?';
$string['confirmremovereviewer'] = '¿Eliminar este revisor?';
$string['confirmunassign'] = '¿Desasignar este usuario?';

// =============================================================================
// CADENAS DE ENTREVISTAS
// =============================================================================

$string['scheduledinterviews'] = 'Entrevistas programadas';
$string['schedulenewinterview'] = 'Programar nueva entrevista';
$string['pendinginterviews'] = 'Entrevistas pendientes';
$string['noupcominginterviews'] = 'No hay próximas entrevistas';
$string['virtualinterview'] = 'Entrevista virtual';
$string['confirmnoshow'] = 'Confirmar no presentado';
$string['noshow'] = 'No presentado';

// =============================================================================
// CADENAS DE ROLES
// =============================================================================

$string['role_dean'] = 'Decano Revisor';
$string['role_dean_desc'] = 'Revisa perfiles de postulantes y aprueba/rechaza perfiles completos';
$string['role_hr'] = 'Talento Humano';
$string['role_hr_desc'] = 'Valida documentos de postulantes preseleccionados por el Decano';
$string['role_coordinator_desc'] = 'Coordina el proceso de reclutamiento';
$string['role_reviewer_desc'] = 'Revisa postulaciones y documentos';
$string['role_lead_reviewer'] = 'Revisor líder';
$string['role_observer'] = 'Observador';
$string['changerole'] = 'Cambiar rol';
$string['rolenotcreated'] = 'Rol no creado';
$string['backtorolelist'] = 'Volver a la lista de roles';

// =============================================================================
// CADENAS DE EXENCIONES
// =============================================================================

$string['exemptiontype_desc'] = 'Tipo de exención';
$string['exemptionactive'] = 'Exención activa';
$string['exemptionlist'] = 'Lista de exenciones';
$string['expiredexemptions'] = 'Exenciones vencidas';
$string['revokedexemptions'] = 'Exenciones revocadas';
$string['grantedby'] = 'Otorgada por';
$string['expiry'] = 'Vencimiento';

// =============================================================================
// CADENAS DE REPORTES
// =============================================================================

$string['report:applications'] = 'Reporte de postulaciones';
$string['report:vacancies'] = 'Reporte de vacantes';
$string['report:documents'] = 'Reporte de documentos';
$string['report:convocatorias'] = 'Reporte de convocatorias';
$string['reporttypes'] = 'Tipos de reportes';
$string['reportsanddata'] = 'Reportes y datos';
$string['filterreport'] = 'Filtrar reporte';
$string['exportas'] = 'Exportar como';
$string['exportoptions'] = 'Opciones de exportación';
$string['generatedby'] = 'Generado por';
$string['generated'] = 'Generado';

// =============================================================================
// CADENAS DE VISUALIZACIÓN DE VACANTES
// =============================================================================

$string['vacanciesavailable'] = 'Vacantes disponibles';
$string['vacanciesforconvocatoria'] = 'Vacantes para la convocatoria';
$string['vacancysummary'] = 'Resumen de vacante';
$string['convocatoriavacancycount'] = 'Vacantes en la convocatoria';
$string['availablepositions'] = 'Posiciones disponibles';
$string['daysleft'] = 'Días restantes';
$string['deadlineprogress'] = 'Progreso de fecha límite';
$string['deadlinewarning_title'] = 'Advertencia de fecha límite';
$string['closes'] = 'Cierra';
$string['starts'] = 'Inicia';
$string['ends'] = 'Termina';
$string['viewallvacancies'] = 'Ver todas las vacantes';
$string['viewvacancies'] = 'Ver vacantes';
$string['viewvacancydetails'] = 'Ver detalles de vacante';
$string['backtovacancy'] = 'Volver a la vacante';
$string['novacanciesfound'] = 'No se encontraron vacantes';
$string['novacanciesyet'] = 'Aún no hay vacantes';
$string['createfirstvacancy'] = 'Cree su primera vacante';
$string['suggestedvacancies'] = 'Vacantes sugeridas';
$string['nosuggestedvacancies'] = 'No hay vacantes sugeridas';
$string['featuredvacancies'] = 'Vacantes destacadas';
$string['nofeaturedvacancies'] = 'No hay vacantes destacadas';
$string['companyvacancies'] = 'Vacantes de la sede';
$string['nocompanvacancies'] = 'No hay vacantes de la sede';
$string['facultyvacancies'] = 'Vacantes de facultad';
$string['facultieswithoutcommittee'] = 'Facultades sin comité';

// =============================================================================
// CADENAS DE CONVOCATORIAS
// =============================================================================

$string['noactiveconvocatorias'] = 'No hay convocatorias activas';
$string['noconvocatorias_desc'] = 'No se encontraron convocatorias';
$string['noconvocatoriasdesc'] = 'No se encontraron convocatorias';

// =============================================================================
// CADENAS DE PROGRESO Y ESTADÍSTICAS
// =============================================================================

$string['progress'] = 'Progreso';
$string['progressindicator'] = 'Indicador de progreso';
$string['complete'] = 'Completado';
$string['result'] = 'Resultado';
$string['results'] = 'Resultados';
$string['found'] = 'Encontrados';
$string['rating'] = 'Calificación';
$string['performance'] = 'Desempeño';
$string['performedby'] = 'Realizado por';
$string['nostatsavailable'] = 'No hay estadísticas disponibles';
$string['dailyapplications'] = 'Postulaciones diarias';
$string['pendingvalidation'] = 'Validación pendiente';
$string['pendingassignment'] = 'Asignación pendiente';
$string['pendingbytype'] = 'Pendientes por tipo';
$string['bydocumenttype'] = 'Por tipo de documento';
$string['unassignedapplications'] = 'Postulaciones sin asignar';
$string['nounassignedapplications'] = 'No hay postulaciones sin asignar';

// =============================================================================
// CADENAS DE GESTIÓN DE USUARIOS
// =============================================================================

$string['manageall'] = 'Gestionar todos';
$string['manageusers'] = 'Gestionar usuarios';
$string['usersassigned'] = 'Usuarios asignados';
$string['usersassignedcount'] = 'Cantidad de usuarios asignados';
$string['nousersassigned'] = 'No hay usuarios asignados';
$string['nousersavailable'] = 'No hay usuarios disponibles';
$string['userunassigned'] = 'Usuario desasignado';
$string['usernotfound'] = 'Usuario no encontrado';
$string['username_differs_idnumber'] = 'El nombre de usuario difiere del número de identificación';
$string['username_matches_idnumber'] = 'El nombre de usuario coincide con su número de identificación';
$string['username_updated'] = 'Nombre de usuario actualizado';

// =============================================================================
// CADENAS DE TABLAS Y LISTAS
// =============================================================================

$string['datatable'] = 'Tabla de datos';
$string['thcode'] = 'Código';
$string['thtitle'] = 'Título';
$string['thstatus'] = 'Estado';
$string['thactions'] = 'Acciones';
$string['statustabs'] = 'Pestañas de estado';
$string['sortby'] = 'Ordenar por';

// =============================================================================
// CADENAS DE LÍNEA DE TIEMPO E HISTORIAL
// =============================================================================

$string['timeline'] = 'Línea de tiempo';
$string['notimeline'] = 'No hay línea de tiempo disponible';
$string['nohistory'] = 'Sin historial';
$string['noactivity'] = 'Sin actividad';
$string['nocomments'] = 'Sin comentarios';
$string['addcomment'] = 'Agregar comentario';
$string['additionalnotes'] = 'Notas adicionales';
$string['optionalcomment'] = 'Comentario opcional';
$string['optionalnotes'] = 'Notas opcionales';
$string['lastmodified'] = 'Última modificación';
$string['timeago:justnow'] = 'hace un momento';
$string['timeago:minutes'] = 'hace {$a} minutos';
$string['timeago:hours'] = 'hace {$a} horas';
$string['timeago:days'] = 'hace {$a} días';

// =============================================================================
// CADENAS PARA COMPARTIR
// =============================================================================

$string['share'] = 'Compartir';
$string['sharepage'] = 'Compartir esta página';
$string['sharebyemail'] = 'Compartir por correo';
$string['shareonfacebook'] = 'Compartir en Facebook';
$string['shareontwitter'] = 'Compartir en Twitter';
$string['shareonlinkedin'] = 'Compartir en LinkedIn';
$string['shareonwhatsapp'] = 'Compartir en WhatsApp';

// =============================================================================
// CADENAS DE ALERTAS DE EMPLEO
// =============================================================================

$string['jobalerts'] = 'Alertas de empleo';
$string['jobalertsdesc'] = 'Suscríbase para recibir alertas de nuevas vacantes';
$string['subscribe'] = 'Suscribirse';

// =============================================================================
// CADENAS MISCELÁNEAS DE UI
// =============================================================================

$string['draft'] = 'Borrador';
$string['private'] = 'Privado';
$string['markprivate'] = 'Marcar como privado';
$string['entity'] = 'Entidad';
$string['companies'] = 'Sedes';
$string['departments'] = 'Departamentos';
$string['filename'] = 'Nombre de archivo';
$string['issuedate'] = 'Fecha de expedición';
$string['issuedatehelp'] = 'Fecha en que se expidió el documento';
$string['choosefiles'] = 'Elegir archivos';
$string['clickfordetails'] = 'Clic para ver detalles';
$string['opennewtab'] = 'Abrir en nueva pestaña';
$string['opensnewwindow'] = 'Abre en nueva ventana';
$string['togglepreview'] = 'Alternar vista previa';
$string['vieweronly_title'] = 'Acceso de Visualización';
$string['vieweronly_desc'] = 'Tiene acceso de solo visualización a la Bolsa de Empleo. Explore las vacantes disponibles en la página pública.';
$string['viewpublicpage'] = 'Ver página pública';
$string['viewpublicvacancies'] = 'Ver vacantes públicas';
$string['viewmyreviews'] = 'Ver mis revisiones';
$string['gotodashboard'] = 'Ir al panel';
$string['needhelp'] = '¿Necesita ayuda?';
$string['interestedinposition'] = '¿Interesado en esta posición?';
$string['welcome'] = 'Bienvenido';
$string['welcometojobboard'] = 'Bienvenido a Bolsa de Empleo';
$string['landingdescription'] = 'Encuentre su próxima oportunidad';
$string['findyournextjob'] = 'Encuentre su próximo empleo';
$string['browsealljobs'] = 'Explorar todos los empleos';
$string['browsebycategory'] = 'Explorar por categoría';
$string['administracion'] = 'Administración';
$string['iomadintegration'] = 'Integración IOMAD';
$string['workflowconfiguration'] = 'Configuración del flujo de trabajo';
$string['workflowmanagement'] = 'Gestión del flujo de trabajo';
$string['workflowactions'] = 'Acciones del flujo de trabajo';
$string['capabilities'] = 'Capacidades';
$string['bulkactions'] = 'Acciones masivas';
$string['bulkoperations'] = 'Operaciones masivas';
$string['bulkcomment'] = 'Comentario masivo';
$string['confirmcancel'] = 'Confirmar cancelación';
$string['confirmdelete'] = 'Confirmar eliminación';
$string['conversionwait'] = 'Por favor espere mientras se convierte el documento';
$string['nextpossible'] = 'Próximo posible';
$string['finalstage'] = 'Etapa final';
$string['needsattention'] = 'Requiere atención';
$string['norejections'] = 'Sin rechazos';
$string['noassignments'] = 'Sin asignaciones';
$string['noassignments_desc'] = 'No se encontraron asignaciones';
$string['noauditlogs'] = 'Sin registros de auditoría';
$string['nopendingapplications'] = 'No hay postulaciones pendientes';
$string['nopendingdocuments'] = 'No hay documentos pendientes';
$string['noapplicationsdesc'] = 'No se encontraron postulaciones';
$string['noapplicationsyet'] = 'Aún no hay postulaciones';
$string['nosecretaryoptional'] = 'El secretario es opcional';
$string['myrecentapplications'] = 'Mis postulaciones recientes';
$string['contract:prestacion_servicios'] = 'Contrato de prestación de servicios';
$string['contract:termino_fijo'] = 'Contrato a término fijo';
$string['doctype_rnmc'] = 'Registro Nacional Docente (RNMC)';
$string['doctype_titulo_postgrado'] = 'Título de posgrado';
$string['status:assigned'] = 'Asignado';

// =============================================================================
// GRADING PANEL STRINGS (mod_assign style interface)
// =============================================================================

$string['gradingpanel'] = 'Panel de revisión';
$string['applicationlist'] = 'Lista de postulaciones';
$string['applicationnavigation'] = 'Navegación de postulaciones';
$string['selectapplication'] = 'Seleccionar una postulación';
$string['selectdocument'] = 'Seleccionar un documento';
$string['selectdocumenttopreview'] = 'Seleccione un documento para previsualizar';
$string['selectdocumenthelp'] = 'Haga clic en un documento de la lista de la izquierda para previsualizarlo';
$string['previewnotavailable'] = 'Vista previa no disponible para este tipo de archivo';
$string['filterbyname'] = 'Filtrar por nombre...';
$string['togglesidebar'] = 'Mostrar/ocultar panel lateral';
$string['pendingdocs'] = 'Documentos pendientes';
$string['exitgrading'] = 'Salir del panel de revisión';
$string['exit'] = 'Salir';
$string['fullscreen'] = 'Pantalla completa';
$string['approveall'] = 'Aprobar todos';
$string['confirmapproveall'] = '¿Está seguro de que desea aprobar todos los documentos pendientes de esta postulación?';
$string['previewunavailable'] = 'Vista previa no disponible para este tipo de archivo';
$string['previewnotavailable'] = 'Vista previa no disponible';
$string['downloadtoview'] = 'Descargar para ver';
$string['withdrawwarning'] = 'Esta acción no se puede deshacer. Deberá volver a postularse si desea ser considerado para esta posición.';
$string['selectrejectreason'] = 'Seleccione motivo de rechazo...';

// Keyboard shortcuts
$string['keyboardshortcuts'] = 'Atajos de teclado';
$string['shortcut_next'] = 'Siguiente documento';
$string['shortcut_prev'] = 'Documento anterior';
$string['shortcut_approve'] = 'Aprobar documento actual';
$string['shortcut_reject'] = 'Enfocar motivo de rechazo';
$string['shortcut_download'] = 'Descargar documento';
$string['shortcut_fullscreen'] = 'Alternar pantalla completa';
$string['shortcut_sidebar'] = 'Mostrar/ocultar panel lateral';
$string['shortcut_navigate_docs'] = 'Navegar documentos';
$string['shortcut_approve_all'] = 'Aprobar todos los pendientes';
$string['shortcut_show_help'] = 'Mostrar esta ayuda';
$string['shortcut_exit'] = 'Salir del panel de revisión';

// Validation feedback
$string['document_approved'] = 'Documento aprobado exitosamente';
$string['document_rejected'] = 'Documento rechazado';
$string['validatedby'] = 'Validado por';
$string['validatedat'] = 'Validado el';

// =============================================================================
// USER TOURS - NOMBRES Y DESCRIPCIONES DE TOURS
// =============================================================================

$string['tour_endlabel'] = 'Finalizar tour';

// Tour del Dashboard
$string['tour_dashboard_name'] = 'Tour del Dashboard de Bolsa de Empleo';
$string['tour_dashboard_desc'] = 'Aprenda a navegar el dashboard de la Bolsa de Empleo';
$string['tour_dashboard_welcome_title'] = 'Bienvenido a Bolsa de Empleo';
$string['tour_dashboard_welcome_content'] = 'Este es el dashboard principal del plugin de Bolsa de Empleo. Aquí puede acceder a todas las funcionalidades según su rol.';
$string['tour_dashboard_stats_title'] = 'Resumen de Estadísticas';
$string['tour_dashboard_stats_content'] = 'Estas tarjetas muestran estadísticas clave como convocatorias activas, vacantes y postulaciones.';
$string['tour_dashboard_admin_title'] = 'Secciones de Administración';
$string['tour_dashboard_admin_content'] = 'Cada tarjeta proporciona acceso a diferentes áreas de gestión. Haga clic en cualquier tarjeta para administrar esa sección.';
$string['tour_dashboard_reviewer_title'] = 'Tareas de Revisor';
$string['tour_dashboard_reviewer_content'] = 'Como revisor, puede ver documentos pendientes y sus revisiones completadas aquí.';
$string['tour_dashboard_applicant_title'] = 'Sección del Postulante';
$string['tour_dashboard_applicant_content'] = 'Vea sus postulaciones y navegue las vacantes disponibles desde esta sección.';

// Tour de Página Pública
$string['tour_public_name'] = 'Tour de Página Pública';
$string['tour_public_desc'] = 'Descubra cómo navegar las oportunidades laborales públicas';
$string['tour_public_welcome_title'] = 'Oportunidades Laborales';
$string['tour_public_welcome_content'] = 'Bienvenido a la bolsa de empleo pública. Aquí puede navegar todas las oportunidades laborales disponibles.';
$string['tour_public_stats_title'] = 'Estadísticas Rápidas';
$string['tour_public_stats_content'] = 'Estas tarjetas muestran el número de convocatorias abiertas, vacantes y posiciones disponibles.';
$string['tour_public_convocatorias_title'] = 'Tarjetas de Convocatoria';
$string['tour_public_convocatorias_content'] = 'Cada tarjeta representa una convocatoria. Haga clic para ver detalles y vacantes disponibles.';
$string['tour_public_vacancies_title'] = 'Tarjetas de Vacantes';
$string['tour_public_vacancies_content'] = 'Navegue las posiciones disponibles. Cada tarjeta muestra información clave sobre la vacante.';
$string['tour_public_filters_title'] = 'Opciones de Filtro';
$string['tour_public_filters_content'] = 'Use estos filtros para refinar su búsqueda por ubicación, tipo de contrato o palabras clave.';
$string['tour_public_apply_title'] = 'Botón de Postulación';
$string['tour_public_apply_content'] = 'Haga clic en Postularse para iniciar su postulación. Deberá iniciar sesión o crear una cuenta primero.';

// Tour de Convocatorias
$string['tour_convocatorias_name'] = 'Tour de Gestión de Convocatorias';
$string['tour_convocatorias_desc'] = 'Aprenda a gestionar convocatorias';
$string['tour_convocatorias_header_title'] = 'Gestión de Convocatorias';
$string['tour_convocatorias_header_content'] = 'Esta página le permite gestionar todas las convocatorias.';
$string['tour_convocatorias_create_title'] = 'Crear Nueva';
$string['tour_convocatorias_create_content'] = 'Haga clic aquí para crear una nueva convocatoria. Deberá proporcionar fechas, descripción y un documento PDF.';
$string['tour_convocatorias_stats_title'] = 'Estadísticas';
$string['tour_convocatorias_stats_content'] = 'Estas tarjetas muestran el estado actual de todas las convocatorias.';
$string['tour_convocatorias_filter_title'] = 'Opciones de Filtro';
$string['tour_convocatorias_filter_content'] = 'Filtre convocatorias por estado, rango de fechas u otros criterios.';
$string['tour_convocatorias_card_title'] = 'Tarjeta de Convocatoria';
$string['tour_convocatorias_card_content'] = 'Cada tarjeta muestra los detalles de la convocatoria, incluyendo fechas, cantidad de vacantes y estadísticas de postulaciones.';
$string['tour_convocatorias_actions_title'] = 'Acciones Rápidas';
$string['tour_convocatorias_actions_content'] = 'Use estos botones para editar, ver vacantes o gestionar la convocatoria.';

// Tour de Formulario de Convocatoria
$string['tour_convocatoria_manage_name'] = 'Tour de Formulario de Convocatoria';
$string['tour_convocatoria_manage_desc'] = 'Aprenda a crear y editar convocatorias';
$string['tour_convocatoria_manage_header_title'] = 'Formulario de Convocatoria';
$string['tour_convocatoria_manage_header_content'] = 'Use este formulario para crear o editar una convocatoria.';
$string['tour_convocatoria_manage_form_title'] = 'Campos del Formulario';
$string['tour_convocatoria_manage_form_content'] = 'Complete todos los campos requeridos. El nombre, código y descripción son esenciales.';
$string['tour_convocatoria_manage_dates_title'] = 'Configuración de Fechas';
$string['tour_convocatoria_manage_dates_content'] = 'Establezca las fechas de inicio y fin de la convocatoria. Las vacantes heredan estas fechas.';
$string['tour_convocatoria_manage_pdf_title'] = 'Documento PDF';
$string['tour_convocatoria_manage_pdf_content'] = 'Cargue el documento PDF oficial con los detalles completos de la convocatoria. Este es obligatorio.';
$string['tour_convocatoria_manage_submit_title'] = 'Guardar';
$string['tour_convocatoria_manage_submit_content'] = 'Haga clic en Guardar para crear o actualizar la convocatoria.';

// Tour de Vacantes
$string['tour_vacancies_name'] = 'Tour de Lista de Vacantes';
$string['tour_vacancies_desc'] = 'Aprenda a navegar y gestionar vacantes';
$string['tour_vacancies_header_title'] = 'Vacantes';
$string['tour_vacancies_header_content'] = 'Esta página muestra todas las vacantes de la convocatoria seleccionada.';
$string['tour_vacancies_selector_title'] = 'Selector de Convocatoria';
$string['tour_vacancies_selector_content'] = 'Seleccione una convocatoria para ver sus vacantes.';
$string['tour_vacancies_card_title'] = 'Tarjeta de Vacante';
$string['tour_vacancies_card_content'] = 'Cada tarjeta muestra los detalles de la vacante incluyendo título, ubicación y número de posiciones.';
$string['tour_vacancies_status_title'] = 'Insignia de Estado';
$string['tour_vacancies_status_content'] = 'La insignia indica el estado actual: borrador, publicada o cerrada.';
$string['tour_vacancies_actions_title'] = 'Acciones';
$string['tour_vacancies_actions_content'] = 'Use estos botones para ver detalles o postularse a la vacante.';

// Tour de Detalle de Vacante
$string['tour_vacancy_name'] = 'Tour de Detalle de Vacante';
$string['tour_vacancy_desc'] = 'Conozca los detalles de la vacante';
$string['tour_vacancy_header_title'] = 'Detalles de Vacante';
$string['tour_vacancy_header_content'] = 'Esta página muestra información completa sobre la vacante.';
$string['tour_vacancy_details_title'] = 'Descripción';
$string['tour_vacancy_details_content'] = 'Lea la descripción completa, responsabilidades y requisitos del cargo.';
$string['tour_vacancy_requirements_title'] = 'Requisitos';
$string['tour_vacancy_requirements_content'] = 'Revise las calificaciones y documentos requeridos antes de postularse.';
$string['tour_vacancy_apply_title'] = 'Postularse Ahora';
$string['tour_vacancy_apply_content'] = 'Haga clic en Postularse para iniciar su proceso de postulación.';

// Tour de Gestión
$string['tour_manage_name'] = 'Tour de Gestión de Vacantes';
$string['tour_manage_desc'] = 'Aprenda a gestionar vacantes';
$string['tour_manage_header_title'] = 'Gestión de Vacantes';
$string['tour_manage_header_content'] = 'Gestione todas las vacantes desde esta página.';
$string['tour_manage_create_title'] = 'Crear Vacante';
$string['tour_manage_create_content'] = 'Haga clic aquí para crear una nueva vacante dentro de una convocatoria.';
$string['tour_manage_table_title'] = 'Tabla de Vacantes';
$string['tour_manage_table_content'] = 'Esta tabla lista todas las vacantes con sus detalles y estado.';
$string['tour_manage_bulk_title'] = 'Selección Masiva';
$string['tour_manage_bulk_content'] = 'Seleccione múltiples vacantes para realizar acciones masivas.';
$string['tour_manage_actions_title'] = 'Acciones de Fila';
$string['tour_manage_actions_content'] = 'Use estos botones para editar, ver o eliminar vacantes individuales.';

// Tour de Postulación
$string['tour_apply_name'] = 'Tour del Proceso de Postulación';
$string['tour_apply_desc'] = 'Aprenda cómo enviar una postulación';
$string['tour_apply_header_title'] = 'Postularse a Vacante';
$string['tour_apply_header_content'] = 'Complete este formulario para enviar su postulación.';
$string['tour_apply_progress_title'] = 'Pasos de Progreso';
$string['tour_apply_progress_content'] = 'Siga estos pasos para completar su postulación. Todos los pasos deben completarse.';
$string['tour_apply_guidelines_title'] = 'Instrucciones';
$string['tour_apply_guidelines_content'] = 'Lea estas instrucciones cuidadosamente antes de iniciar su postulación.';
$string['tour_apply_form_title'] = 'Formulario de Postulación';
$string['tour_apply_form_content'] = 'Complete todos los campos requeridos y cargue los documentos necesarios.';
$string['tour_apply_sidebar_title'] = 'Resumen de Vacante';
$string['tour_apply_sidebar_content'] = 'Esta barra lateral muestra los detalles de la vacante y la fecha límite.';
$string['tour_apply_checklist_title'] = 'Lista de Documentos';
$string['tour_apply_checklist_content'] = 'Asegúrese de tener todos los documentos requeridos listos antes de enviar.';
$string['tour_apply_submit_title'] = 'Enviar Postulación';
$string['tour_apply_submit_content'] = 'Haga clic en Enviar cuando haya completado todas las secciones y cargado todos los documentos.';

// Tour de Detalle de Postulación
$string['tour_application_name'] = 'Tour de Detalle de Postulación';
$string['tour_application_desc'] = 'Conozca los detalles de su postulación';
$string['tour_application_header_title'] = 'Detalles de Postulación';
$string['tour_application_header_content'] = 'Vea todos los detalles de su postulación aquí.';
$string['tour_application_status_title'] = 'Estado';
$string['tour_application_status_content'] = 'Esta insignia muestra el estado actual de su postulación.';
$string['tour_application_timeline_title'] = 'Línea de Tiempo';
$string['tour_application_timeline_content'] = 'Siga el progreso de su postulación a través de cada etapa.';
$string['tour_application_documents_title'] = 'Documentos';
$string['tour_application_documents_content'] = 'Vea y gestione sus documentos cargados aquí.';
$string['tour_application_actions_title'] = 'Acciones';
$string['tour_application_actions_content'] = 'Use estos botones para retirar o modificar su postulación (si está permitido).';

// Tour de Mis Postulaciones
$string['tour_myapplications_name'] = 'Tour de Mis Postulaciones';
$string['tour_myapplications_desc'] = 'Aprenda a seguir sus postulaciones';
$string['tour_myapplications_header_title'] = 'Mis Postulaciones';
$string['tour_myapplications_header_content'] = 'Vea todas sus postulaciones enviadas aquí.';
$string['tour_myapplications_stats_title'] = 'Estadísticas de Postulaciones';
$string['tour_myapplications_stats_content'] = 'Estos contadores muestran cuántas postulaciones tiene en cada estado.';
$string['tour_myapplications_card_title'] = 'Tarjeta de Postulación';
$string['tour_myapplications_card_content'] = 'Cada tarjeta representa una postulación con su estado actual y detalles.';
$string['tour_myapplications_status_title'] = 'Insignia de Estado';
$string['tour_myapplications_status_content'] = 'Los colores indican el estado: azul para enviada, amarillo para en revisión, verde para aprobada.';
$string['tour_myapplications_browse_title'] = 'Explorar Vacantes';
$string['tour_myapplications_browse_content'] = 'Haga clic aquí para explorar más vacantes y postularse a nuevas posiciones.';

// Tour de Documentos
$string['tour_documents_name'] = 'Tour de Gestión de Documentos';
$string['tour_documents_desc'] = 'Aprenda a gestionar sus documentos';
$string['tour_documents_header_title'] = 'Gestión de Documentos';
$string['tour_documents_header_content'] = 'Gestione todos los documentos de su postulación desde aquí.';
$string['tour_documents_list_title'] = 'Lista de Documentos';
$string['tour_documents_list_content'] = 'Cada fila muestra un tipo de documento con su estado actual.';
$string['tour_documents_status_title'] = 'Estado del Documento';
$string['tour_documents_status_content'] = 'Verde significa aprobado, amarillo significa pendiente de revisión, rojo significa rechazado.';
$string['tour_documents_upload_title'] = 'Cargar Documento';
$string['tour_documents_upload_content'] = 'Haga clic para cargar un nuevo documento o reemplazar uno existente.';
$string['tour_documents_preview_title'] = 'Vista Previa';
$string['tour_documents_preview_content'] = 'Haga clic para previsualizar su documento cargado sin descargarlo.';

// Tour de Revisión
$string['tour_review_name'] = 'Tour de Revisión de Documentos';
$string['tour_review_desc'] = 'Aprenda a revisar documentos de postulantes';
$string['tour_review_header_title'] = 'Revisión de Documentos';
$string['tour_review_header_content'] = 'Revise y valide documentos de postulantes desde esta página.';
$string['tour_review_stats_title'] = 'Estadísticas de Revisión';
$string['tour_review_stats_content'] = 'Estas tarjetas muestran conteos de documentos pendientes, aprobados y rechazados.';
$string['tour_review_navigation_title'] = 'Navegación';
$string['tour_review_navigation_content'] = 'Navegue entre postulaciones usando estos botones.';
$string['tour_review_documents_title'] = 'Lista de Documentos';
$string['tour_review_documents_content'] = 'Todos los documentos de esta postulación se listan aquí.';
$string['tour_review_actions_title'] = 'Acciones Rápidas';
$string['tour_review_actions_content'] = 'Use estos botones para aprobar, rechazar o previsualizar cada documento.';
$string['tour_review_applicant_title'] = 'Info del Postulante';
$string['tour_review_applicant_content'] = 'Vea los detalles del postulante e información de la vacante en la barra lateral.';
$string['tour_review_progress_title'] = 'Progreso de Revisión';
$string['tour_review_progress_content'] = 'Esta barra muestra cuántos documentos han sido revisados.';
$string['tour_review_submit_title'] = 'Completar Revisión';
$string['tour_review_submit_content'] = 'Haga clic aquí cuando haya revisado todos los documentos para enviar su revisión.';

// Tour de Mis Revisiones
$string['tour_myreviews_name'] = 'Tour de Mis Revisiones';
$string['tour_myreviews_desc'] = 'Aprenda a seguir sus asignaciones de revisión';
$string['tour_myreviews_header_title'] = 'Mis Revisiones';
$string['tour_myreviews_header_content'] = 'Vea todas las postulaciones asignadas a usted para revisión.';
$string['tour_myreviews_stats_title'] = 'Estadísticas de Revisión';
$string['tour_myreviews_stats_content'] = 'Estas tarjetas muestran sus revisiones pendientes y completadas.';
$string['tour_myreviews_filter_title'] = 'Filtros';
$string['tour_myreviews_filter_content'] = 'Filtre postulaciones por estado, convocatoria o fecha.';
$string['tour_myreviews_queue_title'] = 'Cola de Revisión';
$string['tour_myreviews_queue_content'] = 'Las postulaciones esperando su revisión se listan aquí.';
$string['tour_myreviews_pending_title'] = 'Conteo Pendiente';
$string['tour_myreviews_pending_content'] = 'Esta insignia muestra cuántos documentos necesitan revisión en cada postulación.';
$string['tour_myreviews_action_title'] = 'Iniciar Revisión';
$string['tour_myreviews_action_content'] = 'Haga clic para abrir el panel de revisión de documentos para esta postulación.';

// Tour de Validación de Documento
$string['tour_validate_document_name'] = 'Tour de Validación de Documento';
$string['tour_validate_document_desc'] = 'Aprenda a validar documentos individuales';
$string['tour_validate_header_title'] = 'Validación de Documento';
$string['tour_validate_header_content'] = 'Revise y valide este documento contra los requisitos.';
$string['tour_validate_preview_title'] = 'Vista Previa del Documento';
$string['tour_validate_preview_content'] = 'El documento se muestra aquí. Puede hacer zoom y desplazarse para revisarlo.';
$string['tour_validate_checklist_title'] = 'Lista de Verificación';
$string['tour_validate_checklist_content'] = 'Marque cada ítem para verificar que el documento cumple todos los requisitos.';
$string['tour_validate_approve_title'] = 'Aprobar';
$string['tour_validate_approve_content'] = 'Haga clic en Aprobar si el documento cumple todos los requisitos.';
$string['tour_validate_reject_title'] = 'Rechazar';
$string['tour_validate_reject_content'] = 'Haga clic en Rechazar si el documento tiene problemas y debe volver a enviarse.';
$string['tour_validate_reason_title'] = 'Motivo de Rechazo';
$string['tour_validate_reason_content'] = 'Si rechaza, proporcione un motivo claro para que el postulante pueda corregir el documento.';

// Tour de Reportes
$string['tour_reports_name'] = 'Tour de Reportes';
$string['tour_reports_desc'] = 'Aprenda a usar las funciones de reportes';
$string['tour_reports_header_title'] = 'Reportes';
$string['tour_reports_header_content'] = 'Genere y exporte reportes desde esta página.';
$string['tour_reports_export_title'] = 'Opciones de Exportación';
$string['tour_reports_export_content'] = 'Exporte reportes en formato CSV, Excel o PDF.';
$string['tour_reports_tabs_title'] = 'Tipos de Reporte';
$string['tour_reports_tabs_content'] = 'Elija entre diferentes tipos de reporte: resumen, postulaciones, documentos o revisores.';
$string['tour_reports_filter_title'] = 'Filtros';
$string['tour_reports_filter_content'] = 'Filtre reportes por convocatoria, rango de fechas o estado.';
$string['tour_reports_stats_title'] = 'Estadísticas Resumidas';
$string['tour_reports_stats_content'] = 'Estas tarjetas muestran métricas clave para los filtros seleccionados.';
$string['tour_reports_table_title'] = 'Tabla de Datos';
$string['tour_reports_table_content'] = 'Los datos detallados se muestran en esta tabla. Haga clic en los encabezados para ordenar.';
$string['tour_reports_progress_title'] = 'Barras de Progreso';
$string['tour_reports_progress_content'] = 'Representación visual de porcentajes y progreso.';

// =============================================================================
// CADENAS DE ACCESIBILIDAD
// =============================================================================

$string['skiptocontent'] = 'Saltar al contenido';
$string['skiptomain'] = 'Saltar al contenido principal';
$string['skiptonavigation'] = 'Saltar a la navegación';

// =============================================================================
// MARCADORES DE PLANTILLA
// =============================================================================

$string['placeholder_username'] = 'Nombre completo del usuario';
$string['placeholder_vacancytitle'] = 'Título de la vacante';
$string['placeholder_sitename'] = 'Nombre del sitio';
$string['placeholder_applicationid'] = 'ID de la postulación';
$string['placeholder_convocatorianame'] = 'Nombre de la convocatoria';
$string['placeholder_deadline'] = 'Fecha límite de postulación';

// =============================================================================
// CADENAS DE CARGA Y DOCUMENTOS
// =============================================================================

$string['uploadtips'] = 'Consejos de Carga';
$string['uploadtip1'] = 'Asegúrese de que el documento sea claro y legible';
$string['uploadtip2'] = 'El tamaño del archivo no debe exceder el máximo permitido';
$string['uploadtip3'] = 'Formatos soportados: PDF, JPG, PNG';
$string['reuploadinstructions'] = 'Instrucciones de Recarga';
$string['uploadnewversion'] = 'Cargar Nueva Versión';
$string['documentrejected'] = 'Documento Rechazado';
$string['documentinfo'] = 'Información del Documento';
$string['documentname'] = 'Nombre del Documento';
$string['documenttype'] = 'Tipo de Documento';

// =============================================================================
// CADENAS DE IMPORTACIÓN/EXPORTACIÓN
// =============================================================================

$string['importwarningtitle'] = 'Advertencia Importante';
$string['previewdata'] = 'Vista Previa de Datos';
$string['nopreviewdata'] = 'No hay datos disponibles para vista previa';
$string['previewsummary'] = 'Resumen de Vista Previa';
$string['recordstoprocess'] = 'Registros a procesar';
$string['confirmimport'] = 'Confirmar Importación';
$string['exemptionsimported'] = 'Exenciones importadas';
$string['exemptionsskipped'] = 'Exenciones omitidas';
$string['datatorexport'] = 'Datos a exportar';
$string['fullexport_info'] = 'Se incluirán todos los datos de configuración';

// =============================================================================
// CADENAS DE ASIGNACIÓN DE REVISORES
// =============================================================================

$string['unassignedapplications'] = 'Postulaciones sin asignar';
$string['availablereviewers'] = 'Revisores disponibles';
$string['totalassigned'] = 'Total asignados';
$string['avgworkload'] = 'Carga promedio';
$string['reviewerworkload'] = 'Carga de Trabajo del Revisor';
$string['activeassignments'] = 'asignaciones activas';
$string['reviewed'] = 'Revisados';
$string['avgtime'] = 'Tiempo prom.';
$string['autoassign'] = 'Asignación Automática';
$string['autoassignall'] = 'Asignar Todo Automáticamente';
$string['autoassignhelp'] = 'Distribuye automáticamente las postulaciones sin asignar de manera equitativa entre los revisores disponibles';
$string['maxperreviewer'] = 'Máximo por revisor';
$string['manualassign'] = 'Asignación Manual';
$string['pendingassignment'] = 'pendiente de asignación';
$string['assignto'] = 'Asignar a';
$string['selectreviewer'] = 'Seleccione un revisor';
$string['assignselected'] = 'Asignar Seleccionados';
$string['alldone'] = '¡Todo Listo!';
$string['nounassignedapplications'] = 'No hay postulaciones sin asignar en este momento';
$string['noreviewers'] = 'No hay revisores disponibles';
$string['allvacancies'] = 'Todas las vacantes';
$string['bulkvalidation'] = 'Validación Masiva';
$string['reviewapplications'] = 'Revisar Postulaciones';

// =============================================================================
// CADENAS DE REVISORES
// =============================================================================

$string['totalreviewers'] = 'Total de Revisores';
$string['activereviewers'] = 'Revisores Activos';
$string['addreviewer'] = 'Agregar Revisor';
$string['selectuser'] = 'Seleccionar usuario';
$string['role_reviewer'] = 'Revisor';
$string['role_lead_reviewer'] = 'Revisor Líder';
$string['nousersavailable'] = 'No hay usuarios disponibles para asignar';
$string['changerole'] = 'Cambiar rol';
$string['confirmremovereviewer'] = '¿Está seguro de que desea eliminar a este revisor?';

// =============================================================================
// CADENAS DE SELECCIÓN DE VACANTE
// =============================================================================

$string['selectconvocatoriafirst'] = 'Seleccione una Convocatoria Primero';
$string['createvacancyinconvocatoriadesc'] = 'Para crear una nueva vacante, primero debe seleccionar a qué convocatoria pertenece.';
$string['noconvocatoriasavailable'] = 'No hay convocatorias disponibles';
$string['gotocreateconvocatoria'] = 'Crear una Convocatoria';
$string['selectconvocatoria'] = 'Seleccionar Convocatoria';
$string['addvacancy'] = 'Agregar Vacante';
$string['addconvocatoria'] = 'Crear Nueva Convocatoria';
$string['or'] = 'o';

// =============================================================================
// CADENAS DE MIGRACIÓN
// =============================================================================

$string['dryrunresults'] = 'Resultados de Prueba';

// =============================================================================
// CADENAS DE ESTADO
// =============================================================================

$string['inactive'] = 'Inactivo';
$string['active'] = 'Activo';
$string['assigned'] = 'Asignado';

// =============================================================================
// CADENAS DE INTERFAZ MISCELÁNEAS
// =============================================================================

$string['pagination'] = 'Paginación';
$string['browservacancies'] = 'Explorar Vacantes';
$string['novacancies'] = 'Sin vacantes';
$string['viewvacancies'] = 'Ver Vacantes';
$string['viewconvocatoria'] = 'Ver Convocatoria';
$string['noconvocatorias'] = 'No se encontraron convocatorias';
$string['noconvocatorias_desc'] = 'No hay convocatorias disponibles en este momento. Por favor, vuelva a consultar más tarde.';
$string['convocatoria_status_open'] = 'Abierta';
$string['convocatoria_status_closed'] = 'Cerrada';
$string['daysremaining'] = 'días restantes';

// =============================================================================
// CADENAS DEL PANEL DE CONTROL
// =============================================================================

$string['myreviews_desc'] = 'Revise las postulaciones asignadas a usted';
$string['pending_reviews_alert'] = 'Tiene {$a} postulaciones pendientes de revisión';
$string['attentionrequired'] = 'Atención Requerida';
$string['nopendingreview'] = 'Sin revisiones pendientes. ¡Buen trabajo!';
$string['viewmyreviews'] = 'Ver Mis Revisiones';
$string['completedreviews'] = 'Revisiones Completadas';
$string['critical'] = 'Crítico';
$string['closingon'] = 'Cierra el';
$string['viewpublicpage'] = 'Ver Página Pública';
$string['managevacancies'] = 'Gestionar Vacantes';
$string['notifications'] = 'Notificaciones';
$string['mystats'] = 'Mis Estadísticas';
$string['reviewertasks'] = 'Tareas del Revisor';
$string['recentactivity'] = 'Actividad Reciente';
$string['statistics'] = 'Estadísticas';

// Página pública
$string['openconvocatorias'] = 'Convocatorias Abiertas';
$string['filtervacancies'] = 'Filtrar Vacantes';
$string['available'] = 'Disponible';

// Mensajes adicionales
$string['allcaughtup'] = '¡Todo al día!';

// Mensajes de error
$string['error:usernotfound'] = 'Usuario no encontrado';
$string['unknownuser'] = 'Usuario desconocido';

// =============================================================================
// CADENAS FALTANTES - COMPARTIR EN REDES SOCIALES
// =============================================================================

$string['shareon'] = 'Compartir en {$a}';
$string['shareconvocatoria'] = 'Compartir esta convocatoria';

// =============================================================================
// CADENAS FALTANTES - FORMULARIO DE REGISTRO/ACTUALIZAR PERFIL
// =============================================================================

$string['signup_email'] = 'Correo electrónico';
$string['signup_email_help'] = 'Ingrese una dirección de correo electrónico válida. Se utilizará para comunicaciones relacionadas con sus postulaciones y recuperación de cuenta.';
$string['currentpassword_help'] = 'Ingrese su contraseña actual para confirmar cualquier cambio en la configuración de su cuenta.';
$string['signup_password'] = 'Contraseña';
$string['signup_password_help'] = 'Cree una contraseña segura con al menos 8 caracteres, incluyendo mayúsculas, minúsculas y números.';
$string['signup_doctype_help'] = 'Seleccione el tipo de documento de identificación que está utilizando.';
$string['signup_birthdate_help'] = 'Ingrese su fecha de nacimiento. Esta información es requerida para algunos procesos de verificación.';
$string['signup_phone'] = 'Número de teléfono';
$string['signup_phone_help'] = 'Ingrese su número de teléfono incluyendo código de país. Se utilizará para comunicaciones urgentes.';
$string['signup_education_level_help'] = 'Seleccione su nivel de educación más alto completado.';
$string['signup_degree_title_help'] = 'Ingrese el título oficial de su grado o certificación más alto.';
$string['signup_institution'] = 'Institución educativa';
$string['signup_institution_help'] = 'Ingrese el nombre de la institución donde obtuvo su cualificación más alta.';
$string['signup_expertise_area_help'] = 'Describa sus principales áreas de experiencia y especialización profesional.';
$string['signup_professional_profile_help'] = 'Proporcione un breve resumen profesional destacando su experiencia y habilidades clave.';

// =============================================================================
// CADENAS FALTANTES - FORMULARIO DE POSTULACIÓN
// =============================================================================

$string['skiptoform'] = 'Ir al formulario de postulación';
$string['indicatesrequired'] = 'indica campo requerido';
$string['pending'] = 'Pendiente';
$string['uploaded'] = 'Cargado';
$string['alldocumentsuploaded'] = '¡Todos los documentos han sido cargados!';
$string['textcontent'] = 'Contenido de texto';
$string['rejectreason_prompt'] = 'Ingrese el motivo del rechazo:';
$string['rejectreason_required'] = 'Debe ingresar un motivo para rechazar el documento.';
$string['consentaccepted'] = 'Consentimiento de procesamiento de datos';
$string['consentaccepted_help'] = 'Al marcar esta casilla, usted consiente el procesamiento de sus datos personales con fines de reclutamiento de acuerdo con las regulaciones de protección de datos.';
$string['digitalsignature_help'] = 'Escriba su nombre completo exactamente como aparece en sus documentos oficiales. Esto sirve como su firma electrónica.';
$string['coverletter_help'] = 'Escriba una breve carta de presentación explicando su interés en el puesto y por qué es un candidato adecuado.';
$string['declarationaccepted'] = 'Declaración de veracidad';
$string['declarationaccepted_help'] = 'Al marcar esta casilla, usted declara que toda la información proporcionada en esta postulación es verdadera y precisa según su mejor conocimiento.';

// =============================================================================
// CADENAS FALTANTES - CATEGORÍAS DE DOCUMENTOS
// =============================================================================

$string['doccat_employment_desc'] = 'Certificados laborales y documentos relacionados con el trabajo';
$string['doccat_identification_desc'] = 'Documentos de identificación oficiales y certificados personales';
$string['doccat_academic_desc'] = 'Credenciales académicas, títulos y certificados educativos';
$string['doccat_financial_desc'] = 'Documentos financieros y certificados bancarios';
$string['doccat_health_desc'] = 'Certificados de salud y documentación médica';
$string['doccat_legal_desc'] = 'Documentos legales, antecedentes y certificados judiciales';

// =============================================================================
// CADENAS FALTANTES - MÚLTIPLES DOCUMENTOS
// =============================================================================

$string['multipledocs_certificacion_laboral'] = 'Puede cargar múltiples certificados laborales. Cada certificado debe ser de un empleador o puesto diferente.';
$string['multipledocs_titulo_academico'] = 'Puede cargar múltiples títulos académicos. Incluya todos los grados y certificaciones relevantes.';
$string['multipledocs_formacion_complementaria'] = 'Puede cargar múltiples certificados de formación complementaria. Incluya todos los cursos y talleres relevantes.';

// =============================================================================
// CADENAS DE LISTA DE VERIFICACIÓN
// =============================================================================

$string['allcomplete'] = '¡Todo completo!';

// =============================================================================
// CADENAS DE FORMULARIO DE POSTULACIÓN (Reconstruidas)
// =============================================================================

$string['applicationform'] = 'Formulario de Postulación';
$string['applicationform_desc'] = 'Complete todos los campos obligatorios y cargue los documentos necesarios para enviar su postulación.';
$string['applicationsubmitted'] = 'Su postulación ha sido enviada exitosamente.';
$string['applicationsubmitfailed'] = 'Error al enviar la postulación. Por favor intente nuevamente.';
$string['applicationcreatefailed'] = 'Error al crear el registro de postulación.';
$string['declarationtitle'] = 'Declaración de Veracidad';
$string['obtaindocument'] = 'Obtener este documento';
$string['missingvacancy'] = 'Falta información de la vacante.';

// =============================================================================
// CADENAS DE PÁGINA DE PERFIL
// =============================================================================

$string['profilerequired_title'] = 'Completar perfil requerido';
$string['profilerequired_desc'] = 'Por favor complete todos los campos requeridos en su perfil antes de continuar con su postulación.';
$string['profilechecklist'] = 'Lista de verificación del perfil';
$string['profile_field_names'] = 'Nombre y apellidos';
$string['profile_field_email'] = 'Correo electrónico';
$string['profile_field_phone'] = 'Número de teléfono';
$string['profile_field_idnumber'] = 'Número de identificación';
$string['profile_required_note'] = 'Los campos marcados en amarillo necesitan ser completados.';
$string['profile_help_text'] = 'Si necesita ayuda para completar su perfil, por favor contacte a soporte.';
$string['completeprofiletoapply'] = 'Complete su perfil para continuar con su postulación.';

// =============================================================================
// CADENAS DE PÁGINA DE REVISIÓN
// =============================================================================

$string['reviewqueue_desc'] = 'Revise y valide los documentos de los postulantes enviados para las vacantes.';
$string['reviewapplication'] = 'Revisar postulación';
$string['selectdocumenttopreview'] = 'Seleccione un documento de la lista para previsualizarlo aquí.';
$string['sequentialreview'] = 'Revisión secuencial';
$string['reviewingdocument'] = 'Revisando documento';
$string['readytosubmit'] = 'Todos los documentos revisados. Puede enviar su revisión ahora.';
$string['alldocsreviewed'] = 'Todos los documentos revisados';
$string['submitreview'] = 'Enviar revisión';
$string['reviewobservations_placeholder'] = 'Agregue cualquier observación final sobre esta postulación...';
$string['fullscreen'] = 'Pantalla completa';

// =============================================================================
// CADENAS DE PÁGINA DE GESTIÓN DE POSTULACIONES
// =============================================================================

$string['searchapplicant'] = 'Buscar por nombre o correo...';
$string['allstatuses'] = 'Todos los estados';
$string['applicantslist'] = 'Lista de postulantes';
$string['noapplicationsfound_desc'] = 'No hay postulaciones que coincidan con sus filtros actuales. Intente ajustar sus criterios de búsqueda.';
$string['exportdesc'] = 'Descargue los datos de las postulaciones en su formato preferido para análisis fuera de línea.';

// =============================================================================
// CADENAS DE FILTRO Y UX
// =============================================================================

$string['filterbyvacancy'] = 'Filtrar por vacante';
$string['selectvacancytofilter'] = 'Seleccione una vacante para filtrar las postulaciones';
$string['applyfilters'] = 'Aplicar filtros';
$string['totaldocuments'] = 'Total de documentos';
$string['allreviewed'] = 'Todos revisados';
$string['urgent'] = 'Urgente';

// =============================================================================
// TIPOS DE DOCUMENTOS - v3.6.38
// =============================================================================

$string['doctype_experiencia_docente'] = 'Experiencia Docente';
$string['doctype_experiencia_profesional'] = 'Experiencia Profesional';
$string['doctype_formacion_pedagogia'] = 'Formación en Pedagogía';
$string['doctype_formacion_tic'] = 'Formación en TIC';
$string['doctype_formacion_complementaria'] = 'Certificaciones Complementarias';
$string['doctype_titulo_academico'] = 'Títulos Pregrado y Posgrado';
$string['doctype_bienes_rentas'] = 'Formato Declaración de Bienes y Rentas';
$string['doctype_antecedentes_judiciales'] = 'Antecedentes Judiciales';
$string['doctype_medidas_correctivas'] = 'Registro Nacional de Medidas Correctivas';
$string['doctype_inhabilidades'] = 'Consulta de Inhabilidades';
$string['doctype_redam'] = 'REDAM';

// =============================================================================
// FILTROS DE VACANTES - v3.6.38
// =============================================================================

$string['filterbycode'] = 'Buscar por código...';
$string['filtering'] = 'Filtrando...';
$string['modality'] = 'Modalidad';
$string['modality_presencial'] = 'Presencial';
$string['modality_virtual'] = 'Virtual';
$string['modality_hibrida'] = 'Híbrida';
$string['modality_distancia'] = 'A Distancia';
$string['department'] = 'Programa Académico';

// =============================================================================
// TOURS DE USUARIO - GENERAL
// =============================================================================

$string['tour_endlabel'] = 'Finalizar tour';

// =============================================================================
// TOUR DE USUARIO: TABLERO
// =============================================================================

$string['tour_dashboard_name'] = 'Tour del Tablero de Convocatorias';
$string['tour_dashboard_desc'] = 'Aprenda a navegar por el tablero de convocatorias y descubra sus características principales.';
$string['tour_dashboard_welcome_title'] = 'Bienvenido al Tablero de Convocatorias';
$string['tour_dashboard_welcome_content'] = 'Este es su tablero principal donde puede acceder a todas las funciones de convocatorias según su rol. Aquí encontrará acciones rápidas e información importante de un vistazo.';
$string['tour_dashboard_stats_title'] = 'Resumen de Estadísticas';
$string['tour_dashboard_stats_content'] = 'Estas tarjetas muestran estadísticas clave sobre vacantes, postulaciones y el proceso de revisión. Monitoree el estado de sus actividades de reclutamiento en tiempo real.';
$string['tour_dashboard_admin_title'] = 'Secciones de Administración';
$string['tour_dashboard_admin_content'] = 'Como administrador, puede gestionar convocatorias, vacantes, postulaciones y la configuración del sistema desde estas secciones. Cada tarjeta proporciona acceso rápido a áreas específicas de gestión.';
$string['tour_dashboard_reviewer_title'] = 'Tareas de Revisor';
$string['tour_dashboard_reviewer_content'] = 'Si es revisor de documentos, esta sección muestra sus revisiones pendientes y el trabajo completado. Haga clic en "Ver Mis Revisiones" para acceder a las postulaciones asignadas.';
$string['tour_dashboard_applicant_title'] = 'Mis Postulaciones';
$string['tour_dashboard_applicant_content'] = 'Como aspirante, puede ver sus postulaciones enviadas, seguir su estado y explorar las vacantes disponibles desde esta sección.';

// =============================================================================
// TOUR DE USUARIO: PÁGINA PÚBLICA
// =============================================================================

$string['tour_public_name'] = 'Tour de la Página Pública';
$string['tour_public_desc'] = 'Descubra cómo explorar y postularse a las vacantes disponibles en la página pública.';
$string['tour_public_welcome_title'] = 'Bienvenido al Portal de Convocatorias';
$string['tour_public_welcome_content'] = 'Esta es la página pública donde se listan todas las oportunidades laborales disponibles. Puede explorar las vacantes sin iniciar sesión, pero necesitará registrarse o iniciar sesión para postularse.';
$string['tour_public_stats_title'] = 'Oportunidades Disponibles';
$string['tour_public_stats_content'] = 'Estas estadísticas muestran el número actual de convocatorias abiertas y vacantes disponibles. Esté atento a estos números para nuevas oportunidades.';
$string['tour_public_convocatorias_title'] = 'Convocatorias Activas';
$string['tour_public_convocatorias_content'] = 'Las convocatorias son llamados que agrupan vacantes relacionadas. Haga clic en una tarjeta de convocatoria para ver todas sus posiciones asociadas y requisitos.';
$string['tour_public_vacancies_title'] = 'Vacantes Disponibles';
$string['tour_public_vacancies_content'] = 'Cada tarjeta de vacante muestra el título del cargo, ubicación, modalidad y requisitos clave. Haga clic en una tarjeta para ver los detalles completos e instrucciones de postulación.';
$string['tour_public_filters_title'] = 'Opciones de Filtro';
$string['tour_public_filters_content'] = 'Use estos filtros para reducir las vacantes por convocatoria, modalidad, ubicación o palabra clave. Esto le ayuda a encontrar posiciones que coincidan con su perfil.';
$string['tour_public_apply_title'] = 'Postularse Ahora';
$string['tour_public_apply_content'] = 'Cuando encuentre una vacante que le interese, haga clic en el botón "Postularse" o "Ver Detalles". Necesitará crear una cuenta o iniciar sesión para enviar su postulación.';

// =============================================================================
// TOUR DE USUARIO: POSTULARSE A VACANTE
// =============================================================================

$string['tour_apply_name'] = 'Tour del Proceso de Postulación';
$string['tour_apply_desc'] = 'Aprenda a completar y enviar su postulación paso a paso.';
$string['tour_apply_header_title'] = 'Formulario de Postulación';
$string['tour_apply_header_content'] = 'Este es el formulario de postulación para la vacante seleccionada. Siga los pasos cuidadosamente para asegurar que su postulación esté completa y precisa.';
$string['tour_apply_progress_title'] = 'Pasos de Progreso';
$string['tour_apply_progress_content'] = 'Estos pasos muestran su progreso a través del proceso de postulación. Complete cada sección antes de pasar a la siguiente.';
$string['tour_apply_guidelines_title'] = 'Instrucciones de Postulación';
$string['tour_apply_guidelines_content'] = 'Lea estas instrucciones cuidadosamente antes de iniciar su postulación. Contienen información importante sobre documentos requeridos y fechas límite.';
$string['tour_apply_form_title'] = 'Carga de Documentos';
$string['tour_apply_form_content'] = 'Cargue todos los documentos requeridos aquí. Asegúrese de que cada archivo cumpla con los requisitos de formato y tamaño. Los documentos requeridos están marcados con asterisco (*).';
$string['tour_apply_sidebar_title'] = 'Información de la Vacante';
$string['tour_apply_sidebar_content'] = 'Esta barra lateral muestra detalles sobre la vacante a la que se está postulando, incluyendo requisitos del cargo y fechas importantes.';
$string['tour_apply_checklist_title'] = 'Lista de Documentos';
$string['tour_apply_checklist_content'] = 'Siga el progreso de su carga de documentos con esta lista. Las marcas verdes indican documentos cargados, mientras que los círculos vacíos muestran elementos pendientes.';
$string['tour_apply_submit_title'] = 'Enviar Postulación';
$string['tour_apply_submit_content'] = 'Una vez que haya cargado todos los documentos requeridos y revisado su información, haga clic en este botón para enviar su postulación. No podrá modificarla después del envío.';
$string['tour_apply_deadline_title'] = 'Fecha Límite de Postulación';
$string['tour_apply_deadline_content'] = 'Esta etiqueta muestra cuántos días quedan antes de que cierre la convocatoria. Asegúrese de enviar su postulación antes de que expire el plazo.';
$string['tour_apply_help_title'] = '¿Necesita Ayuda?';
$string['tour_apply_help_content'] = 'Si tiene preguntas sobre el proceso de postulación o necesita asistencia, encuentre información de contacto y recursos de soporte en esta sección.';

// =============================================================================
// TOUR DE USUARIO: LISTA DE VACANTES
// =============================================================================

$string['tour_vacancies_name'] = 'Tour de Gestión de Vacantes';
$string['tour_vacancies_desc'] = 'Aprenda a explorar y gestionar las vacantes en el sistema.';
$string['tour_vacancies_header_title'] = 'Página de Vacantes';
$string['tour_vacancies_header_content'] = 'Esta página muestra todas las vacantes en el sistema. Como administrador, puede crear, editar y gestionar los estados de las vacantes desde aquí.';
$string['tour_vacancies_selector_title'] = 'Filtrar por Convocatoria';
$string['tour_vacancies_selector_content'] = 'Use este desplegable para filtrar vacantes por su convocatoria asociada. Esto le ayuda a gestionar vacantes para llamados específicos.';
$string['tour_vacancies_card_title'] = 'Tarjetas de Vacantes';
$string['tour_vacancies_card_content'] = 'Cada tarjeta representa una vacante con su código, título e información clave. Las tarjetas están codificadas por color según el estado para identificación rápida.';
$string['tour_vacancies_status_title'] = 'Etiquetas de Estado';
$string['tour_vacancies_status_content'] = 'La etiqueta muestra el estado actual de cada vacante: Borrador (no publicada), Publicada (aceptando postulaciones), Cerrada (ya no acepta) o Asignada (posición cubierta).';
$string['tour_vacancies_actions_title'] = 'Acciones Rápidas';
$string['tour_vacancies_actions_content'] = 'Use estos botones para ver, editar, publicar o cerrar una vacante. Las acciones disponibles dependen del estado actual de la vacante y sus permisos.';

// =============================================================================
// TOUR DE USUARIO: CONVOCATORIAS
// =============================================================================

$string['tour_convocatorias_name'] = 'Tour de Gestión de Convocatorias';
$string['tour_convocatorias_desc'] = 'Aprenda a crear y gestionar convocatorias.';
$string['tour_convocatorias_header_title'] = 'Página de Convocatorias';
$string['tour_convocatorias_header_content'] = 'Esta página muestra todas las convocatorias en el sistema. Las convocatorias agrupan vacantes relacionadas bajo un período de postulación único.';
$string['tour_convocatorias_create_title'] = 'Crear Nueva Convocatoria';
$string['tour_convocatorias_create_content'] = 'Haga clic en este botón para crear una nueva convocatoria. Definirá el período de postulación, términos, y luego podrá agregar vacantes.';
$string['tour_convocatorias_stats_title'] = 'Estadísticas';
$string['tour_convocatorias_stats_content'] = 'Estas tarjetas muestran el conteo de convocatorias por estado: Abiertas (aceptando postulaciones), Cerradas (finalizadas) y Borrador (aún no publicadas).';
$string['tour_convocatorias_filter_title'] = 'Opciones de Filtro';
$string['tour_convocatorias_filter_content'] = 'Use estos filtros para encontrar convocatorias específicas por estado, rango de fechas o búsqueda por palabra clave.';
$string['tour_convocatorias_card_title'] = 'Tarjetas de Convocatoria';
$string['tour_convocatorias_card_content'] = 'Cada tarjeta muestra una convocatoria con su código, nombre, fechas y cantidad de vacantes. Haga clic en una tarjeta para ver sus detalles y vacantes asociadas.';
$string['tour_convocatorias_actions_title'] = 'Menú de Acciones';
$string['tour_convocatorias_actions_content'] = 'Use estos botones para ver detalles, editar, abrir, cerrar o eliminar una convocatoria. Las acciones disponibles dependen del estado actual.';

// =============================================================================
// TOUR DE USUARIO: REVISIÓN DE DOCUMENTOS
// =============================================================================

$string['tour_review_name'] = 'Tour de Revisión de Documentos';
$string['tour_review_desc'] = 'Aprenda a revisar y validar documentos de aspirantes de manera efectiva.';
$string['tour_review_header_title'] = 'Interfaz de Revisión de Documentos';
$string['tour_review_header_content'] = 'Esta es la interfaz de revisión de documentos donde valida las presentaciones de los aspirantes. Cada postulación contiene documentos que necesitan ser verificados.';
$string['tour_review_stats_title'] = 'Estadísticas de Revisión';
$string['tour_review_stats_content'] = 'Estas tarjetas muestran el progreso de revisión: total de documentos, validados, rechazados y pendientes de revisión. Siga su carga de trabajo de un vistazo.';
$string['tour_review_navigation_title'] = 'Navegación';
$string['tour_review_navigation_content'] = 'Navegue entre postulaciones usando estos controles. Puede moverse a la siguiente revisión pendiente o saltar a una postulación específica.';
$string['tour_review_documents_title'] = 'Lista de Documentos';
$string['tour_review_documents_content'] = 'Cada elemento en esta lista representa un documento enviado por el aspirante. Haga clic en un documento para previsualizarlo y realizar la validación.';
$string['tour_review_actions_title'] = 'Acciones de Validación';
$string['tour_review_actions_content'] = 'Use estos botones para aprobar o rechazar documentos. También puede agregar notas y seleccionar razones específicas de rechazo.';
$string['tour_review_applicant_title'] = 'Información del Aspirante';
$string['tour_review_applicant_content'] = 'Este panel muestra la información del perfil del aspirante incluyendo nombre, datos de contacto e historial de postulaciones.';
$string['tour_review_progress_title'] = 'Progreso de Revisión';
$string['tour_review_progress_content'] = 'Esta barra de progreso muestra cuántos documentos ha revisado para la postulación actual. Complete todas las revisiones para finalizar la validación.';
$string['tour_review_submit_title'] = 'Completar Revisión';
$string['tour_review_submit_content'] = 'Después de revisar todos los documentos, haga clic en este botón para finalizar la revisión de la postulación. Esto actualizará el estado de la postulación correspondientemente.';

// =============================================================================
// TOUR DE USUARIO: MIS POSTULACIONES
// =============================================================================

$string['tour_myapplications_name'] = 'Tour de Mis Postulaciones';
$string['tour_myapplications_desc'] = 'Aprenda a seguir y gestionar sus postulaciones laborales.';
$string['tour_myapplications_header_title'] = 'Mis Postulaciones';
$string['tour_myapplications_header_content'] = 'Esta página muestra todas sus postulaciones enviadas. Siga su estado y vea los comentarios de los revisores.';
$string['tour_myapplications_filters_title'] = 'Filtrar Postulaciones';
$string['tour_myapplications_filters_content'] = 'Use estos filtros para encontrar postulaciones específicas por estado, vacante o fecha de envío.';
$string['tour_myapplications_card_title'] = 'Tarjetas de Postulación';
$string['tour_myapplications_card_content'] = 'Cada tarjeta representa una de sus postulaciones mostrando el nombre de la vacante, fecha de envío y estado actual.';
$string['tour_myapplications_status_title'] = 'Estado de Postulación';
$string['tour_myapplications_status_content'] = 'La etiqueta de estado muestra dónde está su postulación en el proceso de revisión: Enviada, En Revisión, Documentos Validados, Entrevista Programada, Seleccionado o Rechazado.';
$string['tour_myapplications_actions_title'] = 'Ver Detalles';
$string['tour_myapplications_actions_content'] = 'Haga clic en este botón para ver su postulación completa incluyendo documentos cargados, comentarios del revisor y cualquier acción requerida.';

// =============================================================================
// TOUR DE USUARIO: MIS REVISIONES (REVISOR)
// =============================================================================

$string['tour_myreviews_name'] = 'Tour de Mis Revisiones';
$string['tour_myreviews_desc'] = 'Aprenda a gestionar sus revisiones de documentos asignadas.';
$string['tour_myreviews_header_title'] = 'Mi Cola de Revisiones';
$string['tour_myreviews_header_content'] = 'Esta página muestra todas las postulaciones asignadas a usted para revisión de documentos. Priorice las revisiones pendientes para mantener el proceso de contratación en movimiento.';
$string['tour_myreviews_stats_title'] = 'Estadísticas de Revisión';
$string['tour_myreviews_stats_content'] = 'Siga su carga de trabajo de revisión con estas estadísticas: revisiones pendientes, completadas hoy y total revisadas este mes.';
$string['tour_myreviews_pending_title'] = 'Revisiones Pendientes';
$string['tour_myreviews_pending_content'] = 'Estas postulaciones están esperando su revisión. Haga clic en cualquier tarjeta para iniciar el proceso de validación de documentos.';
$string['tour_myreviews_completed_title'] = 'Revisiones Completadas';
$string['tour_myreviews_completed_content'] = 'Aquí puede ver las postulaciones que ya ha revisado. Puede volver a visitarlas si es necesario.';
$string['tour_myreviews_start_title'] = 'Iniciar Revisión';
$string['tour_myreviews_start_content'] = 'Haga clic en este botón para comenzar a revisar los documentos de la postulación. Será llevado a la interfaz de validación de documentos.';

// =============================================================================
// TOUR DE USUARIO: DETALLE DE POSTULACIÓN
// =============================================================================

$string['tour_application_name'] = 'Tour de Detalle de Postulación';
$string['tour_application_desc'] = 'Aprenda a ver y entender los detalles de su postulación.';
$string['tour_application_header_title'] = 'Detalles de Postulación';
$string['tour_application_header_content'] = 'Esta página muestra los detalles completos de su postulación incluyendo estado, documentos enviados y comentarios del revisor.';
$string['tour_application_status_title'] = 'Estado Actual';
$string['tour_application_status_content'] = 'Esta sección muestra el estado actual de su postulación y cualquier mensaje importante del equipo de revisión.';
$string['tour_application_documents_title'] = 'Documentos Enviados';
$string['tour_application_documents_content'] = 'Vea todos los documentos que envió con esta postulación. Cada documento muestra su estado de validación y cualquier comentario del revisor.';
$string['tour_application_timeline_title'] = 'Línea de Tiempo de Postulación';
$string['tour_application_timeline_content'] = 'Esta línea de tiempo muestra el historial de su postulación desde el envío hasta cada etapa de revisión.';
$string['tour_application_actions_title'] = 'Acciones Disponibles';
$string['tour_application_actions_content'] = 'Dependiendo del estado de su postulación, puede volver a cargar documentos, retirar su postulación o completar pasos adicionales.';

// =============================================================================
// TOUR DE USUARIO: DETALLE DE VACANTE
// =============================================================================

$string['tour_vacancy_name'] = 'Tour de Detalle de Vacante';
$string['tour_vacancy_desc'] = 'Aprenda a ver los detalles y requisitos de una vacante.';
$string['tour_vacancy_header_title'] = 'Detalles de la Vacante';
$string['tour_vacancy_header_content'] = 'Esta página muestra información completa sobre la vacante incluyendo requisitos, responsabilidades e instrucciones de postulación.';
$string['tour_vacancy_info_title'] = 'Información del Cargo';
$string['tour_vacancy_info_content'] = 'Aquí encontrará el título del cargo, departamento, ubicación, modalidad y detalles del contrato.';
$string['tour_vacancy_requirements_title'] = 'Requisitos';
$string['tour_vacancy_requirements_content'] = 'Esta sección lista las calificaciones requeridas, experiencia y habilidades para el cargo. Asegúrese de cumplir con estos antes de postularse.';
$string['tour_vacancy_documents_title'] = 'Documentos Requeridos';
$string['tour_vacancy_documents_content'] = 'Revise la lista de documentos que necesitará enviar con su postulación. Prepárelos antes de iniciar su postulación.';
$string['tour_vacancy_apply_title'] = 'Postularse Ahora';
$string['tour_vacancy_apply_content'] = 'Si cumple con los requisitos, haga clic en este botón para iniciar su postulación. Será guiado a través del proceso de carga de documentos.';

// =============================================================================
// TOUR DE USUARIO: GESTIONAR VACANTES
// =============================================================================

$string['tour_manage_name'] = 'Tour de Gestión de Vacantes';
$string['tour_manage_desc'] = 'Aprenda a gestionar y administrar las vacantes.';
$string['tour_manage_header_title'] = 'Gestión de Vacantes';
$string['tour_manage_header_content'] = 'Esta página proporciona herramientas completas para gestionar todas las vacantes en el sistema incluyendo creación, edición y gestión de estados.';
$string['tour_manage_create_title'] = 'Crear Vacante';
$string['tour_manage_create_content'] = 'Haga clic en este botón para crear una nueva vacante. Primero seleccionará una convocatoria, luego completará los detalles del cargo.';
$string['tour_manage_filters_title'] = 'Filtros Avanzados';
$string['tour_manage_filters_content'] = 'Use estos filtros para encontrar vacantes específicas por estado, convocatoria, rango de fechas o palabra clave.';
$string['tour_manage_bulk_title'] = 'Acciones Masivas';
$string['tour_manage_bulk_content'] = 'Seleccione múltiples vacantes para realizar acciones masivas como publicar, cerrar o exportar. Use las casillas de verificación para seleccionar elementos.';
$string['tour_manage_table_title'] = 'Tabla de Vacantes';
$string['tour_manage_table_content'] = 'Esta tabla muestra todas las vacantes con columnas ordenables. Haga clic en los encabezados de columna para ordenar, o use los botones de acción para gestión individual.';
$string['tour_manage_actions_title'] = 'Acciones Rápidas';
$string['tour_manage_actions_content'] = 'Cada fila tiene botones de acción para ver, editar, publicar o gestionar postulaciones para esa vacante.';

// =============================================================================
// TOUR DE USUARIO: DOCUMENTOS
// =============================================================================

$string['tour_documents_name'] = 'Tour de Gestión de Documentos';
$string['tour_documents_desc'] = 'Aprenda a gestionar y revisar documentos de postulaciones.';
$string['tour_documents_header_title'] = 'Gestión de Documentos';
$string['tour_documents_header_content'] = 'Esta página le permite ver y gestionar todos los documentos enviados con las postulaciones.';
$string['tour_documents_filters_title'] = 'Filtrar Documentos';
$string['tour_documents_filters_content'] = 'Use estos filtros para encontrar documentos por tipo, estado o nombre del aspirante.';
$string['tour_documents_list_title'] = 'Lista de Documentos';
$string['tour_documents_list_content'] = 'Cada fila muestra un documento con su tipo, aspirante, fecha de carga y estado de validación.';
$string['tour_documents_preview_title'] = 'Previsualizar Documentos';
$string['tour_documents_preview_content'] = 'Haga clic en el botón de previsualización para ver documentos sin descargarlos. Esto ayuda a acelerar el proceso de revisión.';
$string['tour_documents_validate_title'] = 'Validar Documentos';
$string['tour_documents_validate_content'] = 'Use los botones de validación para aprobar o rechazar documentos. Agregue notas para proporcionar retroalimentación a los aspirantes.';

// =============================================================================
// TOUR DE USUARIO: VALIDAR DOCUMENTO
// =============================================================================

$string['tour_validate_document_name'] = 'Tour de Validación de Documentos';
$string['tour_validate_document_desc'] = 'Aprenda el proceso paso a paso para validar documentos de aspirantes.';
$string['tour_validate_document_header_title'] = 'Validación de Documentos';
$string['tour_validate_document_header_content'] = 'Esta interfaz le permite revisar y validar documentos individuales de los aspirantes a fondo.';
$string['tour_validate_document_preview_title'] = 'Previsualización del Documento';
$string['tour_validate_document_preview_content'] = 'El documento se muestra aquí para su revisión. Puede hacer zoom, desplazarse por las páginas y verificar todos los detalles.';
$string['tour_validate_document_checklist_title'] = 'Lista de Verificación';
$string['tour_validate_document_checklist_content'] = 'Use esta lista de verificación para confirmar que el documento cumple con todos los requisitos. Marque cada elemento a medida que lo verifica.';
$string['tour_validate_document_notes_title'] = 'Notas del Revisor';
$string['tour_validate_document_notes_content'] = 'Agregue notas para explicar su decisión de validación. Estas notas ayudan al aspirante a entender qué necesita corregir si es rechazado.';
$string['tour_validate_document_decision_title'] = 'Tomar Decisión';
$string['tour_validate_document_decision_content'] = 'Seleccione Aprobar si el documento es válido, o Rechazar si necesita correcciones. Asegúrese de agregar notas explicando cualquier rechazo.';
$string['tour_validate_document_submit_title'] = 'Enviar Validación';
$string['tour_validate_document_submit_content'] = 'Haga clic en este botón para enviar su decisión de validación. El aspirante será notificado del resultado.';

// =============================================================================
// TOUR DE USUARIO: REPORTES
// =============================================================================

$string['tour_reports_name'] = 'Tour del Tablero de Reportes';
$string['tour_reports_desc'] = 'Aprenda a acceder y generar reportes de los datos de convocatorias.';
$string['tour_reports_header_title'] = 'Tablero de Reportes';
$string['tour_reports_header_content'] = 'Esta página proporciona acceso a varios reportes y análisis sobre el proceso de reclutamiento.';
$string['tour_reports_overview_title'] = 'Estadísticas Generales';
$string['tour_reports_overview_content'] = 'Estos gráficos y números le dan un resumen rápido de la actividad de reclutamiento, volúmenes de postulaciones y tiempos de procesamiento.';
$string['tour_reports_filters_title'] = 'Filtros de Reportes';
$string['tour_reports_filters_content'] = 'Use estos filtros para personalizar el rango de fechas, convocatoria u otros parámetros para sus reportes.';
$string['tour_reports_types_title'] = 'Tipos de Reportes';
$string['tour_reports_types_content'] = 'Seleccione entre diferentes tipos de reportes: Postulaciones por Estado, Documentos por Tipo, Tiempos de Procesamiento y más.';
$string['tour_reports_export_title'] = 'Exportar Datos';
$string['tour_reports_export_content'] = 'Descargue reportes en varios formatos (PDF, Excel, CSV) para análisis fuera de línea o compartir con interesados.';

// =============================================================================
// TOUR DE USUARIO: GESTIÓN DE CONVOCATORIA (CREAR/EDITAR)
// =============================================================================

$string['tour_convocatoria_manage_name'] = 'Tour del Formulario de Convocatoria';
$string['tour_convocatoria_manage_desc'] = 'Aprenda a crear y configurar una convocatoria.';
$string['tour_convocatoria_manage_header_title'] = 'Formulario de Convocatoria';
$string['tour_convocatoria_manage_header_content'] = 'Use este formulario para crear o editar una convocatoria. Complete todos los campos requeridos para definir su llamado.';
$string['tour_convocatoria_manage_basic_title'] = 'Información Básica';
$string['tour_convocatoria_manage_basic_content'] = 'Ingrese el código, nombre y descripción de la convocatoria. El código debe ser único y ayudar a identificar este llamado.';
$string['tour_convocatoria_manage_dates_title'] = 'Configuración de Fechas';
$string['tour_convocatoria_manage_dates_content'] = 'Establezca las fechas de inicio y fin para aceptar postulaciones. Asegúrese de dar suficiente tiempo para que los aspirantes envíen sus documentos.';
$string['tour_convocatoria_manage_settings_title'] = 'Configuración de Postulación';
$string['tour_convocatoria_manage_settings_content'] = 'Configure si los aspirantes pueden postularse a múltiples vacantes dentro de esta convocatoria y establezca cualquier límite de postulaciones.';
$string['tour_convocatoria_manage_terms_title'] = 'Términos y Condiciones';
$string['tour_convocatoria_manage_terms_content'] = 'Defina los términos que los aspirantes deben aceptar al postularse. Esto puede incluir políticas de privacidad y acuerdos de manejo de datos.';
$string['tour_convocatoria_manage_save_title'] = 'Guardar Convocatoria';
$string['tour_convocatoria_manage_save_content'] = 'Haga clic en Guardar para crear o actualizar la convocatoria. Puede agregar vacantes después de guardar.';

// =============================================================================
// FLUJO DE TRABAJO DECANO/TALENTO HUMANO v4.0
// =============================================================================

// Campos de convocatoria para fechas de revisión
$string['reviewdates'] = 'Fechas de revisión';
$string['reviewdates_help'] = 'Configure las fechas en las cuales cada rol puede acceder a revisar las postulaciones.';
$string['dean_review_startdate'] = 'Inicio revisión Decano';
$string['dean_review_startdate_help'] = 'Fecha desde la cual el Decano puede revisar los perfiles de postulantes.';
$string['dean_review_enddate'] = 'Fin revisión Decano';
$string['dean_review_enddate_help'] = 'Fecha hasta la cual el Decano puede revisar y aprobar/rechazar perfiles.';
$string['hr_review_startdate'] = 'Inicio validación Talento Humano';
$string['hr_review_startdate_help'] = 'Fecha desde la cual Talento Humano puede validar documentos.';
$string['hr_review_enddate'] = 'Fin validación Talento Humano';
$string['hr_review_enddate_help'] = 'Fecha hasta la cual Talento Humano puede realizar la validación final.';

// Mensajes de flujo de trabajo
$string['convocatoria_closed_advancing'] = 'Convocatoria cerrada, postulación avanzada a revisión de Decano';
$string['profile_approved_advancing'] = 'Perfil aprobado por Decano, avanzando a validación de Talento Humano';
$string['profile_approved'] = 'Perfil aprobado';
$string['profile_rejected'] = 'Perfil rechazado';
$string['hr_validation_complete'] = 'Validación de Talento Humano completada';

// Errores de acceso por fechas
$string['error:dean_access_dates'] = 'El período de revisión del Decano no está activo para esta convocatoria';
$string['error:hr_access_dates'] = 'El período de validación de Talento Humano no está activo para esta convocatoria';
$string['error:invalidtransition'] = 'Transición de estado no permitida';
$string['error:dean_cannot_validate_docs'] = 'Los Decanos no pueden aprobar/rechazar documentos individuales. Solo pueden aprobar/rechazar la postulación completa.';

// Capabilities
$string['jobboard:reviewprofiles'] = 'Revisar perfiles de postulantes';
$string['jobboard:approveprofile'] = 'Aprobar/rechazar perfiles completos';
$string['jobboard:validatehr'] = 'Validación final de documentos por Talento Humano';

// Acciones
$string['approve_profile'] = 'Aprobar perfil';
$string['reject_profile'] = 'Rechazar perfil';
$string['validate_hr'] = 'Validar (Talento Humano)';
$string['reject_hr'] = 'Rechazar (Talento Humano)';

// Acciones del Decano
$string['deanactions'] = 'Acciones del Decano';
$string['dean_actions_help'] = 'Como Decano, puede aprobar o rechazar esta postulación completa. Si aprueba, el candidato pasará a preselección. Si rechaza, debe indicar el motivo.';
$string['approveprofile'] = 'Aprobar postulación';
$string['rejectprofile'] = 'Rechazar postulación';
$string['approvecomments_placeholder'] = 'Comentarios opcionales sobre la aprobación...';
$string['deanreviewpanel'] = 'Panel de Revisión del Decano';
$string['deanreviewpanel_desc'] = 'Revise y apruebe/rechace las postulaciones de sus facultades asignadas';
$string['nopendingapplications_dean_desc'] = 'No hay postulaciones con estado "enviada" pendientes de su revisión en sus facultades asignadas.';

// Estados del período de revisión
$string['review_period_not_configured'] = 'Período de revisión no configurado';
$string['review_period_pending'] = 'Período de revisión pendiente';
$string['review_period_active'] = 'Período de revisión activo';
$string['review_period_ended'] = 'Período de revisión finalizado';

// Errores de validación de fechas de revisión
$string['error:reviewdatebeforeend'] = 'La fecha de inicio de revisión debe ser posterior a la fecha de cierre de la convocatoria';
$string['error:hrreviewbeforedean'] = 'La fecha de inicio de validación de Talento Humano debe ser posterior a la fecha de fin de revisión del Decano';

// Mensajes de acciones
$string['action_deprecated'] = 'Esta acción está obsoleta. Use las nuevas acciones del flujo de trabajo.';
$string['nopermission'] = 'No tiene permiso para realizar esta acción';

// =============================================================================
// FAQ - PREGUNTAS FRECUENTES
// =============================================================================

$string['faq_title'] = 'Preguntas Frecuentes';
$string['faq_subtitle'] = 'Respuestas a las consultas más comunes sobre el proceso de postulación';

$string['faq_q1'] = '¿Cómo puedo postularme a una vacante?';
$string['faq_a1'] = 'Para postularse: 1) Cree una cuenta usando su número de identificación. 2) Inicie sesión y vaya a "Vacantes". 3) Seleccione la vacante de su interés y haga clic en "Postularse". 4) Complete todos los pasos del formulario y suba los documentos requeridos.';

$string['faq_q2'] = '¿Qué documentos necesito para postularme?';
$string['faq_a2'] = 'Los documentos varían según la vacante, pero generalmente incluyen: documento de identidad, hoja de vida, títulos académicos, certificados laborales y antecedentes. Cada vacante especifica los documentos obligatorios y opcionales.';

$string['faq_q3'] = '¿Cómo puedo ver el estado de mi postulación?';
$string['faq_a3'] = 'Inicie sesión con su cuenta y vaya a "Mis Postulaciones" en el panel lateral. Allí verá el estado actual de cada postulación, los documentos pendientes de validación y las observaciones de los revisores.';

$string['faq_q4'] = '¿Qué hago si rechazan uno de mis documentos?';
$string['faq_a4'] = 'Si un documento es rechazado, recibirá una notificación indicando el motivo. Puede subir un nuevo documento corregido mientras la convocatoria esté activa. Asegúrese de revisar los comentarios del revisor antes de reenviar.';

$string['faq_q5'] = '¿Puedo postularme a varias vacantes?';
$string['faq_a5'] = 'Depende de las condiciones de cada convocatoria. Antes de postularse, lea los términos y condiciones en los detalles de la convocatoria para verificar si se permite aplicar a más de una vacante. Por lo general, solo se permite una postulación por convocatoria.';

$string['faq_q6'] = '¿Olvidé mi contraseña, qué hago?';
$string['faq_a6'] = 'En la página de inicio de sesión, haga clic en "¿Olvidó su contraseña?". Ingrese su correo electrónico registrado y recibirá instrucciones para restablecer su contraseña.';

$string['faq_q7'] = '¿Hasta cuándo puedo postularme?';
$string['faq_a7'] = 'Cada convocatoria tiene fechas específicas de apertura y cierre. Verifique las fechas en la página de la convocatoria. Las postulaciones no se aceptan después de la fecha de cierre.';

$string['faq_q8'] = '¿Cómo me contactan si soy seleccionado?';
$string['faq_a8'] = 'Todas las comunicaciones se envían al correo electrónico registrado en su cuenta. Asegúrese de mantener actualizado su correo y revisar regularmente su bandeja de entrada, incluyendo la carpeta de spam.';

$string['faq_q9'] = '¿Qué hago si no recibo el correo de confirmación?';
$string['faq_a9'] = 'Después de registrarse, recibirá un correo para confirmar su cuenta. Si no lo recibe en unos minutos, revise su carpeta de spam o correo no deseado. Si aún no lo encuentra, puede solicitar el reenvío desde la página de inicio de sesión o contactar a soporte técnico.';

// =============================================================================
// CORREO DE CONFIRMACIÓN DE CUENTA
// =============================================================================

$string['confirm_email_subject'] = 'Confirme su cuenta - Bolsa de Empleo';
$string['confirm_email_greeting'] = '¡Bienvenido/a, {$a}!';
$string['confirm_email_intro'] = 'Gracias por registrarse en la Bolsa de Empleo. Su cuenta ha sido creada exitosamente.';
$string['confirm_email_action'] = 'Para activar su cuenta y comenzar a postularse a vacantes, haga clic en el siguiente botón:';
$string['confirm_email_button'] = 'Confirmar mi cuenta';
$string['confirm_email_link_alt'] = 'Si el botón no funciona, copie y pegue el siguiente enlace en su navegador:';
$string['confirm_email_expires'] = 'Este enlace de confirmación expirará en 24 horas.';
$string['confirm_email_next_steps_title'] = '¿Qué sigue después de confirmar?';
$string['confirm_email_step1'] = 'Complete su perfil profesional';
$string['confirm_email_step2'] = 'Explore las vacantes disponibles';
$string['confirm_email_step3'] = 'Prepare sus documentos';
$string['confirm_email_step4'] = 'Postúlese a las vacantes de su interés';
$string['confirm_email_help'] = 'Si tiene alguna duda, visite nuestra página de soporte o contáctenos.';
$string['confirm_email_ignore'] = 'Si usted no solicitó esta cuenta, puede ignorar este correo de forma segura.';
$string['confirm_email_footer'] = 'Este es un mensaje automático. Por favor no responda a este correo.';

// =============================================================================
// CADENAS DE AYUDA PARA FORMULARIO DE CONVOCATORIA
// =============================================================================

$string['convocatoriacode_help'] = 'Código identificador único de la convocatoria. Use un código corto y descriptivo (ej., CONV-2026-001).';
$string['convocatorianame_help'] = 'El nombre oficial de la convocatoria que se mostrará a los postulantes.';
$string['briefdescription'] = 'Descripción breve';
$string['briefdescription_help'] = 'Un resumen corto de la convocatoria que se mostrará en listados y vistas previas.';
$string['convocatoriadescription_help'] = 'Descripción detallada de la convocatoria, incluyendo objetivos, alcance y cualquier información relevante para los postulantes.';
$string['convocatoriastartdate_help'] = 'La fecha de apertura de la convocatoria para postulaciones. Los postulantes pueden comenzar a enviar desde esta fecha.';
$string['convocatoriaenddate_help'] = 'Fecha límite para enviar postulaciones. No se aceptarán postulaciones después de esta fecha.';

// Sección de fechas de revisión.
$string['reviewdates'] = 'Fechas de revisión';
$string['dean_review_startdate'] = 'Fecha de inicio de revisión del decano';
$string['dean_review_startdate_help'] = 'La fecha cuando los decanos pueden comenzar a revisar las postulaciones asignadas a su facultad.';
$string['dean_review_enddate'] = 'Fecha de fin de revisión del decano';
$string['dean_review_enddate_help'] = 'Fecha límite para que los decanos completen sus revisiones.';
$string['hr_review_startdate'] = 'Fecha de inicio de revisión de RRHH';
$string['hr_review_startdate_help'] = 'La fecha cuando Recursos Humanos puede comenzar a validar las postulaciones aprobadas por los decanos.';
$string['hr_review_enddate'] = 'Fecha de fin de revisión de RRHH';
$string['hr_review_enddate_help'] = 'Fecha límite para que Recursos Humanos complete sus validaciones.';

// Publicación y alcance.
$string['publicationtype_help'] = 'Seleccione si esta convocatoria es pública (visible para todos) o interna (solo visible para miembros de la organización).';
$string['convocatoria_companyid'] = 'Empresa/Institución';
$string['convocatoria_companyid_help'] = 'Seleccione la empresa o institución a la que pertenece esta convocatoria. Esto determina qué usuarios pueden ver y postularse.';
$string['convocatoria_departmentid'] = 'Departamento';
$string['convocatoria_departmentid_help'] = 'Opcional. Seleccione un departamento específico para restringir esta convocatoria a una unidad particular.';

// Términos y configuración.
$string['convocatoriaterms_help'] = 'Los términos y condiciones legales que los postulantes deben aceptar antes de enviar su postulación.';
$string['maxapplicationsperuser_help'] = 'Número máximo de postulaciones que un usuario puede enviar a vacantes en esta convocatoria. Configure en 0 para ilimitado.';
$string['exempteddoctypes_help'] = 'Tipos de documentos que pueden ser eximidos para postulantes con estado de exención válido.';
$string['exemptionreason_help'] = 'La razón o criterio para otorgar exenciones de documentos a los postulantes.';

// FUNCIONALIDAD TEMPORAL: Aviso de repostulación después de retiro.
// @todo Eliminar cuando la funcionalidad de repostulación sea deprecada.
$string['withdrawnapplication_reapply_notice'] = 'Usted tiene una postulación previamente retirada para esta vacante. Si hace clic en "Aplicar", su postulación retirada será reactivada como borrador, permitiéndole editarla y reenviarla.';
