 ...
 *
 * This page is accessible without authentication and displays public vacancies.
 * Authenticated users can also see internal vacancies for their organization.
 *
...
// Setup page - no login required for viewing public vacancies.
$context = context_system::instance();
$PAGE->set_context($context);
$PAGE->set_url(new moodle_url('/local/jobboard/public/index.php', [
    'page' => $page,
    'search' => $search,
...
echo html_writer::start_tag('form', [
    'method' => 'get',
    'action' => new moodle_url('/local/jobboard/public/index.php'),
    'class' => 'mb-4',
]);
...
if (empty($vacancies)) {
    ...
    foreach ($vacancies as $vacancy) {
        ...
        // Description excerpt.
        $description = shorten_text(strip_tags($vacancy->description), 120);
        ...
        // View details button.
        echo html_writer::link(
            new moodle_url('/local/jobboard/public/vacancy.php', ['id' => $vacancy->id]),
        ...
        if ($isloggedin && has_capability('local/jobboard:apply', $context)) {
            echo html_writer::link(
                new moodle_url('/local/jobboard/apply.php', ['id' => $vacancy->id]),
                get_string('apply', 'local_jobboard'),
                ['class' => 'btn btn-sm btn-primary']
            );
        } elseif (!$isloggedin) {
            echo html_writer::link(
                new moodle_url('/login/index.php', ['wantsurl' => (new moodle_url('/local/jobboard/apply.php', ['id' => $vacancy->id]))->out(false)]),
    ...
    // Pagination.
    $baseurl = new moodle_url('/local/jobboard/public/index.php', [
        'search' => $search,
        'contracttype' => $contracttype,
        'location' => $location,
        'type' => $publicationtype,
